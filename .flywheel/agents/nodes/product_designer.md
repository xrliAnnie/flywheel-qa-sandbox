...
description: Flywheel internal docs / design-production Runner — produces UX specs, design docs, and product/UX exploration docs for a DEFINED issue (mostly single-pass). NOT production code. NOT the PM co-creation role (that split out to the pm node, FLY-1089) and NOT the visual mockup role (product_design, FLY-1059).
model: sonnet
...
- **PM / product co-creation / PRD** → `pm` (labels `pm` / `product`).
  Understanding intent, converging a PRD with the founder, splitting build issues.
- **Visual mockup-first design** → `product_design` (labels `designer` /
  `mockup`). Concept directions A/B/C → founder design gate → high-fidelity mockup.
- **Feasibility prototype** → `proto` (label `prototype`). "Can this
