...
- **Audit the codebase first**; pick the right discipline: code-shaped work → follow the `engineer` node (TDD, its mandatory targeted-local-test policy, affected builds/typechecks, `codex:rescue` review, PR); doc / UX-spec / design-production → `product_designer`; PM / product co-creation / PRD → `pm`; visual mockup-first design → `product_design`; feasibility prototype → `proto`.
