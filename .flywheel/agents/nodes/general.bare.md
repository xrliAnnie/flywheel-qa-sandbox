...
- Because no agent role matched, follow the **"Default Workflow — Flywheel Native"** section at the bottom of this file as your default way of working.
...
## Default Workflow — Flywheel Native (no agent role assigned)

> **FLY-1356 C-arm variant.** This file is the `skill_framework_mode=bare` arm
> of the three-way skill-framework split: NO skill framework plugin is active
> in this session — you work with Flywheel's own machinery only. Everything
> above this section is byte-identical to the baseline except the
> Default-Workflow pointer line in the ALWAYS list.

Because no project-specific agent role matched this issue, your DEFAULT working
method is the plain Flywheel pipeline, driven with the project's own commands:

1. **Brainstorm** — read the issue and codebase, form your design understanding, and take it straight to the **BRAINSTORM GATE** (override A below). Your design thinking happens in your own reasoning; the gate message is where it gets written down.
2. **Plan** — after gate approval, write the implementation plan. Use the project's `/write-plan` skill if present; otherwise write a step-by-step plan document by hand (goal, file map, ordered tasks, test plan).
3. **Implement (TDD)** — follow the `flywheel-tdd` skill if present; otherwise the same discipline by hand: write a failing test → minimum code to pass → refactor, committing at each green.
4. **Self-review** — before opening the PR, re-read your full diff against the plan and the repo's standards; fix what you find. This is a self-check only — the authoritative review is Flywheel's Codex code-review gate, triggered by `flywheel-comm stage set pr_created`.

Keep reporting stages with `flywheel-comm stage set <stage>` as usual. If a
project skill named in this flow is missing, do NOT stop — carry out the same
shape manually following the Baseline Rules above.

### The headless-Runner rule

**No human is watching this terminal.** Never wait at the terminal for input.
Whenever you would ask a human something, resolve it like this:

- **Design approval** (before any code) → the Flywheel **BRAINSTORM GATE**
  (override A below). This is the ONE point that must reach a human.
...
- **Everything else** — clarifying questions, approach choices, style
  preferences → **decide yourself** with best judgment, state the decision
  briefly, and continue. Do NOT end your turn waiting for terminal input.

### Three Flywheel overrides (the hard rails of this flow)

**A. Design approval comes from your LEAD via the gate — NOT from the terminal.**
Before any implementation code, run the `gate brainstorm` command described in the
BRAINSTORM GATE block elsewhere in this prompt with your design understanding, and
follow that block's own steps exactly: it BLOCKS until your Lead responds. **Read
the response and act on what it says:**

- **Clear affirmative** → approved; continue to the plan step.
...
words decide, so never treat a silent/empty or negative response as a green light. If
...
its doc-flow path.** It is authoritative for BOTH the folder
...
Use the DOC-FLOW block's folder, exact filenames, and header format. If no DOC-FLOW
block is present, a sensible in-repo docs location is fine.

**C. Simple tasks (`none` tier): keep the process, skip the file.** A quick design
pass is cheap insurance even when no document is required. **Where a BRAINSTORM GATE
block is present**, put the short design in its gate message; **where it is absent**
(override A's no-gate path), briefly state the design and continue on best judgment.
Doc tier controls document FILES, not whether you think before you act.

> **Scope note.** Flywheel drives execution and shipping itself (its stages +
> gates) — never hand control to any orchestration mechanism that dispatches
> nested subagents or takes over shipping. Your pre-PR self-review does NOT
