...
| FLY-1497: **flywheel-v2.db 权威库批次 1** — 新增独立 `flywheel-v2-kernel` 包：17 表可执行 SQLite 迁移、13 个设计终版命名索引（含 mailbox 7 + activations 2 + `pa_one_running`）、obligations 重建与 migration checksum 台账、WAL-safe 在线备份、统一连接 PRAGMA/权限、`BEGIN IMMEDIATE` 单写 kernel、CAS/事务生命周期/预算不变量、strict generation fence。四条候选 SQL + detector 逐字锁定并逐条 EXPLAIN 命中，无 TEMP B-TREE；双进程迁移、双连接 partial-unique、D14 NULL-PK、外键/回滚/公开 API 边界均有验收测试。 | ⏳ Implementation complete; PR pending |
