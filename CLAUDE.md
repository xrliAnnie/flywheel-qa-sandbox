...
| FLY-1671: **既有独立 updater 的手动全量重启入口 + 本体口径修正** — 不新建重启器；`scripts/request-restart.sh` 解析远端 `origin/main` 的精确 SHA，复用 FLY-270 `self-ship-pending.d` marker + `com.flywheel.updater` nudge，仍由 updater 单写者执行 `git pull origin main --ff-only` 后 `restart-services.sh --reason updater`，因此发起 Lead 也属于被换代集合。入口不伪造 PR/issue deployment identity，成功只表示入队，不冒充重启完成；guard 与运行手册把 Lead 直跑降级为明确紧急兜底。Lead supervisor 以 `(carrier PID,start)` 绑定的 0600 breadcrumb best-effort 留下 `launched|adopted`，restart 波次在 10 秒总预算内只读聚合为「N 新建 / M 接管(未换) / K 未知」；观测永不改变 launchd supervisor verdict，旧的「新本体已起」假断言已删除。Design review R3 APPROVED；验证：新入口/证据 26/26、updater/self-detach/v2 45/45、adoption/storm 33/33、restart 128/128、guard 137/137、通知 43/43、全仓 lint（0 error/13 既有 warning）、22 workspace build、受控单 worker `pnpm test:packages:run` 全绿（teamlead 9,049 pass/5 skip）。 | ⏳ PR #804；Code review R1 APPROVED，最终 doc-only head 复审中；merge 后需 bootstrap/revive production updater，再由 updater 发起一次真机全量重启验收 15/15 新本体、零豁免 |
