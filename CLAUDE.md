...
| FLY-1602: **Lead restart 换代孤儿/lease catch-22 自愈** — lease 新增 supervisor/body 分层证据与 generation-fenced CAS adoption；继任 supervisor 只在“唯一同身份 body + 无异己 supervisor + exact tmux/session/process 闭集”时收养并重绑 lease，任何 sensor/authority 漂移 fail-closed。restart-services 改为 controlled-wave arm + N0-N5 终判与 900s convergence journal，newborn verification 假失败不再提前报死；durable 0600 intent marker 让 mid-restart crash 在下次 run 正向对账，只有完整 supervisor/PID/body/model/lease 闭集才退休。v2 scheduler 与 daemon 的旁路重启改为 graceful-only/subordinate lock，global restart 享有 fenced ownership；新增 informational `lead_body_adopted` 告警与 runbook。Code review R3 的唯一 HIGH 已修：旧 transaction marker 完整 reconcile 后只关闭旧 intent，不再豁免当前 deploy 的 full-fleet restart，防止部署误报 healthy 但 Lead 继续跑旧代码。TDD: lease/adoption/authority、restart-services 86、storm 22、controlled wave 30、reconcile 6、daemon refusal 2 全绿；全仓 lint/build 通过，package suite 各 package 全绿（并发总入口仅触发既有 5s `chat-receipt` flake，隔离 101 files/1358 tests 全绿）。真机有负载连续两次 restart + 单次 newborn fault injection 留 QA phase，不在 implement Runner 内动生产舰队。 | ⏳ Final code re-review pending |
