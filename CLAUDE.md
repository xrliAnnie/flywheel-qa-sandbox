...
| FLY-494 v1.55.0: **Kimi Code(`kimi`)作第四个 first-class Runner executor backend `kimi-tmux`**(agent-agnostic:Claude / Codex / Antigravity / **Kimi** 并列)。精确镜像 FLY-493 的 no-transport tmux-adapter 模式 —— FLY-493 已把整套 `pr_handoff` no-transport 生命周期写成以 `transport === "none"` 为键的**通用**机制(DecisionLayer HR-PR-HANDOFF / dispatcher / Blueprint finish / wake-guard / complete.ts / event-route / marker-reconciler / DirectEventSink),所以 Kimi **执行逻辑零改动**自动继承,只需:① 新 `KimiTmuxAdapter`(TmuxAdapter 瘦子类,覆盖 `type`=kimi-tmux / `binaryName`=kimi / `runPreflight` fail-closed auth 探针 / `buildCliArgs`)② 6 处后端注册(`ExecutorBackend` + `EXECUTOR_BACKENDS` + runner-label vendor `kimi`/`kimi-code` + `EXECUTOR_TO_TRANSPORT`/`VENDOR_TO_EXECUTOR`/env 别名 + index 导出 + run-infra factory)③ **一处通用 runtime 文本去 vendor 化**(DecisionLayer `pr_handoff` reasoning 原硬编码 "antigravity" → "No-transport runner build complete...",Codex R1 #1 运营真实性;相邻注释泛化为 "no-transport (e.g. antigravity / kimi)")。**CLI flags live-verified against kimi 0.18.0**(FLY-494 spike 期间 brew 装了 `kimi-code` 0.18.0 → `kimi --help` 实测):无人值守调用 `kimi --yolo --model X -p <bootstrap 指针>`(`--yolo`=auto-approve all actions parity claude `--dangerously-skip-permissions`;`-p`=非交互跑一个 prompt 并打印响应)。**更正**:草稿原写的 `--print`/`--afk` 在 0.18.0 **不存在**,Codex code R1 后 live spike 揪出并改为 `--yolo`+`-p`+`-m`;实测未登录 `kimi -p` 会挂起等 device-code login → preflight bounded 20s timeout(Codex code R1 #1)是 load-bearing fail-closed。Kimi 同样无 claude-code Agent Team 邮箱 → transport=none → 同 pr_handoff 终态(build→PR→报 Lead→founder 手动 ship)。**字节兼容默认**(不配置=`claude-tmux`,79-test TmuxAdapter 套 claude 路径未动;`ExecFileFn` 加可选 `{timeoutMs}` 向后兼容)。**关键交付边界**:本 PR ship 的是 **wiring + flag-verified 脚手架,不是 e2e live-usable** —— kimi CLI 已装、flags 已验,**仍需** ① Annie 经 `kimi login`(device-code flow)登录 ② 一次已登录 e2e spike(真派 kimi runner 验 `-p`+`--yolo` 扛长阻塞 brainstorm-gate)。Codex **design review 4 轮 + code review 2 轮 APPROVED**(design R1 MED DecisionLayer 去 vendor + 3 LOW;code R1 MED bounded preflight 防挂起 wedge)。CI Build & Test 绿。**follow-up**:Annie login + e2e spike;kimi mailbox push-wake + auto-ship;ACP(`kimi acp`)+ video-in-context;MCP parity。| ⏳ Pending ship |
