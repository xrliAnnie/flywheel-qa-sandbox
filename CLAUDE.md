...
| FLY-1462: **rework 永久 `persisted_target_missing` hold 修复** — terminal holder 丢失 persisted tmux target 时，不再仅凭 FSM status 猜死；要求 CommDB registration 明确 absent + proven-dead status(`terminated/failed/rejected/blocked/deferred/shelved`) + 全局 exec-marker inventory 明确 `missing` 三证齐全才 replace。marker `found/ambiguous/indeterminate`、CommDB read error、running+空 target 全部继续 fail-closed hold；`completed` 刻意排除以保护 parked-alive holder。PhaseOrchestrator 与 WorkflowReworkCoordinator 共用同一 classifier/probe 语义，无 feature flag。TDD 覆盖 6 个终态、3 个不确定分支、running 反例、CommDB error 与 coordinator 端到端；相关 104 项回归、build、lint 通过。全包残留仅 `origin/main` 同样复现的机器态 harness 与 Vitest worker RPC timeout。 | ⏳ Code review |
