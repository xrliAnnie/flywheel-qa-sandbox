...
| FLY-1638: **self-ship 自动化收尾 + launch 孤儿自锁根治** — schema-v2 `runner_ship` gate 统一绑定冻结 git head，seed 编译后逐份验证 gate authority；rework delivery 五次封顶转 `needs_lead` 并只进 Lead alert，同 head 已有 QA PASS 时不再铸幽灵 rework；QA verdict 节点默认凭证窗扩到 6h。generic run 补 `no_code` 合法终态、terminal/dead-exec 重派 fence 与 completed-without-receipt hold，消除取消单伪造 PR 和已完 run 复活。`restart-services` 在停 Bridge 前设置 30min durable admission pause，健康替换/rollback 显式恢复。generalized launch 改为 5min lease + heartbeat、10min 绝对 horizon、released-generation tombstone、精确 tmux window identity/fingerprint 与结构化 start 错误；旧 completed 同名窗可安全收敛，attempt=1 root design 不再自指 predecessor。TDD 验证覆盖所有改动文件（TeamLead 374、config 4、TmuxAdapter 128）、seed authority 17/17、restart pause 13/13、全仓 build；canonical aggregate 的 host-only model-policy、Terminal.app/launchctl 与 Vitest worker RPC 例外已隔离复跑，未伪报整门全绿。 | ⏳ PR review / founder-gated ship pending |
