...
**Correlation Key**: `callbackToken` (UUID)，由 TmuxRunner per-run 生成（Blueprint 不感知）。不使用 `sessionId` 作为 correlation key，因为 Claude hook 的 `session_id` 和 CLI `--session-id` 参数[不一定相同](../../packages/claude-runner/src/TmuxRunner.ts#L151)。详见 Part 2.1 "Transport Contract"。
...
## Reactions 系统 — 明确延迟到 v0.2.1+

v0.1.0 Phase 2 设计了完整的 Reactions 系统（[原文档 Task 11b](archive/v0.1.0-flywheel-orchestrator.md)）。**v0.2 不实现 Reactions** — 整体延迟到 v0.2.1+，原因是需要 GitHub webhook ingress 基础设施（Tailscale Funnel 或 Cloudflare Tunnel），且 v0.2 核心目标是并行执行 + Decision Layer。

**重要澄清**: v0.2 Decision Layer 的 `auto_approve` 路由 **不等同于** Reactions 的 `approved-and-green` handler。区别:

| 维度 | v0.2 Decision Layer auto_approve | v0.1.0 Reactions approved-and-green |
|------|--------------------------------|-----------------------------------|
| 触发源 | Issue 完成后，Blueprint 内部调用 | GitHub webhook (PR approved + CI pass) |
...
当 v0.2.1+ 实现 Reactions 时，参考 v0.1.0 的完整设计:
- `ReactionsEngine` — config match → dedup → dispatch
- `ReviewFixHandler` — PR review comment auto-fix (dedup by repo+PR+headSha+type)
- `ApprovedAndGreenHandler` — 渐进式 notify → auto-merge (CEO 通过 config 切换)
- `reaction_runs` 表 — 持久化防重入 + 审计
- Crash-safe escalation — 使用 DB `started_at` 而非进程时间
...
| **ConfigLoader + FlywheelConfig** | ✅ Preserved + Extended | `parallel:` / `decision_layer:` / `skills:` 段新增 (Part 4) |
| **Blueprint** | ✅ Preserved + Extended | 增加 worktree/skill/evidence/decision 步骤 (Part 2.4) |
| **PreHydrator** | ✅ Preserved | 不变，hydrate(node: DagNode) |
| **GitResultChecker** | ✅ Preserved + Extended | 不变，新增 `ExecutionEvidenceCollector` 在其上扩展 |
| **TmuxRunner** | ✅ Preserved + Extended | 增加 HTTP callback 路径 (Part 2.1) |
| **DagDispatcher** | 🔄 Rewritten | 串行 → 并行 (Part 2.5), `DispatchResult` 接口保留 |
| **IFlywheelRunner / FlywheelRunRequest / FlywheelRunResult** | ✅ Preserved | `packages/core/src/flywheel-runner-types.ts` 不变 |
| **Decision Layer** | 🆕 New in v0.2 | Hard Rules + Haiku Triage/Verify + Fallback (Part 2.2) |
| **Skill Injection** | 🆕 New in v0.2 | 5 个 Flywheel Skills (Part 2.3) |
| **WorktreeManager** | 🆕 New in v0.2 | superset-ai 移植 (Part 2.1) |
| **Reactions System** | ⏸️ Deferred to v0.2.1+ | 需要 webhook ingress；设计规格见上方 "Reactions 系统" 节 |
| **Auto-Loop Controller** | ⏸️ Deferred to v0.2.1+ | 连续执行需要 Decision Layer 稳定后再加；规格摘要见 Part 7 |
| **RunState 崩溃恢复** | ⏸️ Deferred to v0.2.1+ | 依赖 Auto-Loop；规格摘要见 Part 7 |
| **DecisionStore SQL schema** | ⏸️ Deferred to v0.2.1+ | v0.2 只需 AuditLogger (SQLite)；完整 event-sourced schema 在 CIPHER 时引入；规格摘要见 Part 7 |
| **Issue Blocking 自动分析** | ⏸️ Deferred to Phase 3+ | 异步 suggestion，需 Auto-Loop + memory；规格摘要见 Part 7 |
| **Multi-Team** | ⏸️ Deferred to Phase 5+ | team isolation, shared memory, standup；规格摘要见 Part 7 |
| **Deployment Strategy** | ⏸️ Deferred to v0.2.1+ | 本地 Mac → Mac Mini；规格摘要见 Part 7 |
| **Per-Project Memory (Task 14-15)** | ⏸️ Deferred to v0.3 | `.flywheel/memory.json` → SQLite + sqlite-vec；摘要见 Part 7 "v0.3: Per-Project Memory" |
| **Decision Intelligence / CIPHER (Task 17-21)** | ⏸️ Deferred to Phase 5 | Pattern promotion + Dual-Gate auto-decide；摘要见 Part 7 "Phase 5: CIPHER Pattern Promotion" |
...
| 类型 | 包 | 变化 | 详情 |
|------|---|------|------|
| `FlywheelConfig` | `packages/config` | **Extended** | `decision_layer`/`reactions` 已存在 (v0.1.1)；v0.2 新增 `parallel`, `skills` 段，细化 `decision_layer` 子配置 |
...
| `ExecutionContext` | NEW | **New** | Decision Layer 输入 (Part 3) |
| `DecisionResult` | NEW | **New** | Decision Layer 输出 (Part 3) |
...
**Step 1 (v0.1.2)**: 利用已有的全局 hook 基础设施 (`scripts/install-hooks.sh` + `scripts/hooks/flywheel-session-end.sh`)，增加环境变量分支:
```bash
...
**Step 2 (v0.1.3, 仅在 spike 证明全局 hook 不够时)**: 引入 per-run `--settings` 注入，为每个 worktree 写入独立的 `claude-settings.json`。
...
-- Reactions 去重 (v0.2.1+ Reactions 实现时)
