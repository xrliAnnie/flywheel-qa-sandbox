...
domain: backend
research:
  - "doc/engineer/research/new/001-flywheel-orchestrator.md"
  - "doc/engineer/research/new/002-research-gaps-supplement.md"
  - "doc/engineer/research/new/003-decision-layer-cross-reference.md"
exploration: "doc/engineer/exploration/new/001-flywheel-autonomous-dev-workflow.md"
...
**Runner 策略:** 我们 **不自己写 agent** — 直接 spawn 现有 CLI 工具 (Claude Code CLI, Codex CLI, Gemini CLI)。`IAgentRunner` 的实现就是启动一个 CLI 进程、传入 prompt 和项目目录、等它跑完。这些 CLI 工具已经足够好，不 reinvent the wheel。Session resume 由 CLI 工具自身的 `--resume` 机制提供（如 Claude Code 的 session continue）。Phase 1 只实现 `ClaudeCodeRunner` (spawn `claude` CLI)；Phase 2+ 可加 Codex CLI、Gemini CLI runner。

**Blueprint 模式 (借鉴 Stripe Minions):** 每个 issue 的执行不是纯 agent loop，而是 **确定性节点 + agent 节点的混合编排**。git clone/checkout/push/CI wait 走确定性代码（零 token 成本、100% 可靠），只有 implement 和 fix 走 LLM。Agent 启动前 **pre-hydrate** context（Linear issue 详情、相关代码、项目 rules），减少 agent 浪费 token 去"发现"已知信息。CI 迭代轮次可配置（默认 2 轮，和 Stripe 一致），避免 diminishing returns。**注意: 这不是要限制 AI 的能力 — 只是把 AI 不需要思考的步骤 (git 操作、CI 等待) 用普通代码执行，省钱且更可靠。AI agent 在自己的节点内拥有完全自主权。**

**Model Agnosticity (架构约束):** Blueprint 的 agent 节点 **不直接 import 任何 LLM SDK**。所有 agent 调用走 `IAgentRunner` 接口，由 `RunnerSelectionService` 在运行时选择具体 runner。**核心设计: 我们不写自己的 agent — 直接 spawn 现有的 CLI 工具 (Claude Code CLI, Codex CLI, Gemini CLI)。** `IAgentRunner` 的实现就是启动一个 CLI 进程、传入 prompt 和项目目录、等它跑完。这些 CLI 工具已经很好了，我们不 reinvent the wheel。**注意：`IAgentRunner` 接口以 Task 1 fork 后的实际 Cyrus 签名为准** — 文档中的接口定义是目标契约，需在 fork 阶段验证并适配（可能需要 adapter 层）。Phase 1 只实现 `ClaudeCodeRunner` (spawn `claude` CLI)，但接口保持开放 — 未来可插入 Codex CLI、Gemini CLI 等。Agent 节点配置声明式 (model/tools/prompt 在 `.flywheel/config.yaml`，不 hardcode)。**原则: "The product is the model" (Boris Cherny) — 模型快速迭代，编排层必须 model-agnostic。**

**Reactions 系统 (借鉴 Agent Orchestrator):** Blueprint 目前只处理 issue → PR 的正向流程。但 PR 创建后的生命周期同样需要自动化。Reactions 是事件驱动的自动响应引擎：监听 GitHub 事件，根据配置自动触发动作（send-to-agent / notify / escalate）。Phase 2 实现两个核心 Reaction — **PR review → agent auto-fix**（reviewer 提 changes-requested → agent 读当前 head SHA 上的 review comments → 修复 → push → CI）和 **approved+green → notify 或 auto-merge** (CEO 通过 config 控制渐进切换)。每个 reaction 有 `escalateAfter` 超时和 `retries` 上限，超过则 escalate 给人。**Agent-stuck 检测延迟到 Phase 3**（依赖 JSONL introspection 或 IAgentRunner activity events，当前 `run(): Promise<RunResult>` 无法提供中间活动信号）。Reactions 状态持久化到 `reaction_runs` 表（dedup key: repo+prNumber+headSha+reactionType），防止 webhook 重投或进程重启导致重复 agent run。Reactions 代码放在 `packages/edge-worker/` 内（非独立包），直接复用 Blueprint 的 git push / CI wait 设施。

**Research doc:** doc/engineer/research/new/001, 002, 003
**Industry reference:** doc/engineer/deep-research/003-stripe-minions-part1.md, 004-stripe-minions-part2.md, 006-boris-cherny-claude-code-future.md, 007-parallel-ai-agents-pkarnal.md, 008-agent-orchestrator-ao.md, 009-ramp-inspect-background-agent.md, 010-gastown-steve-yegge.md
**Status:** draft
...
把 Cyrus (一个开源的 "Linear issue → Claude Code → PR" 工具) fork 过来，在它的基础上加 **依赖排序能力**。结果：你在 Linear 里标好 issue 之间的 blocking 关系，Flywheel 会自动按正确顺序执行它们。Blocking 关系可以手动标注，也可以在让 AI 创建 issues 时指定 — Phase 3 会加入自动分析代码依赖推断 blocking 关系的能力。
...
graph LR
    ...
    subgraph "Blueprint (per issue)"
        ...
        IMPL["☁️ Implement<br/>(spawn CLI agent)"]
...
新增 **Reactions 引擎** (借鉴 Agent Orchestrator 的 Reactions 系统): PR 创建后的生命周期也自动化 — reviewer 提 changes-requested 时，agent 自动读当前 head SHA 上的 review comments 并修复；PR approved + CI green 时 **渐进式处理**: 初期 notify CEO (CEO 快速 preview 后手动 merge)，条件成熟后切换为 auto-merge + FYI 通知 (低风险 PR 直接 merge，涉及 auth/billing/secrets 的 PR 仍需 CEO 确认)。CEO 通过 config 控制这个渐进过程。每个 reaction 有超时和重试上限，超过则 escalate 给人。（Agent-stuck 检测延迟到 Phase 3，依赖 JSONL introspection。）
...
graph TB
    ...
    subgraph "Reactions Engine (edge-worker 内)"
        ...
        DISPATCH -->|"approved + CI green"| AGH["ApprovedAndGreen<br/>Handler"]
        AGH -->|"action=notify"| NOTIFY["通知 CEO"]
        AGH -->|"action=auto-merge<br/>+ no sensitive files"| MERGE["Squash Merge<br/>+ FYI 通知"]
        AGH -->|"sensitive files<br/>或 merge 失败"| ESC
...
| 11b | Reactions Engine | 事件驱动自动响应: config match → dedup → dispatch。changes-requested → agent auto-fix (dedup by repo+PR+headSha+type); approved+green → notify 或 auto-merge (CEO 通过 config 渐进切换)。`reaction_runs` 表持久化状态，防 webhook 重投。escalation 走 DecisionRouter 统一路径 (issueId + audit trail)。代码在 edge-worker/ 内 | PR review comment 自动修复 + approved notify/auto-merge |
...
- **Verification**: CEO 可以检查:
  ...
  - **approved + CI green → 通知 CEO (初期); auto-merge + FYI (CEO 在 config 启用后)**
...
新增 **Issue Blocking 自动化**: Phase 1-2 的 blocking 关系由 CEO 手动标注（或在让 AI 创建 issues 时口头指定）。Phase 3 增加智能分析 — 当一批新 issues 被创建后，Flywheel 用 LLM 分析它们之间的代码依赖关系，自动在 Linear 上建议 blocking 关系 (CEO 确认后生效)。
...
| 15b | Issue Blocking 自动分析 | **异步 suggestion-only**: 当一批新 issues 被创建后，LLM 分析它们之间的代码依赖关系，输出候选 blocking 关系 `{blockedIssueId, blockerIssueId, confidence, evidence[]}`。默认**不直写 Linear** — 通过 Slack 发给 CEO 确认后才应用。不在 auto-loop 热路径上运行（异步后台任务）。手动标注和创建时指定 blocker 仍作为独立输入源，不在 15b 范围内 | Slack 消息: "建议 GEO-5 blocks GEO-8 (confidence: 0.82, evidence: both touch auth module)" → CEO 确认 → 写入 Linear |
...
**渐进式部署**: 本地开发 → Garage Mac Mini (24/7 运行)

| 阶段 | 环境 | 说明 |
|------|------|------|
| Phase 1-2 开发 | 本地 Mac (CEO 的开发机) | 开发和调试，手动启动 |
| Phase 2+ 运行 | Garage Mac Mini | 24/7 无人值守，SSH + Tailscale 远程管理 |

**为什么不用云**:
- Claude Code CLI 在 macOS 上最自然（和 CEO 日常开发环境一致）
- 零云费用 — 只付 Anthropic API 费 ($5/issue)
- Mac Mini M2 ($599) + UPS ($50) = 一次性投入，之后零运维成本
- 断电/crash recovery 由 `run-state.json` checkpoint + `reaction_runs` 持久化保障

**入站事件 (Ingress)**: Garage Mac Mini 在 NAT 后面，GitHub webhook 和 Slack interactive callback 无法直接到达。方案:
- **Tailscale Funnel**: 把本地端口暴露为公网 HTTPS URL (e.g. `https://flywheel.tail12345.ts.net/webhook`)，零配置、免费、自动 TLS。GitHub/Slack webhook URL 指向这个地址。
- **备选**: Cloudflare Tunnel (如果需要自定义域名) 或改为 polling transport (定期轮询 GitHub API，但延迟更高)

**Secrets 管理**:
- macOS Keychain 存储 `ANTHROPIC_API_KEY`, `LINEAR_API_KEY`, `GITHUB_TOKEN`, `SLACK_BOT_TOKEN`
- `launchd` plist 通过 `EnvironmentVariables` 注入，或用 `security find-generic-password` 在启动脚本中读取

**运维需求** (Phase 2+):
- `launchd` plist 保持 Flywheel 进程常驻 (crash 后自动重启)
- Tailscale 远程 SSH 管理
- Slack 通知 Flywheel 自身健康状态 (启动/重启/异常停止)
- Log rotation: stdout/stderr → `~/Library/Logs/flywheel/` + `newsyslog` 定期清理
...
| CLI 工具接口不匹配 | Medium | Medium | Task 1 验证 `claude` CLI 的 `--print` / `-p` 非交互模式、`--max-turns`、`--allowedTools` 等参数；确认 session resume (`--resume`) 机制；验证通过后下调为 Low |
...
│   ├── claude-runner/         [MODIFIED] ClaudeCodeRunner (spawn CLI) 实现 IAgentRunner
...
- 验证 Cyrus 的 `ClaudeRunner` 实现方式 — 是通过 Agent SDK `query()` 还是 CLI spawn？如果是 SDK，评估是否改为 CLI spawn（CEO 方向: 直接用 CLI 工具）
- 确认 `claude` CLI 的非交互模式参数 (`-p`/`--print`, `--max-turns`, `--allowedTools`, `--max-cost`)
- 确认 session resume 机制（`claude --resume` vs session ID）
...
3. **保留 `IAgentRunner` 接口和 `RunnerSelectionService`** — 只删具体 runner 实现，不简化接口。Phase 1 仅注册 `ClaudeCodeRunner` (spawn `claude` CLI)，但接口保持 model-agnostic（未来可插入 OpenAI/Gemini/local runners）
...
export interface FlywheelConfig {
  ...
  agent_nodes?: {                 // declarative agent node config (model-agnostic)
    implement?: {
      tools?: string[];           // CLI --allowedTools, default: ["Read", "Edit", "Write", "Bash", "Grep", "Glob"]
      max_turns?: number;         // CLI --max-turns, default: 50
    };
    fix?: {
  ...
  reactions?: {                 // 事件驱动自动响应 (借鉴 Agent Orchestrator)
    ...
    'approved-and-green'?: {
      action: 'notify' | 'auto-merge';  // CEO 渐进式选择:
      // Phase 2 初期: 'notify' — CEO review 每个 PR (但通常快速通过)
      // Phase 2 中后期: 'auto-merge' — 低风险 PR 直接 merge + FYI 通知
      // auto-merge 条件: CI pass + no sensitive files (由 Hard Rules 判断)
      // CEO 随时可通过 config 切换
    };
...
1. **Spawn 现有 CLI 工具** (Claude Code CLI, Codex CLI, Gemini CLI)，不自己写 agent，不用 Agent Teams (实验性, 无编程控制, ~7x token 成本)
2. **Blueprint 混合编排** (借鉴 Stripe Minions): 确定性节点 + agent 节点交替执行，只在 implement/fix 时消耗 token
3. **Model-agnostic**: Blueprint 通过 `IAgentRunner` 接口调用 agent，不直接 import 任何 LLM SDK。`RunnerSelectionService` 根据 config 选择 runner。Phase 1 仅 `ClaudeCodeRunner` (spawn `claude` CLI)，但接口可扩展
...
Pre-Hydrate (确定性)     — 拉 Linear issue 详情 + 相关代码 + 项目 rules
    ↓
Implement (agent)        — spawn CLI (claude -p "..."), hydrated context 注入 prompt
    ↓
...
export class Blueprint {
  ...
  async run(node: DagNode, projectRoot: string, bpCtx: BlueprintContext): Promise<BlueprintResult> {
    ...
    // Tools from config (model-agnostic, not hardcoded)
    const implTools = this.config.agent_nodes?.implement?.tools
      ?? ["Read", "Edit", "Write", "Bash", "Grep", "Glob"];
    const implMaxTurns = this.config.agent_nodes?.implement?.max_turns ?? 50;

    // === Step 1: Pre-Hydrate (确定性, 零 token) ===
    const ctx = await this.hydrator.hydrate(node);

    // === Step 2: Implement (spawn CLI agent via IAgentRunner — model-agnostic) ===
    ...
    for (let round = 1; round <= this.maxCIRounds; round++) {
      ...
      if (round < this.maxCIRounds) {
        ...
        const fixResult = await runner.run({
          ...
          cwd: projectRoot,
          allowedTools: this.config.agent_nodes?.fix?.tools ?? implTools,
...
export interface AgentRunRequest {
  prompt: string;
  cwd: string;
  allowedTools?: string[];   // CLI --allowedTools flag
  maxTurns?: number;         // CLI --max-turns flag
  maxBudgetUsd?: number;     // CLI cost cap (if supported)
  timeoutMs?: number;        // process timeout (default 30m)
  sessionId?: string;        // for session resume (CLI --resume)
}
...
**Phase 1 唯一实现 — ClaudeCodeRunner (spawn `claude` CLI):**

```typescript
// packages/claude-runner/src/ClaudeCodeRunner.ts
...
export class ClaudeCodeRunner implements IAgentRunner {
  ...
  async run(request: AgentRunRequest): Promise<RunResult> {
    // 直接 spawn claude CLI — 不自己写 agent，不调 LLM SDK
    const args = [
      "--print",                          // 非交互模式
      "--max-turns", String(request.maxTurns ?? 50),
      "--output-format", "json",          // 结构化输出 (cost, session ID 等)
    ];
    if (request.allowedTools?.length) {
      args.push("--allowedTools", request.allowedTools.join(","));
    }
    if (request.sessionId) {
      args.push("--resume", request.sessionId);  // session resume
    }
    args.push("--", request.prompt);

    const { stdout } = await execFileAsync("claude", args, {
      cwd: request.cwd,
      timeout: request.timeoutMs ?? 30 * 60 * 1000,  // default 30m
...
**注意**:
- 上面的代码是伪代码 — `claude` CLI 的实际参数、JSON 输出格式需在 Task 1 fork 阶段验证。
- Session resume 的 `--resume` 参数需确认具体语法。
- Blueprint 的确定性节点 (lint, push, CI) 需要适配目标项目的工具链。
- **Blueprint 通过 `IAgentRunner` 接口调用 agent**，CLI 特定逻辑封装在 `ClaudeCodeRunner` 中。未来新增 runner (Codex CLI, Gemini CLI) 只需实现 `IAgentRunner` 并注册到 `RunnerSelectionService`。
...
3. Dispatch:
    ...
    └── approved-and-green → ApprovedAndGreenHandler
          ├── action=notify → SlackActivitySink.notifyStatus() → completed
          ├── action=auto-merge + HardRules pass → squash merge + FYI 通知 → completed
          └── sensitive files 或 merge 失败 → DecisionRouter.escalate() → escalated
```
...
**ApprovedAndGreenHandler (渐进式 notify → auto-merge):**

```typescript
// packages/edge-worker/src/reactions/ApprovedAndGreenHandler.ts
export class ApprovedAndGreenHandler {
  constructor(
    private githubClient: any,
    private decisionStore: DecisionStore,
    private decisionRouter: DecisionRouter,
    private slackSink: SlackActivitySink,
    private hardRules: HardRules,
    private config: { action: 'notify' | 'auto-merge' },
  ) {}

  async handle(ctx: { repo: string; prNumber: number; headSha: string; issueId: string; branch: string }): Promise<void> {
    // 1. Preflight checks (both modes)
    const pr = await this.githubClient.pulls.get({ ...ctx });
    if (pr.data.head.sha !== ctx.headSha) {
      // Stale event — new commits pushed since approval. Mark as completed (superseded).
      await this.decisionStore.updateReactionRunStatus(
        ctx.repo, ctx.prNumber, ctx.headSha, "approved-and-green", "completed",
      );
      return;
    }
    if (pr.data.merged || pr.data.state !== "open") {
      // Already merged or closed. Mark as completed.
      await this.decisionStore.updateReactionRunStatus(
        ctx.repo, ctx.prNumber, ctx.headSha, "approved-and-green", "completed",
      );
      return;
    }
    // Verify approval is still valid and required checks pass (GitHub API mergeable state)
    if (!pr.data.mergeable || pr.data.mergeable_state !== "clean") {
      // Merge not ready (pending checks, review dismissed, etc.) — escalate
      await this.decisionRouter.route({
        issueId: ctx.issueId, category: "merge_not_ready",
        context: `PR #${ctx.prNumber} not mergeable: ${pr.data.mergeable_state}`,
        repo: ctx.repo, branch: ctx.branch, commitSha: ctx.headSha,
        evidenceRefs: [`https://github.com/${ctx.repo}/pull/${ctx.prNumber}`],
      });
      await this.decisionStore.updateReactionRunStatus(
        ctx.repo, ctx.prNumber, ctx.headSha, "approved-and-green", "escalated",
      );
      return;
    }

    // 2. Notify mode (Phase 2 初期 default)
    if (this.config.action === "notify") {
      await this.slackSink.notifyStatus({
        type: "pr_ready",
        repo: ctx.repo, prNumber: ctx.prNumber,
        message: `PR #${ctx.prNumber} approved + CI green. Ready to merge.`,
      });
      await this.decisionStore.updateReactionRunStatus(
        ctx.repo, ctx.prNumber, ctx.headSha, "approved-and-green", "completed",
      );
    ...
    // 3. Auto-merge mode (CEO 在 config 启用后)
    // 3a. Hard Rules check — sensitive file changes → escalate, don't auto-merge
    const files = await this.githubClient.pulls.listFiles({ ...ctx });
    const sensitive = this.hardRules.checkFiles(files.data.map(f => f.filename));
    if (sensitive.length > 0) {
      await this.decisionRouter.route({
        issueId: ctx.issueId,
        category: "auto_merge_blocked",
        context: `PR #${ctx.prNumber} touches sensitive files: ${sensitive.join(", ")}`,
        repo: ctx.repo, branch: ctx.branch, commitSha: ctx.headSha,
        evidenceRefs: [`https://github.com/${ctx.repo}/pull/${ctx.prNumber}`],
      });
      await this.decisionStore.updateReactionRunStatus(
    ...
    // 3b. Merge
    try {
      await this.githubClient.pulls.merge({
        owner: ctx.repo.split("/")[0], repo: ctx.repo.split("/")[1],
        pull_number: ctx.prNumber, merge_method: "squash",
      });
      await this.decisionStore.updateReactionRunStatus(
        ctx.repo, ctx.prNumber, ctx.headSha, "approved-and-green", "completed",
      );
      // FYI notification (not blocking)
      await this.slackSink.notifyStatus({
        type: "pr_merged",
        repo: ctx.repo, prNumber: ctx.prNumber,
        message: `PR #${ctx.prNumber} auto-merged (approved + CI green, no sensitive files).`,
      });
    } catch (err) {
      // Merge failed (merge conflict, branch protection, etc.) → escalate
      await this.decisionRouter.route({
        issueId: ctx.issueId,
        category: "auto_merge_failed",
        context: `PR #${ctx.prNumber} auto-merge failed: ${err.message}`,
        repo: ctx.repo, branch: ctx.branch, commitSha: ctx.headSha,
        evidenceRefs: [`https://github.com/${ctx.repo}/pull/${ctx.prNumber}`],
      });
      await this.decisionStore.updateReactionRunStatus(
...
- `ReactionsEngine.test.ts: "approved-and-green notify mode sends Slack notification"` — config action=notify → verify SlackActivitySink.notifyStatus() called
- `ReactionsEngine.test.ts: "approved-and-green auto-merge mode merges PR"` — config action=auto-merge, CI pass, no sensitive files → verify gh pr merge called + FYI notification sent
- `ReactionsEngine.test.ts: "approved-and-green auto-merge blocked by Hard Rules"` — diff touches auth file → escalate to CEO instead of auto-merge
- `ReactionsEngine.test.ts: "approved-and-green auto-merge failed escalates with correct terminal state"` — mock GitHub merge API throws (conflict / branch protection) → verify DecisionRouter.route() called + reaction_run status = `escalated` (not `failed`)
...
**Commit:** `feat: ReactionsEngine — PR review auto-fix + approved notify/auto-merge (with dedup persistence)`
