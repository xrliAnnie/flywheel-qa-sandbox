# Flywheel v2 设计稿 v4 (2026-07-27)
> 相对 v3:吸收 Codex R3 最小修改集 5 项([R3-n] 标记)。R3 已确认:单库 mailbox 决定类别性关闭跨库窗口。
...
索引 [R3-3]:`partial INDEX mailbox_pending ON (to_agent, seq) WHERE state='pending' AND (next_retry_at IS NULL OR next_retry_at<=now)`;`INDEX mailbox_retry ON (next_retry_at) WHERE state='pending' AND retry_count>0`。
...
- P12:**bypass 封闭矩阵** [R3-5](escape/recovery transition 另列不混):
  ...
  | 提醒静默 | `mute_reminder` | Lead | lead 凭据 | ≤72h | events(kind=bypass_used,result_code) |
  | 超时宽限 | `extend_timeout` | Lead | lead 凭据 | ≤24h | 同上 |
  | 路由改道 | `route_override` | Lead | lead 凭据 | 单次 | 同上 |
  | break-glass | `emergency_transition` | founder | **独立 founder 凭据** | 单次 | 同上+obligation 记录 |
  每行两条自动化测试:正向可达(授权 actor 执行成功)+反向拒绝(未授权 403+零副作用)。
