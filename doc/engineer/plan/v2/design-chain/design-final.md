# Flywheel v2 设计稿(终版·Codex R5 APPROVED)
...
### 2.4 resume 同 v2
### 2.4a 注入垫片(唯一允许的适配层,vendor-neutral)[Annie 2026-07-27]
背景:JSON 信箱唯一难替代的价值=claude-code harness 原生轮询+自动注入对话;但**我们不会只有 Claude Code**(现有 Codex,未来可能 PyAgent 等),做成 Claude-specific 是错的。
决定:注入垫片是**必须的且必须 generic**——定义一个 vendor-neutral 契约 `InjectionShim`,每个 backend 各实现一份适配:
- 统一接口:`poll(kernel_api) → messages`、`inject(runner_session, message)`、`ack(kernel_api, message_uid)`;垫片**无状态**(只读 kernel API 的 pending,注入,ack,不持久化)。
- 每 backend 只实现「怎么把一条消息注入这个 vendor 的会话」这一个方法:Claude=写它原生监听的信箱文件(白拿 harness 注入);Codex=走它的 turn/inbox 通路;新 backend(PyAgent…)=实现同一接口即接入。
- 这是全系统**唯一允许存在的适配层**,且被 vendor-neutral 契约收口——不允许每个 backend 再各长一套状态。
- 验收:①垫片零持久化,崩溃后重启即好、无恢复步骤;②新增一个 backend 只需实现 InjectionShim 接口,不改 kernel/schema/任何其他部分(可插拔性回归测试)。
