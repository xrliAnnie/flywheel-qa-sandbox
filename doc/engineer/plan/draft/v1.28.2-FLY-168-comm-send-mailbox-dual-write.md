...
| mailbox 写失败静默吃掉指令 | CommDB 永远先写（审计可查）；写失败 `console.error` 到 stderr（loud）；sentinel 仍让 hook 在有 tool call 时兜底注入 CommDB |
| `CLAUDE_CONFIG_DIR` 不一致 → 写错 inbox 树 | Phase 3 临时目录 path-invariant 测试；实现注明 mailbox-mode send 必须继承 Lead/Runner 同一 `CLAUDE_CONFIG_DIR`，否则降级为 CommDB-only + warn（Codex r1 #4） |
