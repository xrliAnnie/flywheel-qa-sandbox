...
- **Event 丢失 + dead window 留存**: 当前 `cleanup_stale_conservative()` 仍使用 `get_tmux_agent_windows()` 判断 window 存在性。在 `remain-on-exit on` 下，如果 `pane-exited` event 丢失（如 watcher crash 导致 event file 未处理），dead window 仍在 `list-windows` 中，conservative cleanup 会认为 window "活着" 而跳过清理。这是已知的边缘场景；如果后续需要更高恢复性，可让 conservative fallback 也基于 `#{pane_dead}` 检查。

## 后续工作（独立 issue）

1. **cos-lead 稳定性**: 调查 cos-lead 频繁 crash 的根因（与 cmux-sync 架构无关）
2. **Conservative cleanup 增强**: 可选 — 让 `cleanup_stale_conservative()` 也使用 `is_pane_alive()` 替代 window 存在性检查，覆盖 event 丢失场景
3. **Event file → unix socket**: 如果 event file 方案有性能/可靠性问题，可升级为 unix socket IPC
