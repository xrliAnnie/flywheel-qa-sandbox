...
**Deployment requirement**: The recommended and reliable path is `gbrain init --supabase`, which creates `~/.gbrain/config.json`. The MCP enablement gate also accepts `GBRAIN_DATABASE_URL` or `DATABASE_URL` env vars as alternatives, but env-only config has a known propagation risk: `claude-lead.sh` launches Claude in tmux with an explicit env allowlist that does not include these vars. In practice, **always use `gbrain init --supabase`** to ensure config is available via file, not env.
...
- **No env block**: Avoids writing secrets to `.mcp.json` on disk. Reliable config requires `~/.gbrain/config.json` via `gbrain init --supabase`; env-only config passes the gate but has known tmux env propagation risk
...
# Case 4: gbrain installed + no config file + GBRAIN_DATABASE_URL set (env-only config)
#   Assert: gbrain entry present (env var passes gate at JSON generation time)
#   Assert: no env block in gbrain MCP JSON
#   CAVEAT: This only verifies MCP JSON generation. Env-only config may NOT
#   reliably reach the MCP subprocess at runtime because claude-lead.sh
#   uses an explicit tmux env allowlist. Always prefer `gbrain init --supabase`.
