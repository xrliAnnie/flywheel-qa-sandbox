...
> **关键交付边界（Tadashi brainstorm-gate 拍 + 写进本 plan）**：FLY-494 ship 的是 **wiring（脚手架，kimi 缺席/未登录时 fail-closed 抛错）**，**不是 live-usable**。进度更新（FLY-494 spike）：① `kimi` CLI **已装**（brew `kimi-code` 0.18.0）；②③ flags 已 **live-verified against 0.18.0**（`kimi --help`，更正了 `--print`/`--afk` 不存在 → 用 `--yolo`+`-p`+`-m`）。**仍未完成**：Annie 经 `kimi login`（device-code flow）登录 + 一次**已登录的 e2e spike**（真派一个 kimi runner 跑完 onboard→brainstorm-gate→PR，验 `-p`+`--yolo` 扛长阻塞 brainstorm-gate）。**在 Annie 登录 + e2e spike 完成前，绝不对外宣称 Kimi runner 端到端可用。**
...
| 模型 | `-m NAME` / `--model`（与 `-p` + `--yolo` 实测可组合无冲突） |
| auto-approve | `-y` / `--yolo` = 「Automatically approve **all** actions」（无人值守用）；`--auto` = 「auto permission mode」（更软） |
...
**选定的无人值守调用（live-verified）**：`kimi --yolo --model <X> -p "<bootstrap 指针>"`
- `--yolo` = 自动批准**所有** action（无人值守 tmux runner 无人应答权限提示，parity claude `--dangerously-skip-permissions` / agy 同款）。
- `-p` = 非交互跑一个 prompt 并打印响应（headless agent loop）；指向 0600 文件（复用 FLY-154 argv-overflow 缓解）。
- `--model` 仅当 `ctx.model` 有值才加。flag 全集中在 `KimiTmuxAdapter.buildCliArgs`，未来 kimi 版本若变单点一行可改。
- **live spike 实测**：未登录的 `kimi -p` **会挂起**（等 device-code login round-trip，非快速报错）→ 印证 Codex code R1，preflight bounded timeout（20s）是 load-bearing；`-p`+`--yolo`+`-m` 组合实测合法（旧 doc 称 `--prompt`/`--yolo` 互斥在 0.18.0 不成立）。
...
- `runPreflight()`：`tmux -V` + `kimi --version` + **fail-closed auth 探针**。探针用 `kimi -p "Reply with exactly: KIMI_OK" --yolo`（非 mutating），**bounded 20s timeout**（Codex code R1 + live spike：未登录 `kimi -p` 会挂起等 device-code，超时即 kill → fail closed）。输出不含 `KIMI_OK` 或命中 `/Authentication required|Please sign in|not logged in/i` → 响亮抛错。
- `buildCliArgs(ctx, _sessionId)`：`["--yolo"]` + （`ctx.model` ? `["--model", ctx.model]`）+ `["-p", "Read the instructions in <promptPath> and follow them exactly. Begin now."]`。combined system+task prompt 写 0600 文件 `kimi-bootstrap.md`（复用 agy 的 tmpdir/perm 逻辑）。**不**带任何 claude-only flag（`--session-id`/`--permission-mode`/`--append-system-prompt-file`/`--allowed-tools`/`--name`），也不用不存在的 `--print`/`--afk`。
...
- Parity with agy：`--yolo`（auto-approve all actions）= 无人值守 tmux runner 的 auto-approve（同 claude 的 `--dangerously-skip-permissions`）。worktree 隔离、comm.db、fail-closed auth preflight 全继承。Kimi 自带 `--plan`（read-only）/`--auto`（更软 permission mode）选项留作未来 config-gated 加固，本 PR 不引入。
...
6. emit kimi flags（`--yolo` + `--model <X>` + `-p`，**不含** `--print`）+ **不含**任何 claude-only flag。
...
## 9. Risks / non-goals
- **R1（已降级）**：CLI flags 现已 **live-verified against kimi 0.18.0**（spike 更正 `--print`/`--afk` 不存在 → `--yolo`+`-p`+`-m`）。残余：版本漂移风险 —— flag 集中在 `buildCliArgs` 单点可改；fail-closed preflight 保证 kimi 缺/未登录时不会静默跑错。
- **R2（Codex R1 #3，强化验收 —— 仍开放）**：`-p`（非交互）模式下单个阻塞工具调用（brainstorm gate 最长 48h）会不会被 Kimi 内部 per-command / idle session timeout 杀，或 step budget 不够跑完整 build+PR？**这需要已登录的 e2e spike 才能验**（flag 层已验，行为层未验）：① `kimi -p --yolo` 能跑一个阻塞「代表性时长」后才返回的 shell 命令并正常恢复；② 一个 brainstorm-gate-like 的阻塞命令不被 kill；③ 查并按需设 step/timeout config（默认太低则调高）；④ 记录 auth 失败 / quota / 瞬时失败 / 工具执行被中断 各自的 exit code。agy/claude 靠 interactive + `BASH_MAX_TIMEOUT_MS`，Kimi 可能有自己的 env 旋钮。不阻塞本 PR 的 wiring。
- **R3**：Kimi 交互模式无法用 launch arg 种 prompt（只有 `-p`/`--prompt` 非交互能），所以走 `kimi -p --yolo` 非交互而非 agy 的 interactive `-i`。这是 Kimi 与 agy 的唯一形态差异，已在 §2 选定调用里处理。
