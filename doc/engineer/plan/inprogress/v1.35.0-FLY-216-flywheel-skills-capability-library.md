...
│   │       └── SKILL.md            # 三分法拆 #222:description=触发场景(b);body=命令格式/7 天过期/降级/失败 fallback/频道优先级(c)≈85 行;禁令(d)不进 skill
│   └── flywheel/
│       └── flywheel-land/
...
└── .github/workflows/skill-guard.yml
    ...
    #    body 必含 5 要素:`flywheel-comm publish-report`、7 天过期、screenshot degrade、failure fallback、channel precedence(grep 级检查,防瘦身后 Lead 只记禁令不调 body)
```
