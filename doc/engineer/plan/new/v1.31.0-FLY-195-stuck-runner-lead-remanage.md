...
| ⟨SHARED⟩ 两边 import | 各自独立 |
|---|---|
| **Lead 规则流程**:异常上报 → 看屏+判断 → 重新纳管(轻)/ founder gate(重)→ **写 disposition** → ping Annie | 检测 helper:191=stale awaiting_review;195=stuck running(输出停滞) |
| **disposition 回执协议**(Lead→Bridge,见 §3.4):让 Lead 判断权威化,Bridge 兜底据此抑制 | 重新纳管手段:191 健康 idle 主路 mailbox;195 阶梯 mailbox→受限 tmux nudge |
| **上报管线** `appendLeadEvent`+guardrail 重投+`resolveLeadForIssue`(已存在) | 入口事件:191 advisory / 195 `runner_stuck_escalation` |
| **founder gate 边界**(轻自主,重/ship 走 consent) | Annie 兜底告警类型(195=`runner_stuck_unhandled`) |

> ⟨SHARED⟩ 确切 API 等 191 Codex R2 收敛(注:191 R2 也独立抓到同一个 running→awaiting_review race,指向同一修法 = 决策时重读 StateStore + stream-error 谓词,与本 plan 一致)。各自独立 PR、共用模块两边 import,细节 Codex review 定。
...
- **载体**:StateStore 一张轻表(键 execution_id+指纹),Lead 经一个 Bridge 端点 / `flywheel-comm` 子命令写(确切 API 与 191 收敛 —— 191 的 `reject_stale` 是同族 disposition)。
