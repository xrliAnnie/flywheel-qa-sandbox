...
**Issue**: FLY-31
**Date**: 2026-03-31
**Source**: `doc/engineer/exploration/new/FLY-31-flywheel-next-gen-architecture.md` (worktree), `doc/engineer/research/new/FLY-3-agentsmesh-deep-dive.md`, `doc/engineer/research/new/FLY-31-claude-code-source-analysis.md`
...
- `CommDBLeadRuntime.deliver()` 捕获 transport failure 后返回 `{ delivered: false, error }` 而非静默 warn
...
- **影响范围**：`LeadRuntime` interface, `CommDBLeadRuntime`, `HeartbeatService`
...
**文件影响**: `packages/teamlead/src/bridge/lead-runtime.ts` (deliver() 返回值), `packages/teamlead/src/bridge/commdb-lead-runtime.ts` (transport error 暴露), `packages/teamlead/src/HeartbeatService.ts` (checked delivery + retry), `packages/teamlead/src/bridge/dashboard-data.ts` (stuck sessions + delivery failures), Lead identity.md (escalation 行为规则)
