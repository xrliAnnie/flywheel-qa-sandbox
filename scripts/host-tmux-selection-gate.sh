...
run_census() {
  ...
  local total=0 generic=0 codex_generic=0 mufasa=0 infra=0
  ...
  trap - EXIT
  /bin/rm -rf "$temp_root"
  echo "host-tmux-selection-gate: census pass plists=$total generic=$generic codex-generic=$codex_generic codex-mufasa=$mufasa codex-infra-bot=$infra"
}
