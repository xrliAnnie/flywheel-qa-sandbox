...
# Prerequisites:
#   - jq installed
#   - Manifests exist at ~/.flywheel/manifests/ (materialize-lead-manifests.sh)
#   - ~/.flywheel/.env contains bot tokens and config
#
# The v2 wrapper closure is installed to ~/.flywheel/bin/
...
source "$DAEMON_SCRIPT_DIR/lib/lead-restart-lifecycle.sh"
...
install_wrapper() {
  # FLY-954: sanity + same-dir tmp + atomic mv + chmod 555.
...
# Exact observed plist carrier. Historical or bespoke paths are unknown.
classify_plist_lead_carrier() {
  local plist="$1" v2
  v2="$(lead_wrapper_path v2)" || return 1
  if grep -qF "<string>${v2}</string>" "$plist" 2>/dev/null \
    && ! grep -qF "flywheel-codex-lead-wrapper-" "$plist" 2>/dev/null; then
    printf 'v2\n'
  else
    printf 'unknown\n'
  fi
...
generate_plist_to() {
  ...
  local wrapper
  [ "$carrier" = v2 ] || {
    log "ERROR: retired Lead carrier '${carrier}' cannot generate a plist"
    return 1
  }
...
# Resolve the routine install carrier from the exact project/Lead identity.
# Absence means v2; every other value fails closed.
resolve_manifest_carrier() {
  ...
  ' "$PROJECTS_JSON" 2>/dev/null)" || {
    log "ERROR: projects config has no unique v2 carrier for ${project}/${lead_id}; refusing install"
    return 1
  }
  printf '%s\n' "$carrier"
}
...
install_one_staged() {
  ...
  local desired_carrier staged_carrier
  desired_carrier=$(jq -r --arg k "$daemon_key" '.leads[$k].desired.carrier // "v2"' "$txn_json" 2>/dev/null || echo unknown)
  staged_carrier="$(classify_plist_lead_carrier "$staged_plist")"
  if [ "$desired_carrier" != v2 ] || [ "$staged_carrier" != v2 ]; then
...
cmd_install() {
  ...
  if [ "$target" = "--all" ]; then
    local count=0 manifest_count=0
    for manifest in $(list_manifests); do
      manifest_count=$((manifest_count + 1))
      if install_one_manifest "$manifest"; then
        count=$((count + 1))
      fi
    done
    if [ "$manifest_count" -eq 0 ]; then
      log "No manifests found in ${MANIFEST_DIR}/. Run materialize-lead-manifests.sh first."
    else
...
install_one_manifest() {
  local manifest="$1"
  local project_name lead_id backend
  project_name=$(jq -er '.projectName | select(type == "string" and length > 0)' "$manifest") \
    || error "Manifest projectName is invalid: ${manifest}"
  lead_id=$(jq -er '.leadId | select(type == "string" and length > 0)' "$manifest") \
    || error "Manifest leadId is invalid: ${manifest}"
  backend=$(lead_restart_project_backend "$PROJECTS_JSON" "$project_name" "$lead_id") \
    || error "projects.json has no unique backend authority for ${project_name}/${lead_id}"
  if [ "$backend" != "claude-code" ]; then
    log "lead manifest ${manifest}: skipping bespoke backend ${backend}; flywheel-daemon only installs Claude v2"
    return 1
  fi
  local daemon_key
  ...
  # Install the shared v2 wrapper only after this manifest has been proven to
  # belong to a Claude Lead. A bespoke-only install request must mutate nothing.
  if [ "${DAEMON_WRAPPER_INSTALLED_THIS_RUN:-0}" -ne 1 ]; then
    install_wrapper
    DAEMON_WRAPPER_INSTALLED_THIS_RUN=1
  fi
...
cmd_status() {
  ...
  if [ "$has_leads" = false ]; then
    log "No manifests found. Run materialize-lead-manifests.sh first."
  fi
