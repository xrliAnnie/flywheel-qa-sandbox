...
#
# FLY-142 (Option 1, this change): the legacy CommDB path below now reads BOTH
...
COUNT=$(sq_ro "SELECT COUNT(*) FROM messages WHERE to_agent='${EXEC_ID}' AND type IN ('instruction','response') AND read_at IS NULL AND expires_at > datetime('now');" || echo "0")
...
JSON_ROWS=$(sq "SELECT json_group_array(json_object('id', id, 'from_agent', from_agent, 'type', type, 'content', content, 'parent_id', parent_id, 'parent_content', parent_content)) FROM (SELECT m.id AS id, m.from_agent AS from_agent, m.type AS type, m.content AS content, m.parent_id AS parent_id, q.content AS parent_content FROM messages m LEFT JOIN messages q ON q.id = m.parent_id WHERE m.to_agent='${EXEC_ID}' AND m.type IN ('instruction','response') AND m.read_at IS NULL AND m.expires_at > datetime('now') ORDER BY m.created_at ASC, m.rowid ASC);") || exit 0
