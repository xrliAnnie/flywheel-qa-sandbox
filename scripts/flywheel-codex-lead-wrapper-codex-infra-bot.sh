#!/bin/bash
# FLY-871 / FLY-2190: launchd -> the windowed full-access Codex InfraBot.
# Source of truth for the installed state-bin wrapper selected by
# com.flywheel.lead.flywheel-codex-infra-bot-lead.plist.
set -euo pipefail
...
LAUNCHER="${MAIN_REPO}/packages/teamlead/scripts/run-codex-infra-bot-tui.sh"

log() { echo "[codex-infra-bot-wrapper] $(date '+%H:%M:%S') $*"; }
...
host_tmux_gate_fail_loud() {
  ...
  local reason="host_tmux_selection_gate_unavailable_codex-infra-bot"
  local title="InfraBot host tmux selection gate unavailable"
  local body="Host tmux selection gate failed with exit ${rc}; refusing InfraBot birth until host selection authority is restored."
...
if [ -n "$HOST_TMUX_GATE_OVERRIDE" ]; then
  ...
  case "$FLYWHEEL_STATE_DIR" in
    ...
    *)
      log "Host tmux gate override is not allowed for the production state root — refusing InfraBot birth."
...
FLYWHEEL_HOST_TMUX_BOUND_TRANSACTION="keepalive:codex-infra-bot" \
FLYWHEEL_HOST_TMUX_MOUNT_POINT="scripts/flywheel-codex-lead-wrapper-codex-infra-bot.sh" \
  "$HOST_TMUX_GATE_BIN" gate codex-infra-bot || HOST_TMUX_GATE_RC=$?
if [ "$HOST_TMUX_GATE_RC" -eq 0 ]; then
  FLYWHEEL_HOST_TMUX_TARGET_SHA="$HOST_TMUX_TARGET_SHA" \
  FLYWHEEL_HOST_TMUX_BOUND_TRANSACTION="keepalive:codex-infra-bot" \
  FLYWHEEL_HOST_TMUX_MOUNT_POINT="scripts/flywheel-codex-lead-wrapper-codex-infra-bot.sh" \
    "$HOST_TMUX_GATE_BIN" verify codex-infra-bot || HOST_TMUX_GATE_RC=$?
fi
if [ "$HOST_TMUX_GATE_RC" -ne 0 ]; then
  log "Host tmux selection gate held or unavailable (exit ${HOST_TMUX_GATE_RC}) — refusing InfraBot birth."
  host_tmux_gate_fail_loud "$HOST_TMUX_GATE_RC"
  exit 0
fi

export FLYWHEEL_CODEX_LEAD_PROJECT_DIR="${HOME}/Dev/flywheel"

# InfraBot does not participate in the Mufasa roundtable. Shared .env values
# must not turn this backend into a roundtable consumer.
unset FLYWHEEL_ROUNDTABLE_REPLY_IN_THREAD
unset FLYWHEEL_ROUNDTABLE_CHANNEL_ID
unset FLYWHEEL_ROUNDTABLE_ENABLED
...
log "Starting Codex InfraBot WINDOWED FULL-ACCESS (TUI) Lead (projectDir=${FLYWHEEL_CODEX_LEAD_PROJECT_DIR})"
