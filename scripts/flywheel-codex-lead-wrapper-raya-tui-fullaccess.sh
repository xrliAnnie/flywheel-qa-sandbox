#!/bin/bash
# FLY-2216: launchd -> canonical windowed full-access Raya Codex Lead.
set -euo pipefail
...
LAUNCHER="${MAIN_REPO}/packages/teamlead/scripts/run-codex-lead-raya-tui-fullaccess.sh"

log() { echo "[codex-raya-fullaccess-wrapper] $(date '+%H:%M:%S') $*"; }
...
host_tmux_gate_fail_loud() {
	...
	local reason="host_tmux_selection_gate_unavailable_codex-raya"
	local title="Raya host tmux selection gate unavailable"
	local body="Host tmux selection gate failed with exit ${rc}; refusing Raya birth until host selection authority is restored."
...
if [ -n "$HOST_TMUX_GATE_OVERRIDE" ]; then
	...
	case "$FLYWHEEL_STATE_DIR" in
		...
		*)
			log "Host tmux gate override is not allowed for the production state root — refusing Raya birth."
...
FLYWHEEL_HOST_TMUX_BOUND_TRANSACTION="keepalive:codex-raya" \
FLYWHEEL_HOST_TMUX_MOUNT_POINT="scripts/flywheel-codex-lead-wrapper-raya-tui-fullaccess.sh" \
	"$HOST_TMUX_GATE_BIN" gate codex-raya || HOST_TMUX_GATE_RC=$?
if [ "$HOST_TMUX_GATE_RC" -eq 0 ]; then
	FLYWHEEL_HOST_TMUX_TARGET_SHA="$HOST_TMUX_TARGET_SHA" \
	FLYWHEEL_HOST_TMUX_BOUND_TRANSACTION="keepalive:codex-raya" \
	FLYWHEEL_HOST_TMUX_MOUNT_POINT="scripts/flywheel-codex-lead-wrapper-raya-tui-fullaccess.sh" \
		"$HOST_TMUX_GATE_BIN" verify codex-raya || HOST_TMUX_GATE_RC=$?
fi
if [ "$HOST_TMUX_GATE_RC" -ne 0 ]; then
	log "Host tmux selection gate held or unavailable (exit ${HOST_TMUX_GATE_RC}) — refusing Raya birth."
	host_tmux_gate_fail_loud "$HOST_TMUX_GATE_RC"
	exit 0
fi

if [ ! -f "$LAUNCHER" ]; then
	log "ERROR: Raya TUI full-access launcher not found: ${LAUNCHER}"
	exit 1
fi

log "Starting canonical Raya WINDOWED FULL-ACCESS (TUI) Lead"
