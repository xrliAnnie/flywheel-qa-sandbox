...
is_first_adoption_name() {
  case "$1" in
    flywheel-codex-lead-wrapper-mufasa-tui-fullaccess.sh|flywheel-codex-lead-wrapper-codex-infra-bot.sh|resident-codex-lead-recover.sh|lib/raya-standard-migration.sh) return 0 ;;
    *) return 1 ;;
  esac
}
adoption_is_complete() { # <name>
  local marker="$ADOPTION_DIR/${1//\//__}"
  [ -f "$marker" ] && [ ! -L "$marker" ] && [ -s "$marker" ] \
    && grep -Fqx "managed=$1" "$marker"
}
record_adoption() { # <name> <source-sha>
  local name="$1" source_sha="$2" marker_name="${1//\//__}"
  local marker="$ADOPTION_DIR/$marker_name" tmp="${ADOPTION_DIR}/.$marker_name.tmp.$$"
