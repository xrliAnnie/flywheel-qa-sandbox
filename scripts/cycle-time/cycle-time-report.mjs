...
export async function main() {
	...
	const sources = {
		teamlead: { name: "teamlead", source: teamDb, kind: "teamlead" },
		comm: { name: "comm", source: commDb, kind: "comm", project: args.project },
	};

	const { bundle, manifests, qaLog } = await collectSources({
		...args,
		withSnapshot: (database, use) =>
			withManagedSnapshots(
				{ label: "cycle-time-report", sources: [sources[database]] },
				({ paths }) => use(paths[database]),
			),
	...
	process.stdout.write(
		`${JSON.stringify({ ok: true, asOf: args.asOfIso, issues: args.issues, verdict: verdict.kind, outputDir })}\n`,
	);
}
