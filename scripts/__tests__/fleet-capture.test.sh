...
CASS_BOT_TOKEN=QA_SCRATCH_NOT_A_TOKEN_1
PETER_BOT_TOKEN=QA_SCRATCH_NOT_A_TOKEN_DISCORD
OPENAI_API_KEY=QA_SCRATCH_NOT_A_TOKEN_4
TEAMLEAD_PORT=9876
EOF
...
rm -rf "$OUT"
inject_leak 'QA_SCRATCH_NOT_A_TOKEN_4'
run_capture
if [ "$CAP_RC" -ne 0 ]; then
...
printf 'leftover token: QA_SCRATCH_NOT_A_TOKEN_4\n' > "$OUT/stale-leak.txt"
run_capture
if [ "$CAP_RC" -ne 0 ]; then
