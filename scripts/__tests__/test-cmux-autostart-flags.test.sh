...
#!/bin/bash
printf '%s|%s|%s|%s\n' "$FLYWHEEL_CMUX_LINKED_VIEW" "$FLYWHEEL_CMUX_VIEW_INVARIANT" "$FLYWHEEL_CMUX_STRICT_VIEW" "$*" > "$RECORD"
STUB
...
if [[ "$(cat "$record")" == "0|1|0|--watch" && ! -e /tmp/must-not-run ]]; then
  pass "extracts only the three bool flags from .env without sourcing unrelated code"
else
  fail "file extraction/exec mismatch: $(cat "$record" 2>/dev/null || true)"
fi

record="$TMP/env-precedence"
run_autostart "$record" FLYWHEEL_CMUX_LINKED_VIEW=1 FLYWHEEL_CMUX_VIEW_INVARIANT=0 FLYWHEEL_CMUX_STRICT_VIEW=1
if [[ "$(cat "$record")" == "1|0|1|--watch" ]]; then
...
record="$TMP/invalid"
run_autostart "$record"
if [[ "$(cat "$record")" == "1|1|1|--watch" ]]; then
