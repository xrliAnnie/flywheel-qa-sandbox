...
async function main() {
	...
	if (!projectName || !projectRoot) {
		console.error("Usage: npx tsx scripts/run-project.ts <project-name> <project-root> [--max-parallel N] [--issue-ids ID1,ID2]");
		process.exit(1);
	}
	...
	let nodes: DagNode[];
	if (issueIdsRaw) {
		nodes = issueIdsRaw.split(",").map(id => ({ id: id.trim(), blockedBy: [] }));
		log(`Using provided issue IDs: ${nodes.map(n => n.id).join(", ")}`);
	} else {
		console.error("ERROR: --issue-ids required. Linear API integration is planned for v0.2.1+.");
		process.exit(1);
	}
		...
		if (components.slackNotifier && result.decision) {
			...
			if (route === "needs_review" || route === "blocked") {
				components.slackNotifier.notify(
					...
					console.warn(`[Slack] Notification failed for ${nodeId}: ${err instanceof Error ? err.message : String(err)}`);
				});
			}
		}
	};
	...
	try {
		...
		// 6. Report
		console.log("\n--- Dispatch Summary ---");
		console.log(`  completed: ${dispatchResult.completed.join(", ") || "(none)"}`);
		console.log(`  shelved:   ${dispatchResult.shelved.join(", ") || "(none)"}`);
		console.log(`  duration:  ${((dispatchResult.durationMs ?? 0) / 1000).toFixed(1)}s`);
		...
		if (dispatchResult.nodeResults) {
			...
			for (const [id, nr] of Object.entries(dispatchResult.nodeResults)) {
				...
				console.log(`  ${id}: ${nr.success ? "OK" : "FAIL"} — $${cost}${nr.error ? ` (${nr.error})` : ""}`);
			}
		}

		console.log("\n========================================");
		if (dispatchResult.completed.length > 0) {
			console.log(`  Completed: ${dispatchResult.completed.length}/${nodes.length}`);
		}
		if (dispatchResult.shelved.length > 0) {
			console.log(`  Shelved: ${dispatchResult.shelved.length}/${nodes.length}`);
		}
		console.log("========================================\n");
