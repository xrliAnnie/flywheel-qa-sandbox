...
# Args: projectName [generalized] [claude|codex] [off|test-discipline]
qa_multilead_config_yaml() {
	local TEST_PROJECT_NAME="$1"
	local mode="${2:-ordinary}"
	local runner="${3:-claude}"
	local discipline="${4:-off}"
	...
	if [[ "$runner" != "claude" && "$runner" != "codex" ]]; then
		echo "[qa-multilead] config runner must be claude or codex" >&2
		return 1
	fi
	if [[ "$discipline" != "off" && "$discipline" != "test-discipline" ]]; then
		echo "[qa-multilead] config discipline must be off or test-discipline" >&2
		return 1
	fi
...
EOF
	...
	if [[ "$discipline" == "test-discipline" && "$mode" == "ordinary" ]]; then
		cat <<'EOF'
