...
fixture_command() {
  case "$1" in
    101|201|301)
      echo "claude --agent lead --append-system-prompt-file /tmp/lead-rules-bundles/proj-lead.9-lstart-a.md --model claude-fable-5"
      ;;
    102)
      echo "claude --agent lead --append-system-prompt-file /tmp/lead-rules-bundles/other-lead.9-lstart-a.md --model claude-opus-4-8"
      ;;
    302)
      echo "claude --agent lead --append-system-prompt-file /tmp/lead-rules-bundles/proj-lead.10-lstart-b.md --model claude-fable-5"
      ;;
    303)
  ...
  case "${1:-}" in
    list-panes)
      if [[ -n "$DISAPPEARING_PID" \
        ...
        printf '%s\n' "$INVENTORY" | awk -F '\t' -v pid="$DISAPPEARING_PID" '$4 != pid'
      else
        printf '%s\n' "$INVENTORY"
      fi
      ;;
