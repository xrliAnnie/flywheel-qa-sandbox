...
export class ClaudeCodeAdapter implements IAgentTeamTransport {
	...
	async verifyLastWrite(args: {
		...
		if (
			!last ||
			last.from !== args.expected.from ||
			last.text !== args.expected.content
		) {
			throw new MailboxWriteError(
				`verifyLastWrite mismatch for ${args.recipient}: expected from=${args.expected.from} content=${truncate(args.expected.content)}`,
				"verify_mismatch",
				args.recipient,
				false,
			);
