...
export const FEATURE_FLAGS: readonly FeatureFlagSpec[] = [
	...
	// ─── FLY-1448: founder decision / park projection / terminal settlement ───
	{
		name: "founder_decision_deadline_ms",
		category: "feature",
		source: "env",
		scope: "bridge_global",
		...
		valueKind: "value",
		default: "180000",
		description: "明确 founder 决定未收敛前的 severe fail-loud deadline(ms)",
		readSites: [
			envSite(
		...
		toggleable: "readonly",
		note: "下一条 founder reply 即读取新 deadline。",
	},
	{
		name: "engine_declared_park",
		category: "kill_switch",
		source: "env",
		scope: "bridge_global",
		envVar: "FLYWHEEL_ENGINE_DECLARED_PARK",
		polarity: "default_on",
		valueKind: "bool",
		default: true,
		description:
			"投影并消费 generalized workflow 的 durable engine-park authority",
		readSites: [
			envSite(
				"packages/teamlead/src/bridge/plugin.ts",
				"projectWorkflowEngineParkOutbox / receipt wake park probes",
				"call_time",
			),
		],
		toggleable: "readonly",
	...
	{
		name: "terminal_receipt_settlement",
		category: "kill_switch",
		source: "env",
		scope: "bridge_global",
		...
		default: true,
		description:
			"终态 session 的 gate/receipt/detection 可恢复清算(=0 冻结 intent 与副作用)",
		readSites: [
			envSite(
				"packages/teamlead/src/StateStore.ts",
				"ensureTerminalSettlementIntent / terminal transition",
				"call_time",
			),
			envSite(
				"packages/teamlead/src/bridge/terminal-receipt-settlement.ts",
				"TerminalReceiptSettlementProjector.pass",
				"call_time",
			),
		],
		toggleable: "readonly",
