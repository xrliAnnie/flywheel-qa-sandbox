...
export class DecisionLayer implements IDecisionLayer {
	...
	async decide(ctx: ExecutionContext, cwd: string): Promise<DecisionResult> {
		...
		// HR-PR-HANDOFF (FLY-493): a no-transport (antigravity) Runner CANNOT be
		// woken to drive the founder-gated ship, so ready_to_merge must NOT enter
		// the wake-dependent needs_review → approve_to_ship loop (it would strand
		// in approved_to_ship). Route to the terminal `pr_handoff` instead — the
		// Runner's build+PR work is done and the founder ships the PR by hand.
		// MUST run BEFORE HR-READY (which maps ready_to_merge → needs_review).
		if (
			...
			const result: DecisionResult = {
				...
				confidence: 1.0,
				reasoning:
					"No-transport (antigravity) build complete; PR open and handed to the founder for the founder-gated ship",
				concerns: [],
				decisionSource: "hard_rule",
