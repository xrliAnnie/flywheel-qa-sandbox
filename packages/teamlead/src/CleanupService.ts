import type { CleanupCandidate, StateStore } from "./StateStore.js";
...
function isAlreadyArchivedError(err: unknown): boolean {
	...
	return (
		err.message.includes("archived") || err.message.includes("Cannot send")
	);
}
...
export class FetchDiscordClient implements DiscordClient {
	...
	async sendMessage(threadId: string, content: string): Promise<void> {
		...
		try {
			...
			if (res.status === 429)
				throw new RateLimitError("Discord rate limit hit");
			if (!res.ok) {
	...
	async archiveThread(threadId: string): Promise<void> {
		...
		try {
			...
			if (!res.ok) {
				const b = await res.text().catch(() => "");
				throw new Error(`archiveThread failed (${res.status}): ${b}`);
			}
		} finally {
			clearTimeout(timeout);
		}
...
export class CleanupService {
	...
	start(): void {
		...
		this.timer = setInterval(() => {
			this.check().catch((e) => console.error("[CleanupService]", e));
		}, this.intervalMs);
	}
	stop(): void {
		if (this.timer) {
		...
		for (const c of this.store.getEligibleForCleanup(this.thresholdMinutes)) {
			...
			try {
				...
				if (err instanceof RateLimitError) {
					console.warn("[CleanupService] Rate limited");
					break;
				}
				console.warn(
					`[CleanupService] Failed ${c.thread_id}:`,
	...
	private async archiveOne(candidate: CleanupCandidate): Promise<void> {
		...
		console.log(
			`[CleanupService] Archived ${candidate.thread_id} (${candidate.issue_id})`,
		);
	}
}
