...
it("projects a sourced opinion through shared Epic facts without modifying workflow state", async () => {
	...
	try {
		...
		const initial = readEpicItemFacts(store, "flywheel", item);
		...
		const facts = readEpicItemFacts(store, "flywheel", item);
		...
		expect(
			readEpicItemFacts(store, "raya", item).ship_judgment,
		).toBeUndefined();
		db.exec("DROP TABLE ship_judgment_opinion");
		expect(
			readEpicItemFacts(store, "flywheel", item).ship_judgment,
