	...
	it.each([
		...
		async (state, cancelled) => {
			...
			const opened = store.openOperatorRework({
				...
				actor: "flywheel-eng-lead",
				founderQuote: null,
				leadFeedback: "replace the approved implementation",
	...
	it("reopens a loop-limit hold only with its current receipt and keeps iteration history continuous", async () => {
		...
		const rework = (clientRequestId: string, escalationAck?: typeof ack) =>
			store.openOperatorRework({
				...
				actor: "flywheel-eng-lead",
				founderQuote: null,
				leadFeedback: "continue after reviewing the loop limit",
				clientRequestId,
				principal: "master",
				founderAuthorEvidence: { kind: "operator", principal: "master" },
				evidence: [],
		...
		for (let attempt = 5; attempt <= 6; attempt += 1) {
			...
			if (attempt === 5) {
				...
				const reopened = store.openOperatorRework({
					...
					actor: "flywheel-eng-lead",
					founderQuote: null,
					leadFeedback: "continue after reviewing the next loop limit",
