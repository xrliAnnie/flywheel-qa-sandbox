...
describe("WorkflowEngineDispatcher", () => {
	...
	it("recovers a persisted replacement_pending after a crash between advance and materialization (FLY-1462 HIGH-2)", async () => {
		const store = await storeWithQaFailKickback();
		const fake = fakeStartDispatcher(store);
		// Simulate the crash seam: the coordinator durably advanced the delivery
		// to replacement_pending and released its lease, then the Bridge exited
		// BEFORE the dispatcher materialized the replacement. On restart the
		// claim path treats replacement_pending as settled, so without the
		// recovery scan this delivery would strand forever.
		const db = (
			store as unknown as {
				db: { run(sql: string, params?: unknown[]): void };
			}
		...
		db.run(
			"UPDATE workflow_rework_delivery SET state = 'replacement_pending', owner_id = NULL, lease_expires_at = NULL",
		);
		const reconcileWorkflowRework = vi.fn(async () => {
			throw new Error(
				"coordinator must not be consulted for replacement_pending",
			);
		});
		const dispatcher = new WorkflowEngineDispatcher({
			store,
			startDispatcher: fake.dispatcher,
			stateRoot: mkdtempSync(join(tmpdir(), "fly1462-crash-recovery-")),
			env: WORKFLOW_ON,
			now: () => new Date("2026-07-16T00:16:00.000Z"),
			resolvePredecessorHead: async () => HEAD,
			reconcileWorkflowRework,
		...
		// Idempotency across ticks: a second reconcile must not mint a second
		// execution (the rework_replacement_materialized receipt replays).
		await dispatcher.reconcile();
		expect(fake.start).toHaveBeenCalledOnce();
		store.close();
	});
	...
	it.each([
		{ seam: "after ledger started, before node running + delivery settle" },
		{ seam: "after node running, before delivery settle" },
	])(
		"finalizes a launched replacement stranded at replacement_pending ($seam) (FLY-1462 R2 HIGH-3)",
		async ({ seam }) => {
			const store = await storeWithQaFailKickback();
			const fake = fakeStartDispatcher(store);
			const db = (
			...
			const dispatcher = new WorkflowEngineDispatcher({
				store,
				startDispatcher: fake.dispatcher,
				stateRoot: mkdtempSync(join(tmpdir(), "fly1462-finalize-")),
				env: WORKFLOW_ON,
				now: () => new Date("2026-07-16T00:16:00.000Z"),
				resolvePredecessorHead: async () => HEAD,
				reconcileWorkflowRework: vi.fn(async () => {
					throw new Error("coordinator must not be consulted");
				}),
			});

			// Drive a full recovery launch to wake_delivered first (ledger reaches
			// `started`), then surgically revert the persists a crash would have
			// lost. The launch scan never revisits a `started` ledger row — only
			// the FLY-1462 finalization recovery can converge these states.
			expect(await dispatcher.reconcile()).toEqual({ started: 1, held: 0 });
			expect(fake.start).toHaveBeenCalledOnce();
			const launched = fake.requests[0]?.generalizedExecution?.executionId;
			expect(launched).toEqual(expect.any(String));
			...
			if (seam.startsWith("after ledger started")) {
				db.run(
					"UPDATE workflow_run_node SET state = 'admitted' WHERE run_id = 'run-1' AND node_id = 'implement' AND attempt = 2",
				);
			}
			...
			const settled = store.listWorkflowReworkDeliveries({
	...
	it("refuses to settle a fast-completed replacement (done stays terminal, delivery held for manual convergence) (FLY-1462 R4 HIGH-3)", async () => {
		const store = await storeWithQaFailKickback();
		const fake = fakeStartDispatcher(store);
		const db = (
			store as unknown as {
		...
		const dispatcher = new WorkflowEngineDispatcher({
			store,
			startDispatcher: fake.dispatcher,
			stateRoot: mkdtempSync(join(tmpdir(), "fly1462-done-preserve-")),
			env: WORKFLOW_ON,
			now: () => new Date("2026-07-16T00:16:00.000Z"),
			resolvePredecessorHead: async () => HEAD,
			reconcileWorkflowRework: vi.fn(async () => {
		...
		expect(await dispatcher.reconcile()).toEqual({ started: 1, held: 0 });
		// Fast completion: the replacement already committed as a PLAIN DAG
		// transition (its verification path was still pending, so the
		// rework-chain routing was skipped). Post-hoc settling cannot restore
		// that semantics (R4 HIGH-3) — the recovery must refuse with ZERO
		// writes: `done` stays terminal and the delivery stays visible at
		// replacement_pending for manual convergence.
		db.run(
			"UPDATE workflow_run_node SET state = 'done' WHERE run_id = 'run-1' AND node_id = 'implement' AND attempt = 2",
		);
		db.run(
			"UPDATE workflow_rework_delivery SET state = 'replacement_pending', owner_id = NULL, lease_expires_at = NULL",
		);

		const outcome = await dispatcher.reconcile();
		expect(outcome.started).toBe(0);
		expect(outcome.held).toBeGreaterThanOrEqual(1);
		expect(fake.start).toHaveBeenCalledOnce();
		expect(store.getWorkflowRunNode("run-1", "implement", 2)).toMatchObject({
			state: "done",
		});
		expect(
			store.listWorkflowReworkDeliveries({ states: ["replacement_pending"] }),
		).toHaveLength(1);
		store.close();
	});

	it("refuses to touch a node rebound to another execution (FLY-1462 R3 HIGH-3 context CAS)", async () => {
		const store = await storeWithQaFailKickback();
		const fake = fakeStartDispatcher(store);
		const db = (
			store as unknown as {
		...
		const dispatcher = new WorkflowEngineDispatcher({
			store,
			startDispatcher: fake.dispatcher,
			stateRoot: mkdtempSync(join(tmpdir(), "fly1462-rebound-")),
			env: WORKFLOW_ON,
			now: () => new Date("2026-07-16T00:16:00.000Z"),
			resolvePredecessorHead: async () => HEAD,
			reconcileWorkflowRework: vi.fn(async () => {
		...
		expect(await dispatcher.reconcile()).toEqual({ started: 1, held: 0 });
		// A later recovery (e.g. dead-exec sweep) rebound the node to another
		// execution. The stale materialization receipt must not clobber it.
		db.run(
			"UPDATE workflow_run_node SET execution_id = 'someone-else', state = 'running' WHERE run_id = 'run-1' AND node_id = 'implement' AND attempt = 2",
		);
		db.run(
			"UPDATE workflow_rework_delivery SET state = 'replacement_pending', owner_id = NULL, lease_expires_at = NULL",
		);
