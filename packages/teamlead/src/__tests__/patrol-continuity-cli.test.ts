...
describe("FLY-2914 validate-report root-cause verification", () => {
	const header = "# Lead Patrol Snapshot\npatrol_schema=2\nproject: flywheel\nlead: flywheel-eng-lead\n";
	...
	async function withBridge(
		...
		const saved = { url: process.env.BRIDGE_URL, token: process.env.TEAMLEAD_API_TOKEN };
	...
	it("asks the Bridge verifier for a complete section and fails on its verdict", async () => {
		...
		await withBridge(
			() => ({ status: 200, json: { valid: false, errors: ["root_cause_snapshot_stale"] } }),
			async (_url, seen) => {
				expect(await runPatrolContinuity(["validate-report", "--report", path])).toBe(1);
				expect(seen).toEqual(["POST /api/patrol/root-causes/verify Bearer token-2914"]);
			},
		);
		await withBridge(
			() => ({ status: 200, json: { valid: true, errors: [] } }),
			async () => {
				expect(await runPatrolContinuity(["validate-report", "--report", path])).toBe(0);
			},
		);
	});
