...
describe("handleFlagStage", () => {
	it.each([
		["codex_memory_distill", FlagStoreReaders.storeCodexMemoryDistillEnabled],
		[
		...
		async (name, read) => {
			...
			for (const payload of [
				...
				expect(read(flagStore, "flywheel")).toBe(
					payload.project === "flywheel" && payload.op === "set",
				);
			}
	...
	it("FLY-2391 manages customer release global/project set and clear through SQLite only", async () => {
		const readFile = vi.fn(() => {
			throw new Error("customer release must not access config or env files");
		});
		const { deps, flagStore, store } = await makeManagedDeps({ readFile });
		const name = "auto_release_on_silence_enabled";
		for (const payload of [
			{ project: "*", to: false, op: "set" },
			{ project: "flywheel", to: true, op: "set" },
			{ project: "flywheel", op: "clear" },
			...
			const result = handleFlagStage(
				deps,
				{ name, ...payload, reason: "adjust customer release switch" },
				"o",
			);
			expect(result.code).toBe(200);
			const body = result.body as {
			...
			expect(
				FlagStoreReaders.storeAutoReleaseOnSilenceEnabled(
					flagStore,
					"flywheel",
				),
