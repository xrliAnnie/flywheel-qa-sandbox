...
import { CleanupService, RateLimitError, type DiscordClient } from "../CleanupService.js";
import { StateStore } from "../StateStore.js";
const toSqlite = (d: Date) => d.toISOString().replace("T", " ").replace(/\.\d+Z$/, "");
function mockDiscord(): DiscordClient & { sendMessage: ReturnType<typeof vi.fn>; archiveThread: ReturnType<typeof vi.fn> } {
	return { sendMessage: vi.fn().mockResolvedValue(undefined), archiveThread: vi.fn().mockResolvedValue(undefined) };
}
describe("CleanupService", () => {
	let store: StateStore; let discord: ReturnType<typeof mockDiscord>;
	beforeEach(async () => { store = await StateStore.create(":memory:"); discord = mockDiscord(); });
	it("archives eligible candidates", async () => {
		const p = toSqlite(new Date(Date.now() - 25*60*60*1000));
		store.upsertSession({ execution_id:"e1",issue_id:"GEO-100",project_name:"t",status:"completed",started_at:p,last_activity_at:p });
	...
	it("sends notification before archive", async () => {
		...
		store.upsertThread("t1","C","GEO-100");
		const c: string[] = [];
		discord.sendMessage.mockImplementation(async()=>{c.push("s")});
		discord.archiveThread.mockImplementation(async()=>{c.push("a")});
		await new CleanupService(store,discord,1440,60000).check();
	...
	it("skips send when already notified", async () => {
		const p = toSqlite(new Date(Date.now() - 25*60*60*1000));
		store.upsertSession({ execution_id:"e1",issue_id:"GEO-100",project_name:"t",status:"completed",started_at:p,last_activity_at:p });
		store.upsertThread("t1","C","GEO-100"); store.markCleanupNotified("t1");
		await new CleanupService(store,discord,1440,60000).check();
	...
	it("rate limit stops cycle", async () => {
		...
		store.upsertSession({ execution_id:"e1",issue_id:"GEO-100",project_name:"t",status:"completed",started_at:p,last_activity_at:p });
		store.upsertThread("t1","C","GEO-100");
		store.upsertSession({ execution_id:"e2",issue_id:"GEO-101",project_name:"t",status:"completed",started_at:p,last_activity_at:p });
	...
	it("start/stop", () => { const s=new CleanupService(store,discord,1440,60000); s.start();s.start();s.stop();s.stop(); });
});
