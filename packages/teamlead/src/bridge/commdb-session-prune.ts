...
export function deleteCommDbSession(
	...
	try {
		...
		// FLY-1269: phase wake/shutdown controls are execution-scoped evidence.
		// Once the backend is confirmed gone, remove them in the SAME transaction
		// as the registry row so no stale request can poison a later replay.
		return db.deleteSessionAndRunnerPhaseLifecycle(executionId) > 0;
	} catch (err) {
		console.warn(
...
export async function pruneDeadTerminalCommDbSessions(
	...
	try {
		...
		for (const s of terminal) {
			...
			db.deleteSessionAndRunnerPhaseLifecycle(s.execution_id);
			result.pruned++;
		}
	} catch (err) {
		console.warn(
