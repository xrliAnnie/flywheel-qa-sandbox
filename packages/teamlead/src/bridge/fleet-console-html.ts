...
.dag-control{background:#fff;border:1px solid #dceadd;border-left:4px solid #34c759;border-radius:12px;padding:14px 16px;margin-top:16px}.dag-control h2{font-size:16px;margin:0 0 8px}.dag-fact{display:flex;gap:8px;align-items:center;flex-wrap:wrap;font-size:12px;padding:3px 0}.dag-fact strong{min-width:92px}.dag-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}.dag-command{border:1px solid var(--blue);background:var(--blue);color:#fff;border-radius:8px;padding:6px 10px;cursor:pointer;font-size:12px}.dag-command:disabled{opacity:.45;cursor:default}.dag-warning{color:var(--amber);font-size:12px;margin-top:7px}
...
(function(){
  ...
  function renderDagControl(){
    var panel=snapshot&&snapshot.dagPanel;if(!panel){return "";}
    function fact(label,value){var missing=(value.missing||[]);return '<div class="dag-fact"><strong>'+esc(label)+'</strong><span class="badge">'+esc(value.state.toUpperCase())+'</span>'+(missing.length?'<span class="help">'+esc((value.state==="degraded"?"分歧: ":"缺: ")+missing.join(", "))+'</span>':'')+'</div>';}
    function preset(label,value){if(!value.enabled||!value.phase1Command){return '<button type="button" class="dag-command" disabled>'+esc(label)+(value.enabled?'（无需变更）':'')+'</button>';}return '<button type="button" class="dag-command" data-dag-command="'+esc(value.phase1Command)+'">'+esc(label)+'</button>';}
    var html='<section class="dag-control"><h2>DAG 控制 · 五杆三事实</h2>'+fact("v1 dispatch",panel.v1Dispatch)+fact("v2 dispatch",panel.v2Dispatch)+'<div class="dag-fact"><strong>ship reader</strong><span class="badge">'+esc(panel.shipReader)+'</span></div>';
    if(panel.degradedFlags.length){html+='<div class="dag-warning">⚠ 来源分歧/不可用: '+esc(panel.degradedFlags.join(", "))+'；preset 已禁用。</div>';}
    html+='<div class="dag-actions">'+preset("开 DAG v1 · 第一阶段",panel.presets.enableV1)+preset("开 DAG v2 · 第一阶段",panel.presets.enableV2)+preset("彻底关 DAG",panel.presets.disable);
    var phase2=panel.presets.enableV2.phase2Command;html+=phase2?'<button type="button" class="dag-command" data-dag-command="'+esc(phase2)+'">确认 claims 后关闭 legacy 回退 · 第二阶段</button>':'<button type="button" class="dag-command" disabled>第二阶段需刷新确认 claims reader</button>';
    return html+'</div><div class="help">命令用 && 失败即停；点击复制给 Lead 执行。</div></section>';
  }
  function renderFlags(){
    ...
    var html='<div class="topline"><div><h1>Feature Flags</h1><div class="subtitle">全部集中展示；全局值与 per-project override 使用同一提交流</div></div></div>'+renderDagControl();
  ...
  function delegate(event){
    var dagCommand=event.target.closest("[data-dag-command]");if(dagCommand&&event.type==="click"){var command=dagCommand.dataset.dagCommand||"";var area=document.createElement("textarea");area.value=command;document.body.appendChild(area);area.select();try{document.execCommand("copy");}finally{area.remove();}dagCommand.textContent="已复制 ✓";return;}
