...
(function(){
  ...
  function rebuildIndex(){
    ...
    snapshot.projects.forEach(function(project){
      ...
      project.dags.forEach(function(dag){dag.nodes.forEach(function(node){register(node.dispatch);if(node.policy){policyIndex[node.dispatch.targetId]=node.policy;}});});
  ...
  function renderProjectList(){
    ...
    snapshot.presentationGroups.filter(function(group){return !group.derived;}).forEach(function(group){
      ...
      var redundant=visible.length===1&&String(group.label).toLowerCase()===String(visible[0].name).toLowerCase();
      if(!redundant){html+='<div class="group-title">'+esc(group.label)+'</div>';}
  ...
  function policyCatalog(managed,surface){
    var policy=surface==="workflow"?policyIndex[managed.targetId]:null;if(policy&&policy.status==="unavailable"){return {providers:[]};}if(!policy||policy.status!=="ready"){return catalogFor(surface);}
    var providers=[];policy.models.forEach(function(model){
      var provider=null;for(var i=0;i<providers.length;i++){if(providers[i].id===model.provider){provider=providers[i];break;}}
      if(!provider){provider={id:model.provider,label:model.provider==="anthropic"?"Anthropic":"OpenAI",models:[]};providers.push(provider);}
      provider.models.push({id:model.alias,canonicalModel:model.model,label:model.label+" · "+model.model,runtimeVendor:model.vendor,efforts:model.allowedEfforts,defaultEffort:model.defaultEffort});
    });
  ...
  function selectedProvider(catalog,value,strict){
    ...
    return strict?null:(catalog.providers[0]||null);
  }
  function selectedModel(provider,value,canonicalModel){
    if(!provider){return null;}var id=canonicalModel||(value&&value.model);for(var i=0;i<provider.models.length;i++){if(provider.models[i].id===id||provider.models[i].canonicalModel===id){return provider.models[i];}}
    return null;
  }
  ...
  function icCard(role){
    ...
    var attrs=' data-role="'+esc(role.id)+'" data-handbook-refs="'+esc(JSON.stringify(role.handbookRefs||[]))+'"';
    var href=roleHref(role);
    if(!href){return '<div class="ic"'+attrs+' tabindex="-1" title="后端没有给这个角色可验证的仓库链接">'+body+'</div>';}
    return '<a class="ic ic-link"'+attrs+' href="'+esc(href)+'" target="_blank" rel="noopener noreferrer" title="在 GitHub 上打开 '+esc(role.agentFile)+'">'+body+'<span class="ic-go">↗</span></a>';
  }
  ...
  function renderModelPanel(project){
    ...
    var groupNote=labels.length?'<div class="group-note">另有 '+groupedCount+' 个 Lead 归在「'+esc(labels.join(" / "))+'」分组下，不在这张表里。</div>':"";
  ...
  function exactHandbookRoles(project,ref){
    if(!ref){return [];}
    return (project.roles||[]).filter(function(role){return Array.isArray(role.handbookRefs)&&role.handbookRefs.indexOf(ref)>=0;});
  }
  function handbookNodeState(project,node){
    if(node.execution==="gate"){return {subtitle:"人工审批"};}
    if(node.execution==="engine"){return {subtitle:"引擎执行"};}
    var ref=typeof node.handbookRef==="string"?node.handbookRef.trim():"";
    if(!ref){return {subtitle:"未关联",reason:"模板未提供执行手册引用"};}
    var matches=exactHandbookRoles(project,ref);
    if(matches.length===1){
      var role=matches[0];
      return {subtitle:role.name,ref:ref,roleId:role.id,reason:project.handbookRegistryActive?"执行手册："+role.name:"按项目 ic-roster 解析到同一手册："+role.name};
    }
    if(matches.length>1){return {subtitle:"未关联",ref:ref,reason:"执行手册引用对应多张角色卡"};}
    if(project.handbookRegistryActive){return {subtitle:"未关联",ref:ref,reason:"找不到对应角色卡"};}
    if(project.handbookRosterStatus==="unreadable"){return {subtitle:"未关联",ref:ref,reason:"项目 ic-roster 读取失败："+(project.handbookResolutionError||"原因未知")};}
    if(project.handbookRosterStatus!=="ready"){return {subtitle:"未关联",ref:ref,reason:"该项目无 ic-roster / 无 overlay"};}
    if((project.handbookResolvedRefs||[]).indexOf(ref)<0){return {subtitle:"未关联",ref:ref,reason:"该节点未在项目 ic-roster 中配置"};}
    return {subtitle:"未关联",ref:ref,reason:"该项目 roster 解析到的文件与任何唯一角色卡不同"};
  }
  function dagGraph(project,cardKey,templateId,graph,M){
    ...
    graph.loops.forEach(function(loop,index){
      ...
      var mx=(cx(from.index)+cx(to.index))/2;
      ...
      labels.push({loop:loop,mx:mx,depth:depth});
    });
    labels.forEach(function(item){
      var loop=item.loop;
      var lbl=String(loop.name||loop.id)+(typeof loop.maxIterations==="number"?" ×"+loop.maxIterations:"");
      var lw=Math.min(108,Math.max(62,NW-6));
      svg+='<rect data-loop-label-box="'+esc(loop.id)+'" x="'+(item.mx-lw/2)+'" y="'+(item.depth-9)+'" width="'+lw+'" height="17" rx="4" fill="#fff" stroke="#e6e6ec"></rect><text data-loop-label="'+esc(loop.id)+'" x="'+item.mx+'" y="'+(item.depth+3)+'" text-anchor="middle" font-size="9.5" fill="#86868b">'+esc(lbl)+'</text>';
    });
    svg+='</svg><div class="dag-chips">';
    nodes.forEach(function(node,index){
      var state=handbookNodeState(project,node);var linked=!!state.roleId;
      var attrs=' data-node="'+esc(templateId+"/"+node.id)+'" data-node-type="'+esc(node.type)+'"';
      if(state.ref){attrs+=' data-handbook-ref="'+esc(state.ref)+'"';}
      if(linked){attrs+=' data-handbook-role="'+esc(state.roleId)+'" role="button" tabindex="0" title="'+esc(state.reason)+'" aria-label="'+esc(node.name+'，'+state.reason+'，点击定位到角色卡')+'"';}
      else if(state.reason){attrs+=' data-handbook-status="unlinked" title="'+esc(state.reason)+'" aria-label="'+esc(node.name+'，未关联：'+state.reason)+'"';}
      svg+='<div class="dag-chip '+esc(node.type)+(linked?' handbook-linked':'')+'"'+attrs+' style="left:'+x(index)+'px;top:'+y(index)+'px;width:'+NW+'px;height:'+NH+'px"><span class="dc-n">'+esc(node.name)+'</span><span class="dc-f">'+esc(node.type+' · '+state.subtitle)+'</span></div>';
    });
    svg+='</div>';
  ...
  function squadCard(project,dag,index,M){
    ...
    if(dag.graph){html+='<div class="dag-scroll">'+dagGraph(project,cardKey,dag.templateId,dag.graph,M)+'</div>';}
    else{
    ...
    dag.nodes.forEach(function(node){
      ...
      var name=graphNode?graphNode.name:node.name;
      html+='<div class="dag-row"><strong>'+esc(name)+bindTag(node.dispatch)+'</strong>'+modelControl(node.dispatch,"workflow","stage 模型",false,false)+'</div>';
    });
    return html+'</article>';
  }
  ...
  function dagColumns(project){
    ...
    return segment+'<div class="stack">'+(visible.length?visible.map(function(dag,index){return squadCard(project,dag,index,metrics);}).join(""):'<div class="empty">这一类下没有模板</div>')+'</div>';
  }
  ...
  function handbookNote(){return '<div class="lay-note" data-rule="handbook-links">悬停或聚焦节点可查看执行手册，点击可定位到角色卡；未关联节点会明示原因。</div>';}
  function renderDagPanel(project){
    return '<div class="panel '+(activeTab==="dag"?'active':'')+'" data-panel="dag">'+layNote()+handbookNote()+'<div class="lay lay-v2"><aside>'+rosterBox(project)+'</aside><div>'+dagColumns(project)+'</div></div></div>';
  }
  ...
  function handleModelChange(select){
    ...
    if(part==="provider"){
      ...
      var selectedProviderForModel=selectedProvider(catalog,value,constrained);var selectedPolicyModel=selectedModel(selectedProviderForModel,{model:select.value});
      if(constrained&&selectedPolicyModel&&managed.canonicalModel===selectedPolicyModel.canonicalModel&&managed.current&&managed.current.model){value.model=managed.current.model;}
      else if(!constrained&&managed.canonicalModel===select.value&&managed.current&&managed.current.model){value.model=managed.current.model;}
      value.effort=constrained&&selectedPolicyModel?selectedPolicyModel.defaultEffort:null;
    }
  ...
  function handbookChip(target){return target&&target.closest?target.closest(".dag-chip[data-handbook-role]"):null;}
  function handbookCards(roleId){return Array.from(byId("detail").querySelectorAll(".ic[data-role]")).filter(function(card){return card.dataset.role===roleId;});}
  function clearHandbookHighlights(){byId("detail").querySelectorAll(".handbook-highlight").forEach(function(item){item.classList.remove("handbook-highlight");});}
  function highlightHandbookChip(chip){
    var cards=handbookCards(chip.dataset.handbookRole);clearHandbookHighlights();
    if(cards.length!==1){return null;}chip.classList.add("handbook-highlight");cards[0].classList.add("handbook-highlight");return cards[0];
  }
  function activateHandbookChip(chip){
    var cards=handbookCards(chip.dataset.handbookRole);if(cards.length!==1){clearHandbookHighlights();return;}
    var card=cards[0];if(typeof card.scrollIntoView==="function"){card.scrollIntoView({behavior:"smooth",block:"center"});}card.focus({preventScroll:true});highlightHandbookChip(chip);
  }
  ...
  byId("detail").addEventListener("click",function(event){
    ...
    var handbook=handbookChip(event.target);if(handbook){activateHandbookChip(handbook);return;}
    delegate(event);
  });
  byId("detail").addEventListener("mouseover",function(event){var chip=handbookChip(event.target);if(chip&&(!event.relatedTarget||!chip.contains(event.relatedTarget))){highlightHandbookChip(chip);}});
  byId("detail").addEventListener("mouseout",function(event){var chip=handbookChip(event.target);if(chip&&(!event.relatedTarget||!chip.contains(event.relatedTarget))){clearHandbookHighlights();}});
  byId("detail").addEventListener("focusin",function(event){var chip=handbookChip(event.target);if(chip){highlightHandbookChip(chip);}});
  byId("detail").addEventListener("focusout",function(event){var chip=handbookChip(event.target);if(chip){var next=event.relatedTarget;var cards=handbookCards(chip.dataset.handbookRole);if(!next||(!chip.contains(next)&&cards.indexOf(next)<0)){clearHandbookHighlights();}}});
  byId("detail").addEventListener("keydown",function(event){var chip=handbookChip(event.target);if(chip&&(event.key==="Enter"||event.key===" ")){event.preventDefault();activateHandbookChip(chip);}});
