...
/** Normalizes an absolute instant to UTC and refuses anything else. */
function scheduledAt(value: unknown, nowMs: number): string {
...
export function createVoiceScheduleRouter(
	...
	router.post("/", masterOnly(), async (req, res) => {
		try {
			...
			const scheduled = scheduledAt(request.scheduledAt, Date.parse(at));
			const meetingId =
