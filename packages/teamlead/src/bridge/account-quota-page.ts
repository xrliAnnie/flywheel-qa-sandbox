...
function renderIdentity(pageRow: AccountQuotaPageRow): string {
	...
	const note = row.note
		...
		: "";
	return `<td class="account-cell"><div class="account-name">${active}${escapeHtml(row.name)}</div><div class="account-tier">${escapeHtml(row.subscriptionTier.display)}</div>${accountMissing}${note}</td>`;
}
...
function renderNextCharge(row: AccountQuotaRow): string {
	return `<td><span class="next-charge">${escapeHtml(row.nextCharge.display)}</span></td>`;
}

function renderPageRow(pageRow: AccountQuotaPageRow): string {
	const { row } = pageRow;
	...
	const fable =
		row.provider === "Claude"
			...
			: "";
	return `<tr class="${classes}">${renderIdentity(pageRow)}<td>${renderInstant(pageRow.weeklyResetAt, pageRow.weeklyResetDisplay)}</td><td>${renderInstant(pageRow.fiveHResetAt, pageRow.fiveHResetDisplay)}</td><td>${renderProgress(pageRow.weeklyPct, "周")}</td>${fable}<td>${renderCards(row.credits)}</td>${renderNextCharge(row)}</tr>`;
}
	...
	return sections
		.map(
			(section) =>
				...
				}${section.rows.map((item) => renderPageRow(item)).join("")}</tbody>`,
		)
		.join(spacer);
}
