...
import { codexHardGateEnabled, isCodexGateSatisfied } from "./codex-gate.js";
...
export class AutoQaCoordinator {
	...
	async onMainAwaitingReview(
		...
		// FLY-827 (Lead follow-up): NEVER auto-QA a session running on an auto-QA
		// issue itself. session_role guards the QA runner spawned by auto-QA; this
		// guards a NORMAL (main-role) runner re-dispatched onto a `QA·FLY-XX` issue —
		// otherwise auto-QA would stack a QA-of-QA (the #828 hole).
		if (this.deps.store.isQaIssue(session.issue_id)) {
			this.log(
				`skip ${session.execution_id} — issue ${session.issue_id} is itself an auto-QA issue (no QA-of-QA)`,
			);
			return;
		}
		const freshTransition = opts?.freshTransition ?? true;
		// FLY-827 (Codex R2 HIGH-3): a Codex-release re-drive (from onCodexReviewResult)
		// forces the FIRST QA spawn even though it is not a fresh awaiting_review
		// transition — the session was codex-held (returned before claiming any
		// auto_qa_record), so without this override the "no owner + !freshTransition"
		// guard below would skip QA forever.
		...
		// origin/main. Checked FIRST (before policy + codex) since both need the sha.
		// The founder is held on missing-sha by isReviewHeld (R2-MED-3); here we alert
		// the Lead (never founder) and stop.
		...
		// No owner record. A GENUINE fresh review-pass gets a first QA — a
	...
	async onCodexReviewResult(event: QaResultEvent): Promise<void> {
		...
		if (!session || (session.session_role ?? "main") !== "main") {
			this.warn(
				`codex_review_result ignored — ${targetExec} is unknown or not a main session`,
			);
			return;
		}
