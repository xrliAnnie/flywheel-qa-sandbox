	...
	requestWake(): Promise<VoiceWakeOutcome> {
		...
		const task: Promise<VoiceWakeOutcome> = (async () => {
			...
			try {
				...
				// Same asymmetry as the probe above: `kickstart` runs under its own
				// 2s execFile timeout, and a host slow enough to hit it has told us
				// nothing about whether the unit started. Spending a proven failure
				// on it would let three slow moments exhaust a demand's budget.
				if (probeTimedOut(error)) {
					(this.deps.log ?? console.warn)("voice launchd wake timed out");
					return "unknown";
				}
