...
export function createRunsRouter(
	...
	router.post("/start", async (req, res) => {
		...
		// Also captures title/identifier for session metadata patching (Bug 1: FLY-24).
		...
		try {
			...
			// FLY-24: Patch session with real issue metadata from pre-flight.
			// Blueprint hydration may fail silently and use a fallback title;
			// this ensures the session always has the authoritative Linear data.
