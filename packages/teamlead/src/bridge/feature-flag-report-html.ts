 ...
 *                                 builds `flywheel-comm feature-flags set/clear …`
...
function copyPasteSurface(): string {
	...
	const script = [
		...
		'  var projectControls=document.querySelectorAll("[data-ffp-control]");',
		...
		"  }",
		"  function resetProjectControl(control){",
		'    var scope=control.querySelector("[data-ffp-scope]");',
		'    var value=control.querySelector("[data-ffp-value]");',
		"    var states={};",
		'    try{states=JSON.parse(control.getAttribute("data-ffp-state")||"{}");}catch(e){}',
		"    var state=states[scope.value]||{p:0};",
		"    while(value.firstChild){value.removeChild(value.firstChild);}",
		"    function add(v,label,selected){",
		'      var option=document.createElement("option"); option.value=v; option.textContent=label; option.selected=selected; value.appendChild(option);',
		"    }",
		"    if(state.p===1){",
		'      add("on","ON（显式行）",state.v==="on"); add("off","OFF（显式行）",state.v==="off"); add("clear","清除（回落继承）",false);',
		'      value.setAttribute("data-current",state.v);',
		"    }else{",
		'      add("inherit","继承（未设行）",true); add("on","ON（新建显式行）",false); add("off","OFF（新建显式行）",false);',
		'      value.setAttribute("data-current","inherit");',
		"    }",
		"  }",
		"  function rebuild(){",
		"    var lines=[];",
		...
		'        lines.push(FleetCmd.flagCommand({op:"set",name:name,to:to,scope:"*",reason:"phone-report"}));',
		"      }",
		"    }",
		...
		'        lines.push(FleetCmd.flagCommand({op:"set",name:s.getAttribute("data-ff-name"),to:s.value,scope:"*",reason:"phone-report"}));',
		"      }",
		"    }",
		"    for (var i=0;i<projectControls.length;i++){",
		"      var pc=projectControls[i];",
		'      var ps=pc.querySelector("[data-ffp-scope]");',
		'      var pv=pc.querySelector("[data-ffp-value]");',
		'      if(pv.value===pv.getAttribute("data-current")) continue;',
		'      var input={op:pv.value==="clear"?"clear":"set",name:pc.getAttribute("data-ffp-name"),scope:ps.value,reason:"phone-report"};',
		'      if(input.op==="set") input.to=pv.value;',
		"      lines.push(FleetCmd.flagCommand(input));",
		"    }",
		...
		'      var scope=control.querySelector("[data-ffp-scope]");',
		'      var value=control.querySelector("[data-ffp-value]");',
		'      scope.addEventListener("change",function(){resetProjectControl(control);rebuild();});',
		'      value.addEventListener("change",rebuild);',
		"      resetProjectControl(control);",
		"    })(projectControls[i]);",
		"  }",
	...
	return [
		'<div class="card">',
		'<div class="sub">勾选 / 选择要改的 flag → 复制下面的命令粘给 Lead 执行（页面本身不改任何东西）。</div>',
		'<textarea id="ffCopyText" class="ff-copy" readonly rows="4" placeholder="勾选 / 选择上面的 flag，这里会生成要粘给 Lead 的命令"></textarea>',
