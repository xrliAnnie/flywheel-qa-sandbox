...
const SUMMARY_REPO = "xrliAnnie/raya";
const summaryPull = {
	number: 24,
...
describe("FLY-2382 summary delivery ledger", () => {
	it("requires the caller to inject the repository instead of shipping a private default", () => {
		const source = readFileSync(
			new URL("../summary-delivery-ledger.ts", import.meta.url),
			"utf8",
		);

		expect(source).not.toContain("xrliAnnie/");
	});
	...
	it("reads every PR state with a bounded command and keeps only strict summary branches", async () => {
		...
		await expect(listSummaryPulls(exec, SUMMARY_REPO)).resolves.toEqual({
			status: "ok",
			pulls: [
				{
					number: 24,
	...
	it("fails closed when the bounded result could be truncated", async () => {
		...
		await expect(
			listSummaryPulls(execWith(pulls), SUMMARY_REPO),
		).resolves.toEqual({
			status: "unavailable",
	...
	it.each([
		...
		async (_name, value, reason) => {
			await expect(
				listSummaryPulls(execWith(value), SUMMARY_REPO),
			).resolves.toEqual({
				status: "unavailable",
				reason,
			});
	...
	it("normalizes command failures without leaking stderr", async () => {
		...
		await expect(listSummaryPulls(failed, SUMMARY_REPO)).resolves.toEqual({
			status: "unavailable",
			reason: "gh exit 7",
		});
		...
		await expect(listSummaryPulls(timedOut, SUMMARY_REPO)).resolves.toEqual({
			status: "unavailable",
			reason: "timeout",
		});
	});
