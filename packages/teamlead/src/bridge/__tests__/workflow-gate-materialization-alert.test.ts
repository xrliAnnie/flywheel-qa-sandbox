...
describe("workflow gate materialization fail-loud", () => {
	it.each([
		...
		["thrown failure", async () => Promise.reject(new Error("discord_down"))],
	])("alerts for an old holder after a %s", async (_label, materialize) => {
		...
		expect(alert?.payload).toEqual({
			...
			body: "FLY-1772 的 ship gate founder_gate 无法 materialize。question_id=question-1, head=aaaaaaaa。请检查 Discord thread/card binding 与 materializer 日志,修复后引擎会自动重试。",
			metadata: {
				workflowEngine: {
					runId: "run-1",
					issueId: "FLY-1772",
