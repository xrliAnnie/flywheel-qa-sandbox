...
import { probeActorProcessRemnant } from "./phase-actor-remnant.js";
import {
	PhaseOrchestrator,
	type PhaseSession,
		...
		try {
			...
			phaseOrchestratorHolder.current = new PhaseOrchestrator({
				...
				effects: {
					...
					// FLY-1462 (Codex R1-R4): second physical proof — claude-only
					// (argv needle + owner-aware pane inventory). All non-claude
					// adapters are a conservative boundary: never auto-replaced.
					probeProcessRemnant: (session) => probeActorProcessRemnant(session),
