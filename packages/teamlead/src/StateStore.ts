...
export class StateStore {
	...
	/**
	 * FLY-827 (Lead follow-up): is `issueId` itself an auto-QA issue (a separate
	 * `QA·FLY-XX` issue this Bridge created via FLY-643)? True iff any
	 * auto_qa_record was spawned ON this issue (`qa_issue_id = issueId`). Used to
	 * SKIP auto-QA when a NORMAL (main-role) runner is re-dispatched onto a QA
	 * issue — otherwise auto-QA would stack a QA-of-QA (the #828 hole). Structural,
	 * not title-pattern based: we only report true for issues we actually created
	 * as QA issues.
	 */
	isQaIssue(issueId: string): boolean {
		const stmt = this.db.prepare(
			"SELECT 1 AS ok FROM auto_qa_record WHERE qa_issue_id = ? LIMIT 1",
		);
		stmt.bind([issueId]);
		const hit = stmt.step();
		stmt.free();
