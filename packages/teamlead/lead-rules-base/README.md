...
| [`auto-qa-pipeline.md`](auto-qa-pipeline.md) | Department Leads (non-cos roles) | FLY-579 — the automatic code-review → independent-QA → founder-gate pipeline: QA auto-spawns (no Lead action), the founder is held until QA is green, QA FAIL wakes the implementer (not the founder), pipeline errors arrive as `auto_qa_stuck` alerts. Per-project opt-in (`qa.auto`); default off = byte-compatible |
| [`default-enable-policy.md`](default-enable-policy.md) | Department Leads (non-cos roles) | FLY-707 (FLY-698 epic) — built features ship ENABLED (config opt-ins like `qa.auto`/`doc_flow`, default-off env flags), not left dormant; enablement + a real fires-check is part of shipping. Security/governance gates (`founder_consent`, `founder_ux_gate`, branch protection) are EXPLICITLY EXEMPT (blind `enforce` can wedge merge/ship) |
