...
usage() {
  cat >&2 <<USAGE
Usage:
  apply-core-room-mention-gate.sh --access-file <path> --channel-id <id> [--id-only] [--dry-run]
...
# Uses ID_ONLY / DRY_RUN / preflight from the outer scope.
apply_one() {
  local af="$1" ch="$2"
  ...
  # Current state.
  local cur_req cur_patterns_is_empty
  ...
  if [ "$cur_req" = "true" ]; then
    if [ "$want_id_only" -eq 0 ] || [ "$cur_patterns_is_empty" = "true" ]; then
      ...
      log "group '$ch' already at target ($state) in $af — no-op"
      return 0
    fi
  fi
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    if [ "$want_id_only" -eq 1 ]; then
      log "DRY-RUN — would set group '$ch' requireMention:true + mentionPatterns:[] (id-only-core) in $af"
    else
      log "DRY-RUN — would set group '$ch' requireMention:true (mention-required-only) in $af"
    fi
    return 0
  fi

  # ── Atomic edit with optimistic-concurrency guard (the plugin also writes
  # this file; mirror add-roundtable-allowfrom.sh). cksum is the change detector
  # (locale-independent, unlike shasum under LANG=C.UTF-8).
  hash_file() { cksum "$1" 2>/dev/null | awk '{print $1"-"$2}'; }

  local swapped=0 backup="" attempt
  for attempt in 1 2 3 4 5; do
    ...
    if [ "$want_id_only" -eq 1 ]; then
      if ! jq --arg ch "$ch" \
          '.groups[$ch].requireMention = true | .groups[$ch].mentionPatterns = []' \
          "$af" > "$tmp" 2>/dev/null; then
        rm -f "$tmp"; sleep 0.2; continue
      fi
    else
      if ! jq --arg ch "$ch" '.groups[$ch].requireMention = true' \
          "$af" > "$tmp" 2>/dev/null; then
        rm -f "$tmp"; sleep 0.2; continue
      fi
    fi
  ...
  if [ "$want_id_only" -eq 1 ]; then
    log "group '$ch' → requireMention:true + mentionPatterns:[] (id-only-core) in $af (backup: $backup)"
  else
    log "group '$ch' → requireMention:true (mention-required-only) in $af (backup: $backup)"
  fi
  return 0
}
