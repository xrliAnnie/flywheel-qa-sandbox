...
# FLY-2357 — Codex memory uses two independent TOML tables. `memories = true`
# under [memories] parses without enabling the feature, so validate effective
# values with tomllib and never merge the two snippets. Existing tables are
# operator-owned: append only when the whole table is absent, otherwise fail
# closed instead of guessing where a missing key belongs.
ensure_memory_pins() {
  local config="$1" repair_path="${2:-$1}"
  local states features_state memories_state misplaced_master misplaced_tools
  [ -f "$config" ] || : > "$config"
  states=$(python3 - "$config" <<'PYMEMORY'
import sys
try:
    import tomllib
except ImportError:
    print("error|error|clear|clear")
    sys.exit(0)
try:
    with open(sys.argv[1], "rb") as f:
        cfg = tomllib.load(f)
...
def state(table_name, key):
    ...
    if not isinstance(table, dict):
        return "error"
    if key not in table:
        return "present_unpinned"
    return "pinned" if table[key] is True else "drift"

memories_table = cfg.get("memories")
features_table = cfg.get("features")
misplaced_master = (isinstance(memories_table, dict)
                    and "memories" in memories_table)
misplaced_tools = (isinstance(features_table, dict)
                   and "dedicated_tools" in features_table)
print(f"{state('features', 'memories')}|{state('memories', 'dedicated_tools')}|"
      f"{'misplaced' if misplaced_master else 'clear'}|"
      f"{'misplaced' if misplaced_tools else 'clear'}")
PYMEMORY
)
  IFS='|' read -r features_state memories_state misplaced_master misplaced_tools <<< "$states"
  if [ "$misplaced_master" = "misplaced" ]; then
    log "ERROR: config.toml memory pin validation failed for $repair_path: [memories] memories is misplaced; the master switch must be [features] memories = true. Fix $repair_path manually."
    return 1
  fi
  if [ "$misplaced_tools" = "misplaced" ]; then
    log "ERROR: config.toml memory pin validation failed for $repair_path: [features] dedicated_tools is misplaced; the tools switch must be [memories] dedicated_tools = true. Fix $repair_path manually."
    return 1
  fi
  case "$features_state" in
    pinned|absent) : ;;
    *)
      log "ERROR: config.toml memory pin validation failed for $repair_path: expected [features] memories = true (state=$features_state). Fix $repair_path manually."
      return 1
      ;;
  esac
  case "$memories_state" in
    pinned|absent) : ;;
    *)
      log "ERROR: config.toml memory pin validation failed for $repair_path: expected [memories] dedicated_tools = true (state=$memories_state). Fix $repair_path manually."
      return 1
      ;;
  esac
  if [ "$features_state" = "absent" ]; then
    cat >> "$config" <<'EOF'
...
EOF
  ...
  if ! python3 - "$config" <<'PYMEMORYVERIFY'
import sys, tomllib
with open(sys.argv[1], "rb") as f:
    cfg = tomllib.load(f)
ok = (cfg.get("features", {}).get("memories") is True
      and cfg.get("memories", {}).get("dedicated_tools") is True)
sys.exit(0 if ok else 1)
PYMEMORYVERIFY
  then
    log "ERROR: config.toml memory pin write verification failed for $repair_path. Fix $repair_path manually."
    return 1
  fi
}
...
write_full_access_config() {
  local cwd="$1"
  local tmp source_tmp
  tmp="$(mktemp "${HOME_DIR}/.config.toml.fullaccess.XXXXXX")" || die "mktemp failed"
  source_tmp="$(mktemp "${HOME_DIR}/.config.toml.memory-source.XXXXXX")" \
    || { rm -f "$tmp"; die "full-access: memory source staging mktemp failed"; }
  if [ -f "$CONFIG" ] && ! cp "$CONFIG" "$source_tmp"; then
    rm -f "$tmp" "$source_tmp"
    die "full-access: failed to stage existing config.toml"
  fi
  if ! ensure_memory_pins "$source_tmp" "$CONFIG"; then
    rm -f "$tmp" "$source_tmp"
    die "full-access: memory pin staging failed; original config.toml was not changed"
  fi
  {
  ...
  python3 - "$source_tmp" "$cwd" >> "$tmp" <<'PYPROJ' || { rm -f "$tmp" "$source_tmp"; die "full-access: failed to assemble trusted [projects] (parser missing or unparseable existing config)"; }
import json, sys
try:
    import tomllib
except ImportError:
...
print("\n".join(out))
PYPROJ
  # Preserve the validated operator-owned tables verbatim. Extraction is
  # accepted only when parsing the standalone block reproduces the source
  # table exactly (all keys and values), so multiline values cannot be
  # truncated by a superficial line scan. Nested/inline/dotted shapes fail
  # closed because this renderer cannot preserve them safely as flat blocks.
  python3 - "$source_tmp" >> "$tmp" <<'PYMEMORYTABLES' || { rm -f "$tmp" "$source_tmp"; die "full-access: cannot preserve [features]/[memories] safely. Fix $CONFIG manually."; }
import re, sys
try:
    import tomllib
except ImportError:
...
try:
    ...
    source_cfg = tomllib.loads(text)
except Exception as error:
    print(f"memory table source parse failed: {error}", file=sys.stderr)
    sys.exit(3)

lines = text.splitlines(keepends=True)

def extract_flat_table(name):
    table = source_cfg.get(name)
    if not isinstance(table, dict) or any(isinstance(value, dict) for value in table.values()):
        raise ValueError(f"[{name}] must be a flat explicit table")
    exact_header = re.compile(rf"^[ \t]*\[{re.escape(name)}\][ \t]*(?:#.*)?(?:\r?\n)?$")
    starts = [index for index, line in enumerate(lines) if exact_header.match(line)]
    if len(starts) != 1:
        raise ValueError(f"[{name}] must have exactly one explicit table header")
    start = starts[0]
    candidate_ends = [
        index for index in range(start + 1, len(lines))
        if re.match(r"^[ \t]*\[", lines[index])
    ]
    candidate_ends.append(len(lines))
    for end in candidate_ends:
        ...
        try:
            standalone = tomllib.loads(block)
        except Exception:
            continue
        if set(standalone) == {name} and standalone[name] == table:
            return block.rstrip("\r\n")
    raise ValueError(f"[{name}] block does not parse standalone to the exact source table")

try:
    blocks = [extract_flat_table("features"), extract_flat_table("memories")]
except ValueError as error:
    print(f"memory table preservation failed: {error}", file=sys.stderr)
    sys.exit(4)
...
PYMEMORYTABLES
  ...
  # plus the two memory pins in their distinct tables.
  python3 - "$tmp" "$cwd" <<'PYVAL' || { rm -f "$tmp" "$source_tmp"; die "full-access config validation failed (must preserve safety pins and [features].memories=true + [memories].dedicated_tools=true)"; }
import sys
try:
    import tomllib
except ImportError:
...
if not isinstance(features, dict) or features.get("memories") is not True:
    sys.exit(16)
if not isinstance(memories, dict) or memories.get("dedicated_tools") is not True:
    sys.exit(17)
if "dedicated_tools" in features or "memories" in memories:
    sys.exit(18)
sys.exit(0)
PYVAL
  mv "$tmp" "$CONFIG"
...
EOF
  ...
  ensure_memory_pins "$CONFIG" || die "config.toml memory pin enforcement failed"
