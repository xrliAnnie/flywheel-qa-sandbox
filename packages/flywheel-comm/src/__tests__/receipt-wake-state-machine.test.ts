...
describe("FLY-1392 receipt wake state machine", () => {
	...
	function admit(
		...
		const messageId = `instruction:instruction-${ordinal}`;
		const existing = db
			.listRunnerPhaseWakes("exec-1")
			.find((wake) => wake.message_id === messageId);
		if (existing) return { kind: "duplicate" as const, wake: existing };
		const queued = db
			.listRunnerPhaseWakes("exec-1")
			.filter((wake) => wake.admission_state === "queued").length;
		const admissionState =
			policy.transportAvailable === false
				? "skipped_no_transport"
				: policy.execPushCap !== undefined && queued >= policy.execPushCap
					? "suppressed_cap"
					: "queued";
		...
		return {
			...
			wake: seedWake({
				...
				metadata: {
					flywheelId: `instruction-${ordinal}`,
					execId: "exec-1",
				},
				queuedAtMs,
				admissionState,
				purpose: "message_traffic",
				sourceInstructionId: `instruction-${ordinal}`,
			}),
		};
	}
	...
	function seedWake(input: {
		...
		queuedAtMs: number;
		admissionState: "queued" | "suppressed_cap" | "skipped_no_transport";
		purpose: "message_traffic" | "park_wake";
		...
		return db
			.listRunnerPhaseWakes("exec-1")
			.find((wake) => wake.message_id === input.messageId)!;
	}

	function mailboxState(id: string): string | undefined {
		const raw = new Database(join(tmpDir, "comm.db"), { readonly: true });
		const row = raw.prepare("SELECT state FROM mailbox WHERE id = ?").get(id) as
			| { state: string }
			| undefined;
		raw.close();
		return row?.state;
	...
	it("bulk ACK consumes only legacy message traffic, not mailbox responses or control wakes", () => {
		const traffic = admit(1, 1_000).wake;
		const questionId = db.insertQuestion("exec-1", "lead-1", "review?", {
			checkpoint: "review_code",
		});
		const verdict = db.insertReviewResponseIfGateOpen({
			questionId,
			fromAgent: "bridge",
		...
		expect(mailboxState(verdict?.responseId ?? "")).toBe("QUEUED");
	});

	it("atomically writes and exactly consumes a review response through mailbox", () => {
		...
		const first = db.insertReviewResponseIfGateOpen(input);
		expect(first).toMatchObject({
			responseId: expect.any(String),
		});
		expect(db.insertReviewResponseIfGateOpen(input)).toMatchObject({
		...
		expect(mailboxState(first?.responseId ?? "")).toBe("ACKED");
		expect(db.listRunnerPhaseWakes("exec-1")).toEqual([]);
	});
	...
	it.each(["finalizeSession", "deleteSessionAndRunnerPhaseLifecycle"] as const)(
		...
		(method) => {
			db.registerSession("exec-1", "session", "proj", "FLY-1", "lead-1");
			const wake = seedWake({
				messageId: "instruction:founder-instruction",
	...
	it.each([
		...
		(method, admissionState) => {
			...
			const wake = seedWake({
				messageId: `founder-response:${admissionState}`,
				content: "founder decided",
	...
	it.each(["finalizeSession", "deleteSessionAndRunnerPhaseLifecycle"] as const)(
		...
		(method) => {
			db.registerSession("exec-1", "session", "proj", "FLY-1", "lead-1");
			const wake = seedWake({
				messageId: "instruction:escalated-founder-instruction",
				content: "founder decided",
				metadata: { origin: "founder", questionId: "question-1" },
				queuedAtMs: 1_000,
				admissionState: "queued",
	...
	it("retains an aged founder wake until its durable terminal alert is delivered", () => {
		db.registerSession("exec-1", "session", "proj", "FLY-1", "lead-1");
		const wake = seedWake({
			messageId: "instruction:aged-founder-instruction",
