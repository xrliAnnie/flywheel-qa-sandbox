...
Usage:
  ...
  flywheel-voice-poc talk --lead <LEAD_ID> --project <dir> [--device :0]
...
async function runTalk(args: CliArgs): Promise<void> {
	...
	const session: ConversationSession = await (
	...
	session.on("transcript", ({ role, text, final }) => {
		if (final)
			process.stdout.write(`  ${role === "user" ? "you" : "lead"}: ${text}\n`);
	});
	...
	mic.start((frame) => session.sendAudio(frame, PCM));
	process.stdout.write(
