...
- **Emergency restart is a separate action**: the bare command remains limited to a fresh, current AUTH-CANON(A) founder instruction. Historical conditional v2 tickets are retired. `--request <absolute-private-json>` accepts only a verified schema-v3 closeout decision under an active `lead-closeout-restart/v1` manifest: authenticated same-founder-day intent, complete recoverable/waived fleet scope, and a bound engineering announcement. The intent is a precondition, not per-instance authority; missing evidence fails closed with no automatic v1 fallback. Neither Leads nor Runners run `restart-services.sh` inline.
- **Raya follows approved main only after activation**: an active `raya-carrier-follow-main/v1` manifest lets the existing updater choose fresh `xrliAnnie/raya` `origin/main` at the next shuttle without a per-SHA founder line. It does not authorize merge, ship, identity repair, an extra trigger, or an emergency restart; quiet15m, stop-window reconciliation, rebuild, and the schema-v2 deployment receipt remain mandatory.
