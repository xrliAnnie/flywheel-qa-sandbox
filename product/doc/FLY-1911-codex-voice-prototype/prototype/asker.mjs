...
const OUT = process.env.OUT || "B1-asker",
	OGG = process.env.OGG || "question.ogg";
const ASK_AFTER = Number(process.env.ASK_AFTER_MS || 6000),
	LISTEN = Number(process.env.LISTEN_MS || 110000);
const log = (d, o) => {
...
c.once("clientReady", async () => {
	...
	conn.receiver.speaking.on("start", (uid) => {
		if (uid === c.user.id || speakers.has(uid)) return;
		speakers.set(uid, true);
		log("HEARD-SPEAKER", { userId: uid, tMs: Date.now() - t0 });
		const s = conn.receiver.subscribe(uid, {
			end: { behavior: EndBehaviorType.AfterSilence, duration: 2000 },
		});
		s.on("data", (ch) => {
			pkts++;
			if (!firstHeardAt) firstHeardAt = Date.now();
			try {
				parts.push(Buffer.from(dec.decode(ch)));
			} catch {}
		});
		s.on("end", () => {
			speakers.delete(uid);
			log("SPEAKER-END", { userId: uid, pkts });
		});
	});
	setTimeout(async () => {
	...
	setTimeout(() => {
		...
		if (parts.length) {
			...
			wav = {
				path: `${OUT}-heard-in-room.wav`,
				durationSec: +(pcm.length / br).toFixed(2),
			};
		}
		log("RESULT", {
			ok: pkts > 0,
			opusPacketsHeard: pkts,
			decoded: wav,
			firstHeardAfterAskMs:
				firstHeardAt && askedAt ? firstHeardAt - askedAt : null,
		});
		bye();
	}, LISTEN);
	function bye() {
