...
const OUT = process.env.OUT || "B1-bridge",
...
cx.stdout.on("data", (d) => {
	...
	for (;;) {
		...
		// ⚠️ 这一段是补上来的:上一场我答不了「它交办时把转写改回来没有」,
		// 因为这里根本没记 itemAdded —— grep 出来是 0,而那个 0 的意思是「我没看见」不是「没发生」。
		if (m.method === "thread/realtime/itemAdded") {
			const it = m.params?.item || {};
			if (it.type === "handoff_request") {
				stats.handoffs.push({
					heard: it.input_transcript ?? null,
					active: it.active_transcript ?? null,
				});
				log("HANDOFF", { 交办时用的转写: it.input_transcript ?? null });
			} else if (it.type === "function_call") {
				log("ITEM", { type: "function_call", name: it.name ?? null });
			}
			continue;
		}
		if (m.method === "error") {
			const e = (m.params?.error?.message || "").slice(0, 160);
			stats.errors.push(e);
			log("CODEX-ERR", { msg: e });
			continue;
		}
		...
		if (m.method === "thread/realtime/closed")
			log("CODEX", { state: "realtime closed", reason: m.params?.reason });
...
cx.stderr.on("data", (d) => {
	...
	if (s) log("CODEX-STDERR", s.slice(0, 160));
});
const rpc = (me, pa) => {
	const i = ++rpcId;
	cx.stdin.write(
...
async function startCodex() {
	await rpc("initialize", {
		clientInfo: {
			name: "fly1911-discord-bridge",
			title: "FLY-1911 Discord↔Codex",
			version: "0.0.1",
		},
	...
	if (r?.error)
		throw new Error(`realtime/start 被拒:${JSON.stringify(r.error)}`);
	return threadId;
}
...
dc.once("clientReady", async () => {
	...
	setInterval(() => {
		while (outCarry.length < FRAME48.length && outQ.length)
			outCarry = Buffer.concat([outCarry, outQ.shift()]);
		if (outCarry.length >= FRAME48.length) {
			speaker.write(outCarry.subarray(0, FRAME48.length));
			outCarry = outCarry.subarray(FRAME48.length);
	...
	player.on("stateChange", (o, n) => {
		if (o.status !== n.status) log("PLAYER", { from: o.status, to: n.status });
	});
	...
	conn.receiver.speaking.on("start", (uid) => {
		...
		s.on("data", (chunk) => {
			let pcm48;
			try {
				pcm48 = Buffer.from(dec.decode(chunk));
			} catch {
				return;
			}
		...
		s.on("end", () => {
			subs.delete(uid);
			log("STREAM-END", { userId: uid, heardChunks: stats.heardChunks });
		});
	});
	...
	setInterval(() => {
		if (!threadId) return;
		while (inCarry.length < FRAME24.length && inQ.length)
			inCarry = Buffer.concat([inCarry, inQ.shift()]);
		let frame;
		if (inCarry.length >= FRAME24.length) {
			frame = inCarry.subarray(0, FRAME24.length);
			inCarry = inCarry.subarray(FRAME24.length);
			stats.heardChunks++;
			stats.heardBytes += frame.length;
	...
	setTimeout(() => {
		log("RESULT", {
			ok: true,
			heardChunks: stats.heardChunks,
			heardBytes: stats.heardBytes,
			spokeChunks: stats.spokeChunks,
			spokeBytes: stats.spokeBytes,
