...
const SHIM = `${process.env.HOME}/.fly1911/shim`; /* gh 闸放在 PATH 最前 */
/* 中继跟桥同起同落 —— ⛔ 不做成「另外记得起一下」的步骤:
 * 两个必须同时在、才构成一条通道的东西,不该有两个生命周期。 */
let relay = null;
if (process.env.HL === "1" && process.env.HL_RELAY !== "0") {
	relay = spawn("node", [`${process.env.HOME}/.fly1911/hl-relay.mjs`], {
		stdio: ["ignore", "pipe", "pipe"],
		env: process.env,
	});
	relay.stdout.on("data", (d) =>
		log("RELAY", { line: String(d).trim().slice(0, 200) }),
	);
	relay.stderr.on("data", (d) =>
		log("RELAY-ERR", { line: String(d).trim().slice(0, 200) }),
	);
	const stopRelay = () => {
...
const cx = spawn(BIN, ["--enable", "realtime_conversation", "app-server"], {
	...
	env: {
		...
		/* ⭐ 分身用自己的 codex home。
		 * 为什么必须显式写:此前桥没设过它 ⇒ 直接继承了【起桥那个 shell】的 CODEX_HOME
		 *   (实测是 ~/.codex-infra-bot)⇒ 它的会话记录一直落在别人的家里。
		 * ⚠️ 默认值是脑子里的,环境变量是世界上的 —— 所以这里钉死,不靠继承。*/
		CODEX_HOME:
...
/* ⭐ 等待音只在【它确实在忙】的时候响:在跑命令 / 在思考 ⇒ 响;跑完、想完、它开口 ⇒ 停。
 * 动态开关:~/.fly1911/bed.switch 写 0 就静音(每秒重读一次,⛔ 不用重起桥)。*/
let bedBusy = 0;
let bedSwitch = true;
let bedSwitchAt = 0;
function bedEnabled() {
	if (process.env.BED !== "1") return false;
	const now = Date.now();
	if (now - bedSwitchAt > 1000) {
		bedSwitchAt = now;
		try {
			bedSwitch =
				readFileSync(
					`${process.env.HOME}/.fly1911/bed.switch`,
					"utf8",
				).trim() !== "0";
		} catch {
			bedSwitch = true; // 没有这个文件 = 开着
		}
	}
	return bedSwitch;
}
/* ⭐ 下限:忙够 bedMinMs 才响(默认 1000ms,~/.fly1911/bed.minms 可活改)。
 * 为什么要下限 —— 不是「省得吵」,是【信息量】:每轮都响 ⇒ 响=常态,她学不到东西;
 * 只在忙得不寻常时响 ⇒ 它才是「这次比平常久」的信号。
 * ⛔ 明确不做的另一条:**不给等待音加尾巴 / 最短时长** ——
 *   「让它听得见」只有两条路,加下限是对的那条,拉长是错的那条,这里把错的那条封死。
 * ⚠️ 代价写在明处:**长等待的头 1 秒是静的**(一个对话节拍的沉默是正常轮次,不是故障)。*/
let bedPending = null;
let bedMinMs = 1000;
let bedMinAt = 0;
function bedMin() {
	const now = Date.now();
	if (now - bedMinAt > 1000) {
		bedMinAt = now;
		try {
			const v = Number(
				readFileSync(`${process.env.HOME}/.fly1911/bed.minms`, "utf8").trim(),
			);
			if (Number.isFinite(v) && v >= 0) bedMinMs = v;
		} catch {
			bedMinMs = 1000; // 没有这个文件 = 默认 1 秒
		}
	}
	return bedMinMs;
}
function bedOn(why) {
	...
	if (bedPending) return; // 已经在等下限了,别叠第二个
	const wait = bedMin();
	const ring = () => {
		bedPending = null;
		if (bedBusy <= 0 || !bedEnabled()) return; // 等待期间它已经不忙了 ⇒ 这一次就不响
		try {
			const kind = globalThis.__bed?.(true) || "";
			log("BED", { state: "on", kind, trigger: why, 等了ms: wait });
		} catch {}
	};
	if (wait <= 0) ring();
	else bedPending = setTimeout(ring, wait);
}
function bedOff(why) {
	if (bedPending) {
		clearTimeout(bedPending);
		bedPending = null;
	}
	try {
		globalThis.__bed?.(false); // ⛔ 立刻停,不留尾巴
		log("BED", { state: "off", trigger: why });
	} catch {}
}
/* 开关被拨到 0 时,正在响的那一段也要立刻停 —— 不然「关掉」要等到下一轮才生效 */
setInterval(() => {
	if (!bedEnabled() && bedBusy > 0) {
		bedBusy = 0;
		bedOff("开关拨到 0");
	}
}, 1000);
...
/* 开场指令 = 语气要求 + 【这一场的议题正文】。
 * 议题在这里喂,不在桥启动时喂 —— 见上面 WAIT-HUMAN 那段的理由。
 * 读不到议题文件就只喂语气要求(⛔ 不编一个议题出来)。*/
function buildStartInstructions() {
	const base =
		process.env.RT_START_INSTR || "你必须始终使用中文回答。回答简短、口语化。";
	const f =
		process.env.HL_MEETING ||
		`${process.env.HOME}/.fly1911/meetings/2026-08-21-voice-meeting.md`;
	let agenda = "";
	try {
		agenda = readFileSync(f, "utf8").trim();
	} catch {
		log("MEETING", { missing: f });
	}
	if (!agenda) return base;
	log("MEETING", { chars: agenda.length, file: f });
	return `${base}\n\n以下是这一场会议的议题(她刚进房,这是当前这场会要聊的):\n\n${agenda}`;
}
...
function onCodexEvent(m) {
	...
	if (meth === "thread/realtime/transcript/done") {
		...
		/* ⛔ 「她说完就响」这一版已作废(2026-08-21 23:24,她的原话:
		 *   「我说完话、他回答完之后,该轮到我说话了……他明明没有在思考,只是在等我回答,
		 *    但等待音却一直在那里响,吵得我都不知道该说什么了」)。
		 * ⇒ 等待音的语义是【它在忙】,不是【这里有空隙】。触发改到下面 bedOn/bedOff 那一套。*/
		if (p.role === "assistant") {
			// 它开口了 ⇒ 无论前面因为什么响着,都停 —— ⛔ 不许有「说完还在响」的可能
			bedBusy = 0;
			bedOff("它开口了");
		}
	...
	if (meth === "item/started" || meth === "item/completed") {
		...
		if (it.type === "commandExecution" && meth === "item/completed") {
			bedBusy = Math.max(0, bedBusy - 1);
			if (bedBusy === 0) bedOff("命令跑完");
			return;
		}
		...
		if (it.type === "reasoning" && meth === "item/completed") {
			bedBusy = Math.max(0, bedBusy - 1);
			if (bedBusy === 0) bedOff("想完了");
			return;
		}
		if (it.type === "agentMessage" && meth === "item/completed") {
			if (it.phase === "commentary") {
				...
				let bedName = "";
				/* 预告也算「它在忙」的一种(它刚宣布要去做什么)⇒ 走同一套 bedOn。
				 * ⚠️ 但它不再是唯一入口 —— 上一版只挂在这里,而它这一轮没说预告 ⇒ 等待音一次都没响。*/
				bedBusy += 1;
				bedOn("预告");
				bedName = globalThis.__bedName?.() || "";
...
dc.once("clientReady", async () => {
	...
	await sleep(400);
	/* FLY-1911 实验 A:让房里这个 Codex 继承 Honey Lemon 的身份
	 * · baseInstructions = identity.md + MEMORY.md 索引(实测 56KB 吃得下)
	 * · sandbox read-only = 能查、不能改(实测:写被拒 / 库和仓库都读得到)
	 * 这两条都是探针里量出来的,不是猜的。 */
	const HL_FILES = (
		process.env.HL_FILES ||
		"/Users/xiaorongli/Dev/flywheel/.lead/flywheel-product-lead/identity.md," +
			process.env.HOME +
			"/.claude/agent-memory/flywheel-product-lead/MEMORY.md," +
			process.env.HOME +
			"/.fly1911/hl-datamap.md,/Users/xiaorongli/Dev/flywheel/packages/teamlead/lead-rules-base/founder-only-authority.md"
	).split(",");
	const stripFm = (x) => x.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/, "");
	let baseInstr = "";
	if (process.env.HL === "1") {
		for (const f of HL_FILES) {
			try {
				baseInstr += `${stripFm(readFileSync(f, "utf8")).trim()}\n\n`;
			} catch {
				log("HL", { missing: f });
			}
		}
		log("HL", {
			baseInstructionsChars: baseInstr.length,
			files: HL_FILES.length,
		});
	}
	/* 她 20:05 推翻了 read-only:「我们没有必要把它做成 read-only」
	 * ⇒ 默认不再钉 sandbox(= 写文件/改代码/派活/网络/联系 Lead 全开)。
	 *   HL_SANDBOX 仍可显式钉回来(read-only 那一档留着,不删)。
	 * ⛔ merge 进 main 仍关着 —— 见下面 PATH 里的 gh 闸;那是机制,合同另有一份。 */
	const startParams =
		process.env.HL === "1"
			? process.env.HL_SANDBOX
				? {
						baseInstructions: baseInstr,
						sandbox: process.env.HL_SANDBOX,
						approvalPolicy: "never",
					}
				: { baseInstructions: baseInstr, approvalPolicy: "never" }
			: {};
	/* ⭐ 等真人进房再建会话 —— 这就是「把喂议题从【桥启动时】挪到【有人进房时】」。
	 * 为什么:实测 5 场,起会话→首问间隔 24/34/38 秒都读得到议题,510/798 秒都读不到。
	 *   桥先起、她十几分钟后才进来 ⇒ 她永远落在读不到的那一档。
	 * ⇒ 现在:桥起来只是待命(已经在房里、能听),**真人进房那一刻才建会话并把议题喂进去** ⇒ 间隔≈0。
	 * ⛔ 不保留「桥起时也喂一次」当保险 —— 那样分不清是哪一次生效的。
	 * HL_WAIT_HUMAN=0 可以关掉这个门(回到旧行为,用来做对照)。*/
	if (process.env.HL_WAIT_HUMAN !== "0") {
		log("WAIT-HUMAN", { msg: "已在房里待命,等真人进来再建会话" });
		await new Promise((resolve) => {
			const ch0 = dc.channels.cache.get(process.env.VOICE_CHANNEL_ID);
			/* 判据用「除了我自己以外有人进来了」,不用「是不是真人」——
			 * 理由:探针也是 bot,若只认真人,这条路在测试里永远走不到 ⇒ 我们会验不到她要走的那条代码。
			 * 而对产品来说也更对:别的东西进房时喂一次议题无害。*/
			const someoneElse = (ch) =>
				!!ch && [...ch.members.values()].some((m) => m.user.id !== dc.user?.id);
			if (someoneElse(ch0)) {
				log("WAIT-HUMAN", { msg: "进来之前房里已经有人了,直接建会话" });
				return resolve();
			}
			const onVoice = (oldS, newS) => {
				const u = (newS.member || oldS.member)?.user;
				if (!u || u.id === dc.user?.id) return; // 我自己进房不算
				if (newS.channelId === process.env.VOICE_CHANNEL_ID) {
					dc.off("voiceStateUpdate", onVoice);
					log("WAIT-HUMAN", { msg: `有人进房:${u.tag} ⇒ 现在建会话` });
					resolve();
				}
			};
			dc.on("voiceStateUpdate", onVoice);
		});
	}
	const th = await rpc("thread/start", startParams);
	...
	if (VER === "v3") {
		...
		const r = await rpc("thread/realtime/start", {
			...
			realtimeStartInstructions: buildStartInstructions(),
		});
		if (r?.error) {
			M.startRejected = r.error;
			log("RESULT", { ok: false, where: "realtime/start", error: r.error });
	...
	// 状态行要拿曲名,但不能因此把等待音打开 ⇒ 单独一个只读的取名字口子
	globalThis.__bedName = () => BED_NAMES[BED.kind] || BED.kind || "";
	...
	/* 🔴 2026-08-24:播放账目【周期性】记一次,不再只在整场收尾记。
	 * 起因:她 01:20 进房、七分钟一句话没听到、01:27 走了 —— 而这一场直到 RUN_MIN 到点
	 * 才会写 manifest,所以她走的时候【一条播放记录都没有】。事后想回答「那七分钟到底有
	 * 没有声音出去」,唯一的账在两小时之后,而且是整场累计的、分不出她那一段。
	 * ⇒ 只在收尾记账的仪器,恰好在【被中途放弃的那种场次】上什么也留不下 ——
	 *   而那正是最需要事后复盘的场次。
	 * ⛔ 这里【只加记录】,不改任何播放行为:读的是已经存在的那两个计数口子。*/
	schedule(30000, () => {
		log("OUT-ACCOUNT", {
			播放缓冲: globalThis.__outStats?.() ?? null,
			播放器自己的账: globalThis.__missed?.() ?? null,
			这一秒收到的语音累计字节: stats.pcmOutBytes,
		});
	});
