...
HL=1 BED=1 RT_VERSION=v2 RT_VOICE=marin OUT=an1-bridge RUN_MIN=60 TOKEN_VAR=TEST_BOT_TOKEN_1 \
  SW_PACER=1 SW_JITTER=1 SW_DEPTH=1 node bridge-hl.mjs > an1-bridge.out 2>&1 &
echo "bridge pid $!"
sleep 25
OUT=an1-probe TOKEN_VAR=TEST_BOT_TOKEN_2 OGG="$LAB/hl-q1.ogg" ASK_AFTER_MS=6000 LISTEN_MS=80000 \
  node asker2.mjs > an1-probe.out 2>&1
