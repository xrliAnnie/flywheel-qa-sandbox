...
const OUT = process.env.OUT || "T4-p6",
	SIL_MIN = Number(process.env.SILENCE_MIN || 30);
...
const c = spawn(BIN, ["--enable", "realtime_conversation", "app-server"], {
...
c.stdout.on("data", (d) => {
	...
	for (;;) {
		...
		if (m.method === "thread/realtime/sdp") {
			...
			log("TX", { phase, role: p.role, text: p.text });
		} else if (
			/^thread\/realtime\/(started|closed|error)$|^error$/.test(m.method || "")
		) {
			events.push({
				phase,
				method: m.method,
...
await rpc("initialize", {
	clientInfo: { name: "fly1911-p6", title: "p6", version: "0.0.1" },
...
const dch = pc.createDataChannel("oai-events");
const dcKinds = new Set();
dch.onMessage.subscribe((msg) => {
	let o;
	try {
...
let seq = (Math.floor(Date.now() / 7) % 30000) + 1,
	ts = 0;
const ssrc = track.ssrc || 123456789;
const speakQ = [];
let rtpOut = 0;
...
(function mic() {
	...
	let g = 0;
	while (next <= now && g < 10) {
		const pl = speakQ.length ? speakQ.shift() : SIL;
		try {
			track.writeRtp(
				new RtpPacket(
					new RtpHeader({
...
function say() {
	...
	let off2 = 12,
		data = null;
	while (off2 + 8 <= src.length) {
		const idd = src.toString("ascii", off2, off2 + 4),
			sz = src.readUInt32LE(off2 + 4);
		if (idd === "data") {
			data = src.subarray(off2 + 8, off2 + 8 + sz);
			break;
		}
		off2 += 8 + sz + (sz % 2);
	}
	let n = 0;
	for (let o = 0; o < data.length; o += BPF) {
		let sl = data.subarray(o, o + BPF);
		if (sl.length < BPF) {
			const p2 = Buffer.alloc(BPF);
			sl.copy(p2);
			sl = p2;
		}
		try {
			speakQ.push(Buffer.from(opus.encode(sl, FRAME)));
			n++;
		} catch {}
	}
	return n;
...
async function ask(tag) {
	phase = tag;
	const _before = tx.length,
		t0 = Date.now();
	const frames = say();
	log("ASK", { phase: tag, frames });
	// 等它答(最多 150 秒)
	const until = Date.now() + 150000;
	while (Date.now() < until) {
		await sleep(500);
		if (tx.filter((x) => x.phase === tag && x.role === "assistant").length)
			break;
	}
	...
	const res = {
		phase: tag,
		它听成: got.filter((x) => x.role === "user").map((x) => x.text),
...
phase = "静默中";
const silStart = Date.now(),
	silEnd = silStart + SIL_MIN * 60000;
const marks = [];
while (Date.now() < silEnd) {
	await sleep(60000);
	const mk = {
		分钟: +((Date.now() - silStart) / 60000).toFixed(1),
		pcState: pc.connectionState,
		dcState: dch.readyState,
		rtpOut,
		rtpIn,
		会话事件数: events.length,
	};
...
function fin() {
	...
	console.log(
		"=== 结论 ===",
		JSON.stringify(M.结论 ?? { fail: M.fail ?? M.startRejected }, null, 1),
	);
	try {
		c.stdin.end();
	} catch {}
