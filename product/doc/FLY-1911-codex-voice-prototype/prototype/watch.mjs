...
if (!FILE) {
	console.error("用法: node watch.mjs <bridge.jsonl>");
	process.exit(1);
}
...
let _itIsWorking = false; // 它是不是正在干活(问完还没答)
...
function raw(o, t) {
	// 看不懂的照样露出来,只是压暗
	console.log(
		`${C.grey}${hhmmss(t)}  · ${JSON.stringify(o).slice(0, 220)}${C.r}`,
	);
}
	...
	silenceTicker = setInterval(() => {
		...
		if (process.stdout.isTTY)
			process.stdout.write(
				`\r${C.yellow}   ⏳ 它还在干活…已经 ${el} 秒没出声${C.r}   `,
			);
...
function render(rec) {
	...
	switch (d) {
		case "GATEWAY":
			return line("🔌", C.dim, `机器人已登录 Discord（${o?.bot ?? "?"}）`, t);
		case "JOINED":
			return line("🚪", C.green, "已经进入语音房", t);
		case "READY":
			sessionStart = new Date(t).getTime();
			return line("●", C.b + C.green, "上线了 —— 现在可以在房里跟它说话", t);
		case "CODEX":
			if (o?.state?.includes("started"))
				return line(
					"🧠",
					C.dim,
					`语音会话已建立（通道 ${o.version ?? "?"}）`,
					t,
				);
			if (o?.state?.includes("closed"))
				return line("🔚", C.yellow, `语音会话关闭：${o.reason ?? "未说明"}`, t);
			return raw(o, t);
		case "SPEAKING":
		...
		case "TX": {
			...
			const waited = lastSheSpokeAt
				? `（她说完后 ${secs(lastItSpokeAt - lastSheSpokeAt)} 秒）`
				: "";
			return line(
				"💬",
				C.b + C.green,
				`它说：「${o.text}」${C.dim}${waited}`,
				t,
			);
		}
		case "HANDOFF":
			return line(
				"📤",
				C.mag,
				`它把活交出去了，交办时用的原话：「${o?.交办时用的转写 ?? "（没记到）"}」`,
				t,
			);
		case "ITEM":
			return line("🔧", C.mag, `它在用工具：${o?.name ?? o?.type ?? "?"}`, t);

		/* ↓ 任务 2 挖出来的:这些事件一直都在,只是上一版桥把它们 continue 掉了。
       其中「打算做什么」那条在她问完 7.8 秒就有了 —— 而它开口说话要 58 秒。
       她要的 indicator 不用做,是本来就有、被我们扔了。 */
		case "PLAN": // agentMessage phase=commentary:它自己说打算干嘛
			stopSilenceTicker(); // 有话说了就先停计时
			line("🗒", C.b + C.mag, `它说它打算：${o?.text ?? ""}`, t);
			startSilenceTicker(); // 说完继续等正式答案
			return;
		case "RUN": // commandExecution:它真的在跑命令
			return line(
				"⚙️",
				C.mag,
				`它在跑：${String(o?.command ?? "")
					.replace(/^\/bin\/zsh -lc\s*/, "")
					.slice(0, 120)}`,
				t,
			);
		case "THINK":
			return line("🤔", C.dim, "它在想…", t);
		case "ANSWER": // agentMessage phase=final_answer:文字版答案(比声音早)
			stopSilenceTicker();
			return line(
				"✅",
				C.b + C.green,
				`答案出来了（文字版，声音随后）：${String(o?.text ?? "")
					.replace(/\n+/g, " ")
					.slice(0, 300)}`,
				t,
			);
		case "QUOTA": {
			const p = o?.primary?.usedPercent;
			if (p === undefined || p === null) return;
			if (p < 80) return; // 没到边上就不打扰
			return line("📊", C.yellow, `额度用了 ${p}%`, t);
		}
		case "MCP":
			if (o?.status === "failed")
				return line(
		...
		case "STREAM-END":
			return;
		case "CODEX-ERR":
			return line("❌", C.red, `Codex 报错：${o?.msg ?? o}`, t);
		case "CODEX-STDERR":
			return; // 噪音
		case "RESULT": {
			stopSilenceTicker();
			line("🏁", C.b, "这一场结束了", t);
			console.log(
				`${C.dim}   她说过的：${JSON.stringify(o?.userTranscripts ?? [])}`,
			);
			console.log(
				`   它说过的：${JSON.stringify(o?.assistantTranscripts ?? [])}${C.r}`,
			);
			return;
		}
		default:
...
console.log(
	`${C.dim}这扇窗只负责一件事：让你随时知道它在干嘛。灰色的 · 行是我还没翻成人话的事件，`,
);
...
async function pump() {
	...
	if (sz < pos) {
		pos = 0;
		carry = "";
		console.log(`${C.yellow}（日志被换掉了，从头开始读）${C.r}`);
	}
	if (sz === pos) return;
		...
		rs.on("end", () => {
			...
			for (const l of lines) {
				...
				try {
					...
					console.log(`${C.grey}${s.slice(0, 200)}${C.r}`);
					continue;
				}
				try {
					render(rec);
				} catch (e) {
					raw({ 渲染失败: String(e), 原始: rec }, rec?.t);
				}
			}
			res();
		});
...
setInterval(() => {
	...
	const m = Math.floor((Date.now() - sessionStart) / 60000),
		s = Math.floor(((Date.now() - sessionStart) % 60000) / 1000);
