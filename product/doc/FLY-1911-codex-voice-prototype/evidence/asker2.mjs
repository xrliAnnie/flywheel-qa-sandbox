...
const OUT=process.env.OUT||"T3-asker", OGG=process.env.OGG||"question.ogg";
const ASK_AFTER=Number(process.env.ASK_AFTER_MS||8000), LISTEN=Number(process.env.LISTEN_MS||150000);
...
const parts=[]; const subs=new Set(); let bytes=0, askedAt=null, askDoneAt=null, recStartAt=null;
...
c.once("clientReady",async()=>{
 ...
 function grab(uid,why){
   if(uid===c.user.id||subs.has(uid))return; subs.add(uid); log("HEARD-SPEAKER",{userId:uid,why});
   const s=conn.receiver.subscribe(uid,{end:{behavior:EndBehaviorType.Manual}});
   s.on("data",ch=>{ try{ const p=Buffer.from(dec.decode(ch)); if(!recStartAt){recStartAt=Date.now();log("REC-START",{})}
     parts.push(p); bytes+=p.length }catch{} });
 }
 conn.receiver.speaking.on("start",uid=>grab(uid,"speaking.start"));
 try{ const ch=await g.channels.fetch(CHAN);
   for(const [uid] of ch.members) grab(uid,"进房时就在房里"); }catch(e){ log("MEMBER-FETCH-FAIL",{e:String(e?.message||e)}) }
 setInterval(async()=>{ try{ const ch=await g.channels.fetch(CHAN); for(const [uid] of ch.members) grab(uid,"轮询发现") }catch{} },5000);
 setInterval(()=>{ if(recStartAt) marks.push([Date.now(),bytes]) },2000);
 setTimeout(async()=>{
   ...
   askedAt=Date.now(); log("ASKED",{});
   try{ await entersState(player,AudioPlayerStatus.Idle,30000) }catch{}
   askDoneAt=Date.now(); log("ASK-DONE",{问话时长ms:askDoneAt-askedAt});
 },ASK_AFTER);
 setTimeout(()=>{
   let wav=null;
   if(parts.length){ const pcm=Buffer.concat(parts),sr=48000,ch=2,br=sr*ch*2,h=Buffer.alloc(44);
     ...
     wav={path:OUT+"-room.wav",durationSec:+(pcm.length/br).toFixed(2),bytesPerSec:br} }
   const M={ok:parts.length>0, recStartAt, askedAt, askDoneAt, wav, marks,
