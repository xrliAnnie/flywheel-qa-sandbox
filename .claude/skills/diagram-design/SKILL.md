---
name: diagram-design
description: Create or redraw polished architecture, flow, sequence, state, data, process, org, timeline, and other editorial diagrams as standalone HTML/SVG/PNG. Use when a visual explains structure or behavior better than prose or a table, including Mermaid or draw.io imports; not for simple lists, one-shape visuals, or routine UI mockups.
license: MIT
metadata:
  version: "2.6"
---

<!-- PROVENANCE: upstream=https://github.com/cathrynlavery/diagram-design commit=648c2a597839301e06df1e7434a08bde9f42eed3 path=skills/diagram-design license=MIT original-skill-sha256=553191201faf2e61a9b3cc24f01b688de172f67b3f9da69b544f194252fcacee local-modifications=description-shortened,provenance-and-FLY-2015-operational-limits-added -->

## Flywheel operational limits (FLY-2015)

These are compatibility disclosures carried with the installed skill. They do not change the upstream drawing workflow.

<!-- FLY-2015-LIMIT-1-AUTO-GENERATION-UNVERIFIED -->
1. **Approved look; unverified automatic-generation stability.** Annie approved a hand-drawn FLY-2004 comparison, not output automatically generated by the installed skill; automatic-generation stability is unverified. Do not present that approval as proof that this skill will reproduce the same quality automatically.

<!-- FLY-2015-QA-AUTO-GENERATION-E2E -->
   **Required first post-install QA:** First post-install QA must explicitly invoke the installed skill to generate the same subject and compare it with the approved hand-drawn B arm. Judge the result side by side; producing a file is not a pass.

<!-- FLY-2015-LIMIT-2-CJK-FONT-FALLBACK -->
2. **The bundled fonts have no Chinese glyphs.** FLY-2004 measured the Chinese sample at the same width across all tested declarations even while the remote faces loaded. Instrument Serif, Geist, and Geist Mono therefore fall back to a system CJK font for our Chinese body copy. Do not choose the three replacement families in this vendor change; choose them only after inspecting the first real generated diagram.

<!-- FLY-2015-LIMIT-3-PUBLISH-REPORT-FONT-CSP -->
3. **`publish-report` blocks the external fonts.** A report without its own CSP receives Flywheel's protective CSP, which does not allow the Google Fonts stylesheet, so those faces do not load on the hosted page. The `hasCsp` branch preserves a report's existing CSP, but do not add a CSP meta tag to bypass Flywheel's protection.

<!-- FLY-2015-LIMIT-4-MOVING-UPSTREAM -->
4. **This upstream is a moving target.** Maintenance is concentrated and the project is changing quickly. This copy is pinned to the provenance commit above; every update requires a new provenance bump plus script, asset, license, and behavior review. Never follow upstream `main` silently.

### Additional hosting audit disclosure
