...
- Exact-head review `d6bc3840` APPROVED，并给出两个值得同 PR 关闭的 MEDIUM advisory。其一指出 stock macOS Bash 3.2 在 `set -u` 下展开空 `FB_ARGS[@]` 会 abort；新增真正无 CLI args 的 D0 先 RED（11 passed / 1 failed），生产三处展开改为 Bash 3.2-safe 的零参数保真形式后 GREEN（12/12）。其二指出 voice tree 的 child 可在 `pgrep` 与 `ps` 之间退出；新增 race fixture 先 RED（16 passed / 2 failed），实现改为仅将已确认死亡的 tuple 视作已回收、仍存活但无 start tuple 继续 fail-close，GREEN 20/20。现代 Bash 整份 restart harness 再次 132/132，provider contract 19/19。
- `git diff --check`:通过。
