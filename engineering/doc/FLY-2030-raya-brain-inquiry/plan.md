...
| M1-c' | **规则装载接线(R1v2-4 + R2v2-1 + R3v2)**:`summary-inflow.md` 不会被自动发现——同时接 **两条显式 load path**:`scripts/claude-lead.sh` 的枚举 + `scripts/lead-rules-bundle.sh` `compute_lead_rule_bundle`,并更新 `lead-rules-base/README.md` 表与 `lead-rules-bundle.test.ts`。**audience = registry 数据里的显式 assignment,不是公式**(R3v2-1:任何按 role 推导的公式要么排除 Mufasa 得 10 人、要么提前替 founder 选掉 CoS-聚合变体):每个 Lead 行**必填**闭合枚举 `summaryRole ∈ {"producer","aggregator","recipient","exempt"}`——**缺失/未知/类型错 = 两个入口(`ProjectConfig` 与 raw `lead-identity` 解析)都 fail-loud 拒绝**(R3v2-2,⛔ 不用 `!== "recipient"` 这类 fail-open 负比较);一次性数据迁移随 M1 flywheel PR 给全部行赋值(Raya=recipient;CoS=aggregator;infra bot=exempt;部门 Lead=producer;**Mufasa = producer**,founder 已在 scope-final.md §⑥拍板,11 人 fixture 就此钉死)。**粒度变体由数据激活,不由公式预选**:变体 A(一 Lead 一条)audience = producer 集;变体 B(按项目聚合)audience = **每项目恰一名作者,由 project 级字段 `summaryAggregatorLeadId` 显式指定**(R4v2-1:「无 aggregator 回落 producer」在 Growth——无 CoS、双 producer——会解出 2-3 个作者,**回落规则废除,无 CoS 项目也必须显式指定,没有任何推导回落**)。**条件化 schema(R5v2-1,选方案 a)**:该字段**仅在变体 B 为 founder 已选定的活动粒度时必填并做跨行校验**(恰一作者/该 Lead 存在于该项目/0 或多个 fail-loud);变体 A 下迁移可整体省略它;变体 B 的 Growth 作者依 scope-final.md §⑦暂定 `mufasa-lead`,未来 founder 真选 B 时随粒度选择再呈她过目;字段一旦出现(任一模式)仍过类型/成员资格校验。**canonical 活动粒度来源(R6v2-1:⛔ 字段存在与否绝不用来推断模式——那是 fail-open)**:founder 选定的粒度落**单一文件 `~/.flywheel/summary-config.json`** = `{granularity: "per-lead"|"per-project", setBy, setAt}`(闭合枚举,值由 founder 拍,⛔ 本 plan 不赋值);`flywheel-comm` 导出**唯一 reader** `readSummaryGranularity()` → `selected(A|B) | unselected`,**两个入口(teamlead `ProjectConfig` 已依赖 flywheel-comm、raw lead-identity 同包)只经这一个 reader 取模式**,⛔ 不接受各自的 ad-hoc 参数;`unselected/未知值`:普通 registry 解析不受伤(不打挂 Bridge 无关启动),但 **duty/audience 计算、迁移、激活一律 fail-loud 拒绝**;**模式值纳入 canonical summary-assignment 投影与回执摘要**——A↔B 切换必然变摘要、作废旧回执、要求重新校验+新回执。两套 fixture 都预先测好(变体 B fixture 含真实 Growth 名册、钉 `mufasa-lead` 且断言总数恰 6),founder 拍哪个切哪个。**单一投影源(R3v2-2)**:谓词结果进 `CanonicalLeadIdentity`(含 digest)与 `identityEnvProjection`,shell 两路只消费投影出的已校验 duty 值,⛔ 不各自重算/默认;Claude 路径查询失败 fail-stop | flywheel | **测试**:两变体 assignment 集正/负 fixture(含 CoS-as-aggregator、**无 CoS 项目也必须显式指定**——真实 Growth 名册、恰 6 断言、缺/空/类型错/非本项目成员的指定四类负测(变体 B 模式下)、infra bot/external/Raya 恒排除、**Raya 任意挂载不含**);负测:非法 `summaryRole`(typo/casing/类型)、投影缺失、config 查询失败、改 duty 值 → canonical digest 变;**模式来源组(R6v2-1)**:A + 省略 aggregator 字段 → 过;B + 任一项目缺指定 → fail;两入口解析出同一模式(同源断言);模式缺失/未知 → 激活路径 fail-loud 而无关解析不受伤;**A↔B 切换 → 投影摘要变、旧回执不可用**。试点证据 = 被选 Lead 的**有效 bundle/prompt 含该文件**;只 merge 文件不算已激活 |
...
- **registry 数据先行栅栏(R4v2-2:生产 `~/.flywheel/projects.json` 现 16 行零 `summaryRole`,新必填 parser 直接上会打挂 Bridge/Lead 配置加载)**:flywheel PR 只交付 schema + 迁移命令,**生产进程继续跑旧的兼容 parser(旧 parser 容忍新字段存在)**;operator(Lead)在激活前依次:①取得 founder 粒度选择(角色 assignment 已由 scope-final.md §⑥–⑦关闭);②取既有 projects-config 写锁,核对整文件期望 SHA(stale 拒绝),写同目录候选文件、**全部行过两个新入口校验**后原子 rename;③记录迁移回执:post-image SHA + 完整 16 行 assignment 证据 + **canonical summary-assignment 投影摘要**(投影 = summaryRole 集 + aggregator 指定 + **活动粒度模式值**(R6v2-1),使无关的 model/config 编辑不误伤,而 A↔B 切换必然作废回执)。**激活门 = 对「当下」registry 的校验,不是回执存在性(R5v2-2)**:激活新 parser/规则 bundle 或重启任何生产 Lead/Bridge **之前**,对 live 文件跑两个新入口校验,并把其 summary-assignment 投影摘要与回执绑定的摘要比对——**不一致或 parse 失败 = 在停掉任何旧进程之前拒绝激活**;rename 前任何失败旧字节原样保留。**测试**:stale-SHA 拒绝、迁移中断保留旧文件、缺行校验、无回执拒绝激活、**有效回执后删/坏一个 summaryRole → 拒绝**、**有效回执后改 aggregator 指向 → 拒绝**、**无关字段编辑不误伤(投影摘要不变 → 放行)**。
...
**R6(v2)(2026-08-28,blob a0d9e526,rev9 manifest 48f0aa67)= CHANGES REQUESTED,1 项,采纳(R5 两项确认关闭):**

| # | 处置 |
|---|---|
| 1 条件化 schema 无 canonical A/B 模式来源;回执摘要不绑模式 | ✅ 单一文件 `~/.flywheel/summary-config.json`(闭合枚举,founder 拍值,plan 不赋值)+ flywheel-comm 唯一 reader,两入口同源消费(⛔ 字段存在推断模式 = fail-open,禁止;⛔ 各自 ad-hoc 参数 = split-brain,禁止);unselected 时普通解析不受伤、duty/迁移/激活 fail-loud;模式值入投影摘要 ⇒ A↔B 切换作废旧回执;六项模式来源测试组 |
