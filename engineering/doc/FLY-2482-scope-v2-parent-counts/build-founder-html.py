...
diagram lost its <svg id="FLY-2482-dN"> root, or a diagram lost its inner width
...
for n, name in diagrams.items():
    ...
    if f'id="FLY-2482-d{n}"' not in svg:
        sys.exit(f"diagram {name} lacks svgId FLY-2482-d{n}")
...
if "【页面意见汇总】" not in out or 'ISSUE = "FLY-2482"' not in out:
