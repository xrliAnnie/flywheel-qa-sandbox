...
- 不碰 `packages/`、不接 pnpm workspace/build 线;测试用 `npx vitest run qa-fly904/` 直跑。
...
| T3 | turn=qa ∧ qa=running ∧ fixRounds<3 | `qa_fail` | fixRounds+1;qa→parked;implement→running(wake);turn→implement | +1 |
...
| X6 | verdict=pass 后的 `design_redo` / `qa_fail`(ship 挂起期不得回卷) | `bad_state` |

拒绝理由**判定优先级**(消除 X 类之间的歧义,implement 按此顺序写 guard):
0. `bad_state` — turn=null(已 finalize,X5)时任何事件,先于一切判定;
1. `fix_cap_exceeded` — 仅 `qa_fail` 且 turn=qa ∧ qa=running ∧ fixRounds≥3 时;
...
- I4:`ok:false` 时返回的 state 与入参**引用相等**(零变异)。
...
- `X1 not_your_turn` … `X6 no rollback after pass`(6 条,X1/X2 可各带 2-3 个变体)
- `I1-I4 invariants`(公共断言 helper,折进每条 case;另加 1 条全链 happy-path
  串烧:T1→T2→T3→T4→T5→T6,沿途断言 epoch 序列 1,2,3,4,5,5,5)
- 预期合计 ≥ 15 条 case;全部通过 `npx vitest run qa-fly904/` 直跑。
...
1. 独立复跑 `npx vitest run qa-fly904/` 全绿;
