...
Usage: python3 build-founder-html.py "<review meta text>" <review-history-html-file>
...
meta = sys.argv[1] if len(sys.argv) > 1 else "设计评审进行中"
history = (
...
svgs = {
    ...
    "__SVG_D3__": "d3-hold-states.svg",
    "__SVG_D4__": "d4-release-sequence.svg",
}
...
out = tpl.replace("__REVIEW_META__", meta).replace("__REVIEW_HISTORY__", history)
for ph, fn in svgs.items():
    svg = (here / fn).read_text(encoding="utf-8")
    assert svg.lstrip().startswith("<svg"), fn
    m = re.search(r'id="([^"]+)"', svg)
    assert m and m.group(1) == "FLY-2478-" + ph[6:8].lower(), (fn, m and m.group(1))
    svg = shrink_numbers(svg)
    assert ph in out, ph
    out = out.replace(ph, svg, 1)
    assert ph not in out, ph
assert "__SVG_" not in out and "__REVIEW_" not in out
