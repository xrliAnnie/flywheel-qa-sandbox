...
> 升级 Tadashi)。每 Phase 先测后码(TDD),频繁 commit。命令命名(/talk 建议)定稿权在
> Annie+Honey Lemon —— 实现用 config 默认值,别写死。

## 0. 目标与非目标

**目标**:`/talk [topic]` → 自动建立项 issue → orchestrator bot + 耳朵 bot 进 `#huddle` VC →
...
> 命名注记(gate ⑥):`/talk` 是 Discord slash 命令;voice-core 另有**本机** talk CLI(543,
> 本机 mic/喇叭)。同词不同面 —— 文档/README 里写明区分,代码里命令模块叫 `TalkCommand`
> (Discord 面),不去动 voice-core cli。
...
|--------|------|--------------------------|
| **单 PR「/talk 助理模式」**(545 PR-1 之上增量) | voice-core `systemPreamble` 可选字段 + voice-bridge `assistant/*` 全部 + SessionSlot + 两条 Bridge 路由(若 545 PR-2 未先落)+ 配置合同 | ①S-A1 首音/打断实测数字;②staged E2E(529 Room 纪律,QA 当 founder 全流程);③**Annie 真用一轮 /talk**(A 侧北极星;A/B 对比由她各开一轮后拍板) |

## 3. 文件结构

```
packages/voice-bridge/src/
├── SessionSlot.ts               # 新:单场互斥(/meet 与 /talk 共用;acquire/release;
│                                #   占用中二次 acquire → 显式拒绝带「有会进行中」文案)。
│                                #   545 PR-2 的 HuddleSession 后落地时改用同一 slot(对齐注记)。
...
└── assistant/
    ...
    └── TalkCommand.ts           # slash 命令注册(名字可配,默认 "talk")+ Join link button +
...
(不设 = /talk 不注册,A 关,字节兼容;545 的 B 面行为零变化):
...
```
idle ──/talk [topic]──▶ invoked(SessionSlot.acquire 失败 → 回执「有会进行中」即止;
      │                 建立项 issue(title「2026-07-07 15:00 · talk(Annie)」+topic)via
...
- **单场互斥**:SessionSlot 为 /meet(545)与 /talk 共享的进程内闸;A 先落地 = A 引入,
...
- **P9 真机闭环 + E2E**:①staged(529 Room 纪律:QA 真人当 founder,测试 guild,全流程
  /talk→聊→收尾→issue 关闭链接可点);②生产部署(daemon 重启走 Bridge 重启纪律,攒批)→
  **Annie 真用一轮 /talk**(A 侧北极星)→ evidence;A/B 对比卡(exploration §7 四维度)
...
| A3 | 真机闭环:/talk 全流程跑通,issue 建/关、summary 引用原话、TIV 卡片链接可点 | staged E2E 记录 |
| A4 | 简报零等待:/talk 到助理开场 ≤3s(缓存命中路径);简报滞后时 TIV 提示可见 | staged E2E + 单测 |
...
| A8 | **Annie 真用一轮 /talk 全程跑通**(A 侧北极星;A/B 拍板是她各开一轮后的体感决定) | 终验 evidence |
...
建 worktree · 早晚会 · OpenAI Realtime/本地模型 · 多场并发 · 跨重启恢复(断了重开 /talk)·
