...
| codex-home-link-truth.sh / codex-home-reconcile.mjs / config/codex-quota-home-policy.json / codex-credential-cutover.sh / codex-home-credential-sweep.mjs (FLY-2404, FLY-2523) | 共享 Codex 凭据真身的单 home 栅栏迁移、外置回执幂等 reconcile、owned-pause 批量切换与 dry-run-first 存量盘点/清理闭包；cutover 只通过脚本相对路径调用另外两件 | **included guarded operations closure**；reconcile 仅处理显式 approved inventory，活动/未知 home fail closed，先留外置 intent，原 auth 先备份；不由 packaged runtime 自动创建调度器 | codex-home-link-truth.test.sh + codex-home-reconcile.test.sh + codex-credential-cutover.test.sh + codex-home-credential-sweep.test.sh + package-onboard.test.sh 白名单闭包 |
