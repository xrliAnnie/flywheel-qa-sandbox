...
| flywheel-buddy.sh / flywheel-buddy-steps.sh / lib/buddy-*.sh | 无命中(buddy-captain-preview 见下) | included(零改动) | gate④ 全树 grep |
...
| flywheel-bridge-wrapper.sh | `:30 FLYWHEEL_DIR` 默认(host.json 覆盖);`:63 lib/bridge-port.sh`(进包);`:79 meta-alert.sh`、`:94 lead-alert.sh`(**不进包**);`:156-159` dist 分支(P1-2 已落) | **included-and-patched**(P1-2);meta-alert/lead-alert 引用 = **included-guarded**(`[[ -x ]]` 守卫 + direct-curl fallback,缺失即降级不致命);FLYWHEEL_DIR 经 host.json flywheelDir=current 解析 | packaged-seams.test.sh(dist 分支两侧)+ provision-prebuilt.test.sh(host.json) |
| flywheel-lead-wrapper.sh | `:35 FLYWHEEL_DIR` 默认;`:179/:185/:189/:194 packages/teamlead/...` | **included-mirror-covered**(全部经兼容镜像)+ host.json flywheelDir | 冒烟②(claude-lead.sh 路径合同)|
...
| converge-flywheel-bin.sh | `:61 FILES` 含 restart-services.sh(packaged 树永缺 → 每次 Lead 启动 rc=1 + 误报 alert);`ALERT_BIN` 默认 lead-alert.sh(不进包) | **included-and-patched**(本 PR 新增):prebuilt 哨兵 → FILES 去 restart-services.sh(lead/bridge wrapper 照管);monorepo 逐字不变;ALERT_BIN 引用 `\|\| true` 守卫 = included-guarded | packaged-seams.test.sh(prebuilt 下 restart-services 缺失不告警不 rc=1 + 无哨兵 sentinel) |
...
| packaged/restart-packaged-services.sh | 无非注释命中 | included(FLY-1062 新增件,P3 更新链用) | packaged-seams.test.sh(supervisor seam 经由 + 健康门) |
...
| lead-alert.sh / meta-alert.sh | Bridge 侧 alert 管道依赖生产 fleet 形态(claims.db 等);Bridge dist 内 LeadAlertNotifier 覆盖客户告警面 | 不在白名单;全部引用位点带 `[ -x ]` 守卫 + fallback(bridge-wrapper、converge `\|\| true`) |
