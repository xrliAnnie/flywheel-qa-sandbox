...
| codex-home-link-truth.sh / codex-home-launch-fence.mjs / codex-home-reconcile.mjs / codex-home-reconcile-cycle.mjs / lib/codex-home-reconcile-process.mjs / config/codex-quota-home-policy.json / codex-credential-cutover.sh / codex-home-credential-sweep.mjs (FLY-2404, FLY-2523) | 共享 Codex 凭据真身的单 home 栅栏迁移、Lead 启动租约、注册表派生清单、外置回执幂等 reconcile、私有进程组 TERM/KILL/退出证明、owned-pause 批量切换与 dry-run-first 存量盘点/清理闭包；cutover 只通过脚本相对路径调用另外两件 | **included guarded operations closure**；cycle 仅挂入既有 health/updater/restart 节律，reconcile 仅处理显式 approved inventory，活动/未知 home fail closed，Lead 启动与迁移共用 home fence，子进程退出未证明则保留独立 fence，先留外置 intent，原 auth 先备份；不创建调度器 | codex-home-link-truth.test.sh + codex-home-launch-fence.test.sh + codex-home-reconcile.test.sh + codex-home-reconcile-cycle.test.sh + codex-home-reconcile-process.test.sh + codex-home-migration-alert.test.sh + codex-home-migration-overdue-mutation.test.sh + codex-credential-cutover.test.sh + codex-home-credential-sweep.test.sh + package-onboard.test.sh 白名单闭包 |
