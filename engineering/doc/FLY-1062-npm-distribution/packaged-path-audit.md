...
| flywheel-comm `summary.js` + founder-only-authority.md × Raya 自有仓名 | package-onboard-smoke.test.sh ④d/④e:客户 Lead 显式为 `summaryRole=exempt`;即使给出其余 plausible selector,`summary` 也在调用 `gh` 前以 `summary_duty_required` fail-closed;规则文档只描述窄豁免,不含 fetch verb |
