...
| flywheel-lead-wrapper-v2.sh / flywheel-lead-attach.sh / lib/lead-address.sh / teamlead:scripts/lead-body.sh / teamlead:scripts/lib/lead-body-receipt.sh (FLY-1663) | v2 launchd carrier 的完整运行时闭包；wrapper 直接启动 private tmux server，body/receipt 承担一次性 Claude 进程与 resume 三振，attach 只负责 cmux 持久重连 | **included runtime closure**；packaged bootstrap 按 projects.json `carrier` 选择 v1/v2，未知值 fail-close | package-onboard.test.sh 白名单闭包 + packaged-seams.test.sh 安装闭包 + fly1663 runtime harness |
