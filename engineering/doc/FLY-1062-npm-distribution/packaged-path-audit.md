...
| flywheel-patrol-continuity.mjs | `packages/teamlead/dist/patrol-continuity-cli.js` via launcher realpath | **included-mirror-covered** — same-payload read-only collector and report gate; missing module fails closed, no checkout fallback | patrol-continuity-launcher.test.sh source/managed/payload execution + package-onboard.test.sh whitelist closure |
| package-gate.mjs / package-gate-worker.mjs / package-gate-host.py / package-gate-reporter.mjs / lib/package-gate-core.mjs (FLY-2702) | local full-package gate entrypoint, host semaphore supervisor/watchdog, isolated worker, receipt reporter, and unchanged gate orchestration core | **included runtime closure** — host admission defaults disabled until the fixed control file is enabled; CI and explicit opt-out bypass before state access; enabled admission fails closed on corrupt or unsafe state | package-gate.test.mjs + package-gate-host.test.mjs + package-onboard.test.sh whitelist closure |
...
| flywheel-voice-wrapper.sh | host-config + restart/tmux gates; `packages/voice-codex/dist/cli.js` | **included-as-is**; generic voice packages use the generated compatibility mirror; Silero model and license remain beside dist under models | package-onboard-smoke.test.sh (tarball closure, installed native model and CLI config rejection) |
...
| lead-patrol-snapshot.sh / flywheel-node-dwell-control.mjs / lib/codex-quota-summary.mjs / codex-quota-readiness-receipt.mjs (FLY-2465) | STEP 2 reads only the public stateRoot/codex-quota/switch-audit.jsonl projection; source and flat state/bin paths resolve the same summary helper; latest committed and failed attempts remain visible | **included runtime closure**; PO_SCRIPT_FILES ships the patrol, dwell launcher and helper; both converge FILES branches install the helper with silent first adoption | codex-quota-summary.test.sh invokes the real package assembler, normal/prebuilt converger, and flat STEP 2; verifies audit facts and zero severe adoption alerts |
| flywheel-lead-wrapper-v2.sh / flywheel-codex-lead-wrapper-mufasa-tui-fullaccess.sh / flywheel-codex-lead-wrapper-codex-infra-bot.sh / flywheel-lead-attach.sh / flywheel-view-attach.sh / flywheel-node-status.sh / lib/lead-address.sh / teamlead:scripts/lead-body.sh / teamlead:scripts/lib/lead-body-receipt.sh / teamlead:scripts/lib/lead-model-authority-receipt.mjs (FLY-1663/FLY-1884/FLY-2190/FLY-2238) | 三类受管 launchd carrier 与 cmux 可见面的完整运行时闭包；两条原本仅存在于宿主 state-bin 的 Codex wrapper 纳入同一打包/converge 权威，v2 wrapper 直接启动 private tmux server，body/receipt 承担一次性 Claude 进程、resume 三振与 model authority 的 last-good receipt，lead attach 负责 Lead 持久重连，view attach 负责 runner 镜像重建后的持久重连，node status 负责无窗口节点的非空白状态面 | **included runtime closure**；packaged bootstrap 按 projects.json `carrier` 选择 v1/v2，未知值 fail-close；A0 census 对三种 carrier 做源码/已部署逐字节校验 | package-onboard.test.sh 白名单闭包 + package-onboard-smoke.test.sh 镜像闭包 + packaged-seams.test.sh S0/S7 安装闭包 + lead-model-authority-receipt.test.sh + host-tmux-selection-census.test.sh + fly1663 runtime harness + fly1884-view-attach.test.sh |
| flywheel-codex-lead-wrapper-raya-tui-fullaccess.sh / resident-codex-lead-recover.sh (FLY-2216) | Raya 的 canonical Codex TUI carrier，以及所有显式 roster opt-in resident Codex Lead 的业务级假活探测命中后原地重启与恢复验证；两者由 package/converge 权威随其他受管 carrier 一同安装 | **included runtime closure**；wrapper 复用 host-tmux gate 与 canonical cmux 会话，generic recover helper 复用既有 restart-storm、identity guard 和 alert 骨架，并按 target fail-close；缺任一都会让 packaged Raya carrier 或 opt-in Codex Lead 自愈失效 | raya-resident-carrier.test.sh + resident-codex-lead-recover.test.sh + package-onboard.test.sh 白名单闭包 |
| codex-home-link-truth.sh / codex-credential-cutover.sh / codex-home-credential-sweep.mjs (FLY-2404) | 共享 Codex 凭据真身的单 home 栅栏迁移、owned-pause 批量切换与 dry-run-first 存量盘点/清理闭包；cutover 只通过脚本相对路径调用另外两件 | **included guarded operations closure**；不由 packaged runtime 自动执行；缺 token、Bridge/CommDB/process/lease/authority 任一证明均 fail closed，默认 sweep 零写，apply 另取 owned pause | codex-home-link-truth.test.sh + codex-credential-cutover.test.sh + codex-home-credential-sweep.test.sh + package-onboard.test.sh 白名单闭包 |
| restart-storm-gate.py / host-tmux-selection-gate.sh / lib/bounded-run.sh / lead-alert.sh / meta-alert.sh | wrapper 的两道 fail-closed gate;gate hold 的 Discord + 本地独立告警;gate 缺失时 wrapper 的有界同步本地告警 | **included runtime closure** — 五件缺一会导致 packaged Bridge/Lead 永久不起或静默;逐文件白名单,保持可执行位 | packaged-seams.test.sh S0(真实 assembly 产物五件齐全且可执行)+ host-tmux-selection-mounts.test.sh |
| payload root `.flywheel-build-sha` (FLY-2190 O1) | packaged runtime 没有 `.git`，首次出生也可能尚无 state `deployed-sha`；六类 carrier 的短时 host-tmux 收据仍须绑定不可变 40 位部署身份 | **assembly-stamped** — `po_assemble` 从 source `HEAD` 写入，payload allowlist/npm `files` 显式纳管，release gate 对 source `HEAD` 做相等校验；wrapper 只在 git 与 state identity 均缺失时读取非 symlink 文件 | package-onboard.test.sh A1/G6 + host-tmux-selection-mounts.test.sh prebuilt first-birth slice |
...
| flywheel-lead.sh / flywheel-config-lock.sh / flywheel-config-lock.py / migrate-summary-registry.sh / lib/lead-host-tmux-gate.sh / teamlead:scripts/lib/canonical-lead-identity.sh (FLY-2444) | 外仓 Lead 注册事务、Claude/Codex launcher、launchd 生命周期、host tmux gate 与打包态 summary receipt 重铸的最小运行时闭包 | **included runtime closure**；逐文件白名单，launcher/helper 进入 state-bin checksum 收敛，迁移 wrapper 优先已安装 dist | flywheel-lead-packaging.test.sh + flywheel-lead.test.sh + lead-restart-lifecycle-generic-carrier.test.sh |
| lib/raya-standard-migration.sh (FLY-2445) | 标准 Raya Lead 的 source-only P0–P7 可恢复迁移事务、cursor fence、两仓配对 rollback target 与 v2 deploy receipt helper；由既有 updater 唯一调用，不建立新 daemon | **included guarded operations closure**；进入逐文件白名单，生产调用必须经 updater 前置、checkpoint 与证据校验，不能单独宣告部署成功 | raya-standard-migration.test.sh + updater-raya-deploy.test.sh + package-onboard.test.sh 白名单闭包 |
| lib/lead-backend-migration.sh (FLY-2459) | Codex 部门 Lead 受控迁移的 source-only 窗口入口、静态前置检查与单目标 staged plist load；委托已编译的 teamlead/comm 工具和既有 generic launcher | **included guarded operations closure**；逐文件白名单与 state-bin 收敛；实际入口复验 updater 进程/锁/admission 窗口，不新增独立 restart 或部署授权 | lead-backend-migration.test.sh + lead-backend-migration-load.test.sh + lead-backend-migration-wave.test.sh + package-onboard.test.sh 白名单闭包 |
...
| packaged/flywheel-auto-update.sh | 无 monorepo 路径;耐久壳入口 | included(FLY-2392) | packaged-auto-update-wrapper.test.sh |
