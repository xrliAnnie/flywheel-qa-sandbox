...
状态：R1 CHANGES_REQUESTED；修正后重新显式设计评审。本文仅设计；Step1 已完成，不重跑；全部实现和真人 QA 尚未执行。
...
| 指定 Lead 上下文 | `packages/teamlead/src/bridge/voice-session-context.ts`（新），`voice-routes.ts` | 由可信Bridge 读取、按 session lease 授权后提供；不把路径或 DB 访问交给模型 |
...
| 共享接口消费者兼容 | `packages/voice-bridge/src/__tests__/{qa-fly545-r4,wire-meeting-resident,wire-meeting,qa-fly545-r23}.test.ts` 及 core 的 GeminiLiveSession/fakes | 仅新增方法适配与类型兼容，RoomIO 迁移仍归 G1 |
...
Bridge 上下文服务从 vendor-neutral ProjectConfig / compileBusinessDirectory 解析 leadId，不能用只含 Codex Leads 的 resident roster。**cosContext 是可选覆盖，不是开场必需配置**：生产当前17个Lead全部缺席，Raya/Honey Lemon必须走下面已核过的派生路径。优先取可信启动manifest中的personaPath/AGENT_SOURCE覆盖（不得由请求任意指定）；没有覆盖时复用 `lead-capabilities/rule-sources.ts:224-234` 的 persona 规则：`<projectRoot>/.lead/<leadId>/identity.md`，仅不存在时 fallback 同目录 agent.md；两者都无才拒绝。路径做 realpath，拒绝逃逸/身份冲突，不读本体 auth 文件。

新增 `resolveVoiceContextSources` 在 Bridge 侧统一产出来源 manifest（不是第二份Lead名册），优先合法 `cosContext` 明示路径，否则从**现有启动载体绑定**派生：

1. Claude `memory:user`：用同一 identity frontmatter 的 `name`（必须等于注册leadId）及有效 CLAUDE_CONFIG_DIR，读取 `<claudeConfigDir>/agent-memory/<leadId>/MEMORY.md`。Honey Lemon 实际为 `~/.claude/agent-memory/flywheel-product-lead/MEMORY.md`；不选旧项目自动memory，更不按显示名猜目录。identity 由 `packages/teamlead/scripts/claude-lead.sh:983` 安装，`:2938` 以 `--agent`启动，是已存在的绑定。
2. Raya 的单Lead专用workspace：当前 identity 的开场约定是 `<projectRoot>/memory/MEMORY.md`；实际 projectRoot 为 `/Users/xiaorongli/Dev/raya-lead-workspace`。本单将**这一现有约定**作为 `workspace-memory` 来源规则，仅用于有明确单Lead workspace归属及该启动约定的注册项，不因某文件恰好存在就给任意Lead加载。叠加由当前载体有效 CODEX_HOME（`scripts/lib/lead-address.sh:22` + `scripts/flywheel-lead.sh:179`）绑定的 `memories/memory_summary.md`（本机 `~/.codex-raya/...`）；不递归读rollout/archive，不读取auth/config秘密。
3. 其他未识别的启动约定返回 `context_source_unresolved` 并列明缺少哪项；不得回退到整个项目/别的Lead memory。显式 cosContext 若提供则必须完整可读，错误时不静默切派生路径。若生产载体绑定变动，在开场重新解析并将版本写入manifest。

这两个人格无需先补cosContext即可得到现有文件集合。实现T2必须用 `cosContext:undefined` 的实际生产形状验证；只生成带路径/摘要的来源manifest，不在本设计节点写任何生产配置。读取根授权由受信Bridge给出，模型/请求方不能提交任意路径。原文文件全量加载，不通过摘要替代选定的memory文件；原memory索引指向的历史材料不自动递归展开，在manifest中明确列为未加载引用。
...
每个文件记录相对路径、字节数、SHA-256；整体记录 rosterDigest、snapshotDigest、capturedAt、source revision、各状态字段是否 unavailable。快照到 open **发起**时刻不超过 60 秒；若过期只允许在发起前重取一次，仍过期就返回 context_stale。open 自发起起最多60秒，不因完成时快照变旧再次启动；状态不是持续实时镜像，回答可说明快照时间。状态读取失败拒绝以“此刻状态已装载”进入验收，不能 silently omit。

**预算按真实文件计量重新制定，不再使用8192码点。** 2026-09-23 本地计量（不含私密原文的证据为 `evidence/context-source-measurements.json`）：

| 人格完整开场文件集合 | Unicode码点 | UTF-8 bytes | o200k_base 本地token估计 |
|---|---:|---:|---:|
| Raya identity + workspace MEMORY + native memory_summary | 42423 | 59139 | 15625 |
| Honey Lemon identity + user MEMORY | 36388 | 41585 | 11040 |

B v1的**应用装配上限**为最终 `prompt` **131072 UTF-8 bytes（128KiB）且32768本地估计tokens**；计数器固定o200k_base及版本，数字是预算估计，不宣称等于上游私有tokenizer。没有字符数8192这道闸。含全部身份/选定memory、状态、会议和协议wrapper一起计数：Raya在两项预算下分别剩71933bytes/17143估计tokens，Honey剩89487bytes/21728估计tokens，供当前状态与会议上下文。模式/状态装配在发起前测量整个最终字符串，不能只分别算块后忽略分隔与wrapper。

0.156.1源码 `build_realtime_session_config` 的8192估计token限制针对V3 initial_items及realtimeStart/EndInstructions，不是本计划使用的V2显式prompt；includeStartupContext=false避免5300估计token的隐式startup截断。官方 [gpt-realtime-2.1 模型页](https://developers.openai.com/api/docs/models/gpt-realtime-2.1)（2026-09-23读取）标128000上下文窗口，但**这不是上游instructions上限或实际接入成功证明**。实现T3/T8仍需用这两份实际规模context开场并验证独有事实/当前事项；provider如拒绝长度就明确返回其原因，不宣称本地预算通过等于上游通过。

超应用上限显式 `context_too_large`（附每块bytes/估计tokens，日志无内容），不静默截断、不自动摘要；由Lead明确调整授权集合后再发起。运行中单次注入上限8192bytes/2048估计tokens，累计静态context仍受总预算；超限拒绝且无部分注入。预算/模型变化使适配能力资格失效，不能沿用上一组合的准入证明。
...
容器宿主进程提供最小 env allowlist，仅系统运行项与**唯一语音 API key**；不继承 bot/Bridge/Lead carrier/comm/ship 凭据，key 不进 argv、文件、日志、纪要。现有 spawn 的 secret washer 会删 KEY，扩成 voice profile 的精确 `OPENAI_API_KEY` 保留通道，其他敏感项仍剥离；不可用 washSecrets=false 全盘绕过。使用 API 认证不回退复制订阅 auth.json。
...
background_agent 允许只读推理，但**不拥有动作路由**。唯一业务出口是模式层调用 Bridge handoffToLead；内置后台不能写 action.json 或持有回执生成工具。`clientManagedHandoffs` 只作协议参数，不作授权证明；收到后台 turn 记录并检查它仍在同一无工具 profile。若出现执行能力/请求或无法验证的后台线程，立即 fence 并终止本场，不能等其干完再撤销。文本声称“已派单”不产生执行回执。
...
| speak(text,kind,opts) | 在已有 ConversationSession 上**新增可选** speak 方法（今天没有该成员），返回 V1 SpeakReceipt；B 实现为必需，未实现的 legacy session 经默认适配返回 rejected/unsupported，V1模式准入关闭，不把旧时延结果冒充proof；所有非 control 经 appendSpeech，序列化单在飞播报。控制类见下段 |
...
| handoffToLead | 模式层调用 Bridge 的可信载体；B 不持有 comm 或 Bridge token。闲聊、问答不投信箱 |
...
TranscriptSink append 增加可等待结果与明确错误。容器宿主先写 JSONL/flush，随后把规范 final utterance 与可信 RoomIO 捕获关联通过带 lease 的 HTTP 上行到 Bridge `/api/voice/sessions/:sessionId/utterances`；模型不能调用这个端点。Bridge 校验当前 lease、会话/Lead/seq 绑定、严格schema、重复 transcriptId 同digest幂等/异digest拒绝，事务写入 `voice_utterances`（sessionId+transcriptId唯一，含原文/归属/捕获摘要/contentDigest/leaseEpoch）并回读 sessionId + transcriptId + contentDigest **三者**后才签发 transcriptDurabilityReceipt。JSONL 为同内容镜像，不是 Bridge 的授权读源；Bridge 不跨进程读取 daemon 文件，daemon 不读取 StateStore DB。handoff 只带 receipt引用，Bridge 自己查此行复核 final/归属，拒绝客户端自签回执。归属来自受信任房间宿主的上行，不声称模型转写自带身份。部分转写供显示不授权；最终 user + known founder + durability receipt 才能进入动作判断。原文本以数据存储，输出 HTML 时 escape，DOM 用 textContent，DB 使用参数化查询。
...
Bridge 授权面（与容器宿主进程通过HTTP通信）在 voice-routes 增加 lease-authenticated handoff 入口，仅接当前 session 所属 Lead，不接受模型指定收件人覆盖。授权在 Bridge 完成：复核 final、founder attribution、逐条持久化、intent 特定原话字段、当前 authorityBinding；敏感 ship/readback 还需 §4.1 arm。payload 严格 schema、长度/枚举/多余字段检查；引擎输出是建议，不是授权。
...
退出不等纪要生成才关进程。容器宿主进程（voice daemon）先 flush/回读最终 transcript，持久化 `(sessionId, transcriptDigest)` 的 minutes job；持久 job 在进程死亡后仍可送本体。含明确“这场未完成/转写有缺口”的状态、上下文快照摘要、事实/决定/未决问题、动作 handoff IDs 与各自状态；不把 pending 说成 done。用同一 Lead mailbox 发一次纪要任务，记录受理与消费证据，本体生成/确认纪要，不重新用语音容器承担写操作。
...
|---|---|---|
| T1 `teamlead/.../CodexLeadProcess.ts`、`codex-lead-runtime.ts`；对应 `lead-backends/codex/__tests__/CodexLeadProcess.test.ts` | RPC error 不当成功；stdin 背压有界；超长/未换行帧封顶；启动中关闭、exit/TERM/KILL、无 orphan；默认 resident 功能不变；真实子进程 env 正负控：仅显式 voice profile保留指定OPENAI_API_KEY，默认/非voice只读与可写路径维持原secret-wash；既有full-access显式例外不扩大，也不通过voice分支新增key | `pnpm --filter flywheel-teamlead exec vitest run src/lead-backends/codex/__tests__/CodexLeadProcess.test.ts src/lead-backends/codex/__tests__/spawn-env-wash.test.ts` |
| T2 新 `bridge/voice-session-context.ts`、`voice-routes.ts`；`src/__tests__/voice-session-context.test.ts` | cosContext缺席的Raya/Opus Honey Lemon真实来源形状与实际规模（原身份17854/17292码点），不可覆盖 identity/memory/ACTIONS、路径逃逸/缺文件/超限/过时/错 lease 拒绝；最终两份 prompt 同 snapshotDigest | `pnpm --filter flywheel-teamlead exec vitest run src/__tests__/voice-session-context.test.ts` |
| T3 新 `voice-codex/src/codex/*`；`src/__tests__/codex-container.test.ts` | 每场不同 thread/home；只允许固定 binary；API key 精确注入、无业务 credentials；快照59秒发起、open再耗59秒不会循环重开；真实context规模可开场且回答使用源事实；shell/file/app/MCP/后台执行负控；metadata 不符拒绝 | `pnpm --filter flywheel-voice-codex exec vitest run src/__tests__/codex-container.test.ts` |
...
| T6 `StateStore.ts`、新 voice-handoff 与 routes；`src/__tests__/voice-handoff.test.ts` | partial/unknown/nonfounder/伪 transcript/错 lease、同 key异digest、伪造durability receipt、Bridge查无utterance、同transcriptId异digest 拒绝；CAS 两恢复者只一人；每个 I/O 前后 crash、ACK丢失、归档查询、无可查询键、24h needs_human；queued 不等于 committed | `pnpm --filter flywheel-teamlead exec vitest run src/__tests__/voice-handoff.test.ts` |
...
本地执行以上受影响用例，以及 `pnpm --filter flywheel-teamlead typecheck`、`pnpm --filter flywheel-voice-core typecheck`、`pnpm --filter flywheel-voice-codex typecheck`、`pnpm --filter flywheel-voice-bridge typecheck` 四包检查；voice-bridge 的共享接口消费者定向命令为 `pnpm --filter flywheel-voice-bridge exec vitest run src/__tests__/qa-fly545-r4.test.ts src/__tests__/wire-meeting-resident.test.ts src/__tests__/wire-meeting.test.ts src/__tests__/qa-fly545-r23.test.ts`；全量由 PR CI。G1/G2 未关闭时不做集成成功声明，T8 不能打勾；未证的 verbatim/attribution/bargeIn 均 false，ship/readback/会议打断等对应格明确关闭。原两场 QA 仍是完成条件，未通过必须报告未完成。
...
有效 verdict CHANGES_REQUESTED；原始结构化结果见 `evidence/design-review-r1.json`。以下接受并修正，无申诉、无用 prose 代替治理 ruling；两项HIGH修正后必须新gate。

| findingKey | 级别 | 处置与证据 |
|---|---|---|
| context-source-coscontext-absent-in-production | HIGH | 接受。§3.1改为cosContext可选覆盖 + 真实启动来源派生；Raya/Honey均在缺cosContext生产形状下有确定文件集合；T2覆盖。§2的上下文服务包含resolver，不再依赖补配置才能开场 |
| context-budget-smaller-than-real-identity-file | HIGH | 接受。两个人格完整开场集合已实量41585/59139bytes、11040/15625估计tokens；移除8192码点闸，改最终prompt128KiB/32768估计tokens且明确上游验收；无截断/摘要替代。证据context-source-measurements.json |
| authorizer-cannot-read-daemon-transcript | MEDIUM | 接受并修正。区分Bridge与voice daemon；utterance先经lease-auth上行，Bridge存储/回读签发receipt，handoff仅携引用且授权面独立回读。T6加伪receipt/缺记录/同id异digest负控 |
| conversationsession-has-no-speak-and-voice-bridge-out-of-scope | MEDIUM | 接受并修正。§4明确新增可选speak与legacy unsupported适配；§2列voice-bridge消费者兼容，§7扩四包typecheck和四个相关用例，不接管G1房间层范围 |
| t1-does-not-run-the-env-wash-sentinel | MEDIUM | 接受并修正。T1命令补spawn-env-wash.test.ts；新增显式voice与默认/非voice正负控，默认保留原secret-wash。既有full-access的受管例外不扩展，不通过voice分支注入额外key |
| snapshot-freshness-equals-open-timeout | LOW | 接受并修正。新鲜度只看发起时刻，过期重取最多一次；T3固定59秒旧快照+59秒成功open不重开 |

三张Mermaid源未因本轮修正改变，与Lead回交SHA仍一致；HTML同步了来源说明并内联已核图形。后续若更改任何图源，先重渲染再发布。
