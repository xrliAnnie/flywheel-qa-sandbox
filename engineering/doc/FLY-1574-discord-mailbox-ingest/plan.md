...
日期: 2026-08-10(R5,按 Codex design review R1-R4 修订)
基于: research.md
...
| 2 | flag 注册(`readonly` toggleability)+ 本仓 reader + 跨仓契约 fixture | flywheel `packages/config` / `flywheel-comm` | registry 仅登记本仓 readsite |
| 3 | Codex Gateway durable-accept strategy + Codex 专属 transition table | flywheel `packages/teamlead` | `CodexDiscordGateway.handle` 内注入 |
...
- **前置硬件:fail-closed per-Lead runtime mutex(R4 BLOCKER-1)**。第 4 格是读检查,其正确性前提「每 Lead 恰一个在拉 Discord 的 runtime」**当前不是代码强制的**:`CodexLeadInboxServer.listen()` bind 前无条件 `unlinkSync` 旧 socket(第二实例可顶掉第一个的 pathname,原监听者仍守着已 unlink 的 inode —— 非 single-listen fence);headless `codex-lead-runtime.ts` 直接 `gateway.start()` 根本不经 inbox socket;FLY-350 的「无 double-post」是当次运维动作(拆 orphan)的结果,不是第二实例必然拒起的不变量。**本单新增**:TUI 与 headless 共用的 per-Lead runtime mutex(stateDir 下 flock 稳定路径:持锁期间绝不 unlink;进程亡锁自动释放 = 崩溃后 stale takeover 天然安全),**任何 REST poll / gateway.start 之前取得,取不到 → 进程在注册 handler/拉 Discord 前退出(fail-closed)**。这同时结构性修复了「今天两个 OFF runtime 也会双注入 turn」的既有隐患;
- 测试矩阵:表 1-5 逐格;旧消息 accepted 后翻 ON + Gateway 重放;`xdept:` submit 后/complete 前崩溃(断言补 ACK);`xdept:` begin 后 journal 前崩溃再 ON(断言经 3 格收敛且行 ACK);**ON commit → cursor 持久化前崩溃 → OFF 重放(断言第 4 格跳过,零双 turn)**;**顺序 OFF 胜 → ON 后到者命中第 2 格 `legacy_codex_accepted`(journal hit,不是 `legacy_external`)**;mutex:TUI-vs-TUI、TUI-vs-headless 第二实例**零 poll 即退**、活 holder 不可被顶(unlink 不破锁)、崩溃后 takeover、双启动竞争恰一胜。

### Phase 1 — flywheel-comm 入队侧

**1a. 渲染器 `renderDiscordChatContent(envelope)`**

- 形态锚 = **真实旧链路捕获的 golden fixture**(实施第一步:测试环境旧路径真发一条,抓 Lead transcript 中 `<channel source="discord" ...>` 实际可见形态;mailbox 版仅允许两处已声明差异:`[receipt:<delivery_id>]` 头 + teammate 信封);
...
- **内容上限(R2 MEDIUM-5)**:mailbox `content` 是 SQLite TEXT,**无既有通用上限,也不发明一个**;输入边界 = Discord 自身已验证约束(正文 ≤4000 字符、附件条目有界)→ **全量保真,零截断**;测试:Discord 最大长度正文逐字往返;
- `from_agent`:`founder`(authorId==FOUNDER_ID)/ `discord:<authorId>`(不凭空造 Lead 映射);
- 测试:golden fixture 对齐;注入(closing-tag/属性/控制字符/附件名);中文/emoji;最大长度。

**1b. `ingestDiscordChat(args)`**:0a 仲裁 + `inserted_inbox` 分支 enqueue(`carrier:'inbox'`, `recipient_kind:'lead'`, `msg_class:'model'`, `type:'discord_chat'`, priority founder=0/其他=1,`created_at`=discord ts)。已核 `QuestionAdmission.revalidate` 对非 question 放行(`question-admission.ts:64-75`)。
...
### Phase 3 — Codex 侧(`CodexDiscordGateway`)
...
- **第 0 步(先于一切 poll):per-Lead runtime mutex**(0c 前置硬件):TUI 与 headless 都在 RestPoll / `gateway.start()` 之前取互斥;取不到 → 结构化日志 + 进程退出(fail-closed,零 poll 零 handler)。mutex 是常驻基础设施(不随清理单删)——它守护的是「单收单发」这个与 flag 无关的通用不变量;**实现钉死为内核级锁(R5 advisory-1)**:Node core 无 `flock`、macOS 无 flock 命令 —— 用 FD-backed 原生机制或 parent-coupled helper 走 `fcntl.flock`(与仓内既有脚本一致);**禁止**用 PID 文件/`mkdir`/lease 库偷换「进程亡即释放」合同;锁贯穿整个 runtime 生命周期(含 generation rebuild),所有 poll/gateway 面停完才放;lock-holder helper 死亡 → runtime fail-stop;

- **两种 flag 状态都先过 0c transition table**(R3 BLOCKER-1):ON 走到第 5 格才同步 `ingestDiscordChat`(better-sqlite3 同步事务,兼容 sync boolean)→ 任一「已有归属」verdict 均不进 `LeadInputRouter`、不注入 turn、不写 saga → 返回 true(cursor 前移);nudge 异步尽力而为(无凭据记 debug,tick 兜底,最坏 +30s 单列 SLO);
...
- OFF:0c 第 1-4 格照跑(过渡保护;第 4 格防翻转双 turn),第 5 格 = 现路径字节等价(哨兵);
- 字段降级显式化:authorName=authorId、attachments=[]、msgKind 按现有 channel 信息;attachment-only 消息今天就被 `passesFilters` 丢弃,保持;
- 测试三层:Gateway(0c 全矩阵、失败 false 重投、cursor 语义)/ RestPoll(回归)/ wiring(TUI+headless 都接 strategy)。
...
| # | 场景 | 判据 |
| -- | -- | -- |
| Q1 | ON:test founder 发 1 条 | mailbox `carrier='inbox' type='discord_chat'` 一行;Lead 眼前形态与 golden fixture 一致(含 `[receipt:]`);reply 正常 |
| Q2 | ON:60s 内连发 3 条 | 3 行独立入账、各恰一次;D 未合:如实记录批次形态(per-message nudge 下大概率独立批,不承诺同 tick 合批);D 已合:一次收 3 条 |
| Q3 | ON:跨 Lead(#test-leads-roundtable) | roundtable 走 mailbox,thread 路由不变;Codex slot 同验(含 0c 表 1/2 格) |
| Q4 | 零丢失(03:57 重放):在途杀 Bridge / Lead 不在 | 行 `QUEUED` **或已 durable-accept**(Lead 不在时 adapter 写 inbox.json 并 ACK = 合法送达);恢复后恰好一次、内容完整 |
...
| Q9 | Codex transition 全矩阵 + mutex | 0c 表 1-5 格:journal 已 accept 翻 ON 重放不双 turn;`xdept:` submit 后崩溃 → 补 `saga.complete`,**断言行终态 ACK**;begin 后崩溃 → fenced recovery 收敛且 ACK;**ON commit → cursor 前崩溃 → OFF 重放被第 4 格拦下,零双 turn**;**mutex 真机:起第二个 runtime(TUI-vs-TUI 与 TUI-vs-headless)→ 零 poll 即退;kill holder → 后继者接锁恢复** |
...
- Codex:`ExternalReceiptSaga` + Gateway 旧直推分支 + 0c 全部 legacy transition(第 1-4 格:journal 补 ACK / journal skip / fenced xdept recovery / chat: 所有权读检查 —— 旧道死后整表退化为纯 0a 仲裁);**runtime mutex 不删**(常驻不变量);
- lane 仲裁的 `legacy_external`/`inserted_external` 分支;
- flag + registry 条目 + 契约 fixture + 本清单。

## 5. 边界(不做)

- ❌ Discord 出站 / 最后一公里 / D 单三能力 / 删旧机制 / permission·pairing·typing·reaction·roundtable 路由 / founder 批准链;
...
3. **capability census**:每个 Lead/shard 探 `chat-ingest --version-probe` + 插件启动日志 + **每个在跑的 Codex Lead runtime 提供 mutex 证据(mutex-aware 且当前持有本 Lead 锁;R5 advisory-2 —— 防旧 Codex 二进制在混版本窗口废掉单实例前提)**,全绿才继续(防旧实例裂脑);
...
4. mailbox 版可见形态与直推版两处已声明差异(`[receipt:]` 头 + teammate 信封);
5. dotenv_live 多进程翻转短暂裂脑窗(§6.1 census 管理);
6. 归档行 lane 不可判时 settle 记 `ignored_unknown_archived`(telemetry 缺一笔,不写错账);
7. Codex 0c 第 4 格为读检查,其正确性由**本单新增的 fail-closed per-Lead runtime mutex** 保证(取不到锁的实例零 poll 即退)——不再依赖运维纪律背书;
