...
f.rows[0].value = '想看一次性消费边界'; f.rows[0].events.input();
assert.equal(f.saved.get('flywheel:design-comments:/test/one:summary'), '想看一次性消费边界');
assert(f.chunks.children[0].children[0].textContent.includes('01 / 一句话\n想看一次性消费边界'));
let reload = fixture({saved:f.saved}); assert.equal(reload.rows[0].value, '想看一次性消费边界');
...
hostile.rows[0].value = '</textarea><img src=x onerror=alert(1)> & \"';
hostile.rows[0].events.input();
assert(hostile.chunks.children[0].children[0].textContent.includes(hostile.rows[0].value));
assert.equal(hostile.chunks.children[0].children.length, 2);
console.log('PASS: artifact shape, all section comments, one nonce script, no external assets/tables/inline handlers, pathname storage, live summary, 1800-character Unicode chunks, clipboard success/rejection/absence/failure, storage denied, clear/reload isolation.');
