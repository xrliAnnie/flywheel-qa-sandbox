...
- **INV-10(检测通知 Lead-only 路由,founder 直令 e5b83559)**:运维检测类通知(session_stuck / session_monitoring_lost / session_monitoring_reestablished / **session_zombie_detected** / delivery_unconsumed 一族)**只路由 Lead 处理队列**(lead_events → Lead runtime),绝不直接进 founder 可见的 issue thread、绝不 @ founder;founder 只经既有 Lead-first 升级链(FLY-1048/1279:Lead 超时未处理/处理失败的 ~30min grace)可达。本单新增/修改的每条事件都不得新增任何直达 founder 面的发射;M4 验收:founder 面向消息里 0 条原始检测事件(真机验收随附)。
...
| M4 | 注册面:GUARDRAIL 含新事件;EventFilter high;payload 字段(liveness_probe/unpushed_work/文件路径进 context);monitoring_lost 诚实文案(三参)+ 两参字节不变;reestablished 证据字段 + 点时文案;**文案反断言:indeterminate 各来源(CommDB error / pane timeout / quarantine)的 advisory 不得含 alive / working / no action needed**(R1 #1);**Lead-only 路由(INV-10)**:zombie/reestablished/monitoring_lost 的投递面只落 lead_events(断言零 founder-thread-notifier 调用、零 founder @);founder 升级只能来自既有 Lead-first 链 | 同上 + notifier/EventFilter 既有测试处 |
...
6. **Lead-only 路由(INV-10,founder 直令)**:真机重演全程 founder 可见 thread 内 0 条原始检测事件(zombie/reestablished/stuck/unconsumed);Lead 队列收到并可处理;founder 升级仅在 Lead 超时链路上。
...
- **StateStore 已迁 better-sqlite3(FLY-663)**:每条 statement autocommit,`save()` 是 no-op;多语句原子性走 CompatDb 的 `transaction`(**私有**,`StateStore.ts:139-143`)——新 store 方法在**内部**用 `this.db.transaction(...)`,不暴露公共事务 API(R17 #1 + R18 #4)。
...
- **Lead 处置面是三个,不是两个**(R18 #1):`POST /:executionId/detection-ack`(ack|resolve|dismiss)、`POST /:executionId/stuck-disposition`(老路由;显式 disposition 全集 = `false_positive|legitimate_wait|snooze|needs_founder`;execution-scoped latch 一次 ack 该 execution 的 **N 个** active case-c episode)、以及 `POST /:executionId/recovery-nudge`——成功 nudge 隐式写 `handled_remanaged`(`runner-recovery-nudge.ts`),Lead HTTP 路由以 `actor:'lead'` 调用,这正是「已接手并采取动作」的处置。**老路由现状**:先独立提交 `setStuckDisposition`,C4a mirror 在 try/catch 里吞错照常 200(R17 #1)——Part D 必须消除这个「处置成功但零 ack/零回执」的假成功。
...
- **周期位点**:`runDetectionReconcileTick` 由 GatePoller 每 ~20 tick **fire-and-forget** 触发(慢 pass 可与下次重叠;且 `FLYWHEEL_DETECTION_RECONCILE_EVERY_N_TICKS=0` 会把它整个关掉——回执消费者若挂在里面就受一个未声明的隐藏开关支配,R18 #2)→ 回执消费者挂**自己的** GatePoller 小 stage(D2.3),自带进程内单飞闩。
...
1. **新表 `disposition_receipts`(幂等 CREATE TABLE IF NOT EXISTS)**:
   `receipt_id INTEGER PRIMARY KEY AUTOINCREMENT`(**不可复用身份**,R17 #2)、`target_key/kind/episode_fingerprint`(episode 键)、`episode_first_detected_at_ms`(**generation 锚,事务内从 fresh escalation 行复制**——不用 wall-clock 推断,R18 #3)、`actor_lead_id`(来自请求 leadId,绝不依赖可空 owner_lead_id)、`disposition`(原样存:ack|resolve|dismiss|false_positive|legitimate_wait|snooze|needs_founder|handled_remanaged)、`content`(**prepare 时构好的最终文案**——backfill 零重建)、`issue_id`、`state`(`pending|posted|unroutable|expired`)、`attempts`、`last_attempt_at_ms`、`created_at_ms`(只负责 7 天窗口)、`posted_at_ms`;必需字段(episode 键/anchor/actor/disposition/content/state/attempts/created_at)全部 **NOT NULL**,state/disposition 加 **CHECK**;**UNIQUE(target_key, kind, episode_fingerprint, episode_first_detected_at_ms)**——本代 dedup 由数据库强制,时钟回拨/重复请求都插不出第二行;revive 重置 anchor → 新代自然允许新行。**写法必须是精确的 `INSERT ... ON CONFLICT(四列) DO NOTHING`,绝不用 `INSERT OR IGNORE`**(R19 #1:OR IGNORE 会连 NOT NULL/CHECK 失败一起静默吞掉——ack 提交而 receipt 永久缺失;只有 generation 冲突允许 no-op,其他约束错误必须抛出令整组事务回滚)。INSERT 后检查 `changes`:真插入才写 issue-id-empty 审计;冲突则读回同代行核验。
2. **prepare 腿(三条路由,零投递,真事务)**:
   - 新 store 方法 `ackDetectionEscalationWithReceipt(...)`(内部 `this.db.transaction(...)`,R18 #4):ack UPDATE → 检查 changed → changed 且 via:'lead' 时按 D2.1 的 ON-CONFLICT 写法插回执行(anchor = 事务内 fresh 读的 `first_detected_at_ms`;issue_id 空 → 直接 `state='unroutable'` 行 + `session_events` 审计行,同事务、仅真插入时写);异常整组回滚(R17 #1)。**每-episode-代恰一行由 UNIQUE 强制**(R18 #3)——旧代 pending 行不挡新代,revive **不删不改** outbox(R17 #2)。
   ...
   - **recovery-nudge 路由(R18 #1,第三处置面)**:仅当 `actor === 'lead'` **且 sendKeys 已确认成功**时,用 `applyStuckDispositionWithReceipts` 持久化隐式 `handled_remanaged` + 对应 case-c episode 的 ack/outbox;auto-repair-bot 路径零 Lead 回执(机器动作不是 Lead 处置)。**诚实的非原子边界**:外部 keystroke 无法随 DB 事务回滚——send 成功后 prepare 事务失败 → 路由仍返回 `nudged:true` 但带显式 `dispositionPersisted:false` + loud log(Lead 可重 nudge 或手动 ack 补账),不假称整条 action 原子。
...
3. **delivery 腿(独立 GatePoller stage 单消费者;R17 #3 + R18 #2)**:**不挂在** `runDetectionReconcileTick` 里(它受 `FLYWHEEL_DETECTION_RECONCILE_EVERY_N_TICKS=0` 支配——回执消费者不能被一个未声明的开关静默停摆);GatePoller 新增独立 piggyback `onDispositionReceiptTick`(默认每 20 tick,零新 timer),`FLYWHEEL_DISPOSITION_RECEIPT` 是**唯一** delivery gate。step 自身 own try/catch + **模块级进程内单飞闩,`finally` 释放**;重叠 tick 跳过:
   ...
   - 每行:`created_at_ms` 早于 7 天 → `state='expired'` + loud log;否则解析 thread:**读完整 chat_threads 行**(thread_id + `lead_id`),token 传 **thread 的 `lead_id`**(thread 创建时的 owner Lead)给 `resolveBotTokenForThread`——**不是** receipt 的 actor_lead_id(issue 转派/actor≠owner 时 actor bot 可能不在 private thread 里 403 到 expired,R19 #2);真实回退链 = **thread 创建时 Lead → 当前 label-resolved Lead → global**(helper 现有合同,照实写)。actor lead/thread 不可解析 → **按一次失败计**:stamp attempts/last_attempt 轮转到队尾,直到 7 天 expired——不许零 stamp 霸占 NULLS-FIRST 前五名(R17 #4)→ 自带投递函数 `postThreadMessage(botToken, threadId, content, {fetchImpl, timeoutMs≈15s})`(**AbortController 真取消**,不是裸 Promise.race——迟到 POST 与重试并发会扩大重复窗口,R17 #3;body 带 `allowed_mentions:{parse:[]}` 传输层防线,不只靠 formatter strip;botToken 解析见上——thread owner Lead 优先、三段回退,不许 global bot 误发进它不是成员的 private thread,R18 #4 + R19 #2):confirmed 2xx 后 `UPDATE ... SET state='posted', posted_at_ms=? WHERE receipt_id=? AND state='pending'`(**receipt_id 定位 + state 守卫**,ABA 结构性关闭,R17 #2);throw/timeout → 同守卫只 stamp attempts/last_attempt,继续后项。
...
5. **revive 零接触**(R17 #2 反转 R16 版的 DELETE):revive 只重置 detection_escalations 自身;旧代 pending 回执**继续投递或自然 expired**(它记录的是真实发生过的处置);新代靠 `episode_first_detected_at_ms` 代锚(UNIQUE)挣得自己的新行。
...
- prepare 面:detection-ack ack/resolve/dismiss → outbox 恰一行(content 即最终文案;无 @;note 截断+去 mention;无 pane 文本);同 episode 同代第二次处置 → 零新行(**含 wall-clock 回拨 fixture:回拨后重复请求仍被 UNIQUE 挡住**,R18 #3);RESOLVED 后迟到 ack → 零行;**事务 all-or-nothing**:fault-inject 在 ack UPDATE 与 receipt INSERT 之间 throw → 重开 DB 验证两者都不存在(R17 #1);**非 UNIQUE 约束失败(如 content NULL)→ 抛出、重开 DB 后 ack/stuck/receipt/audit 全部回滚;同代 UNIQUE 冲突 → 幂等成功不抛**(R19 #1 两类反例);老路由 `applyStuckDispositionWithReceipts` 同注入 → stuck 写与 ack/receipt 全回滚 + 路由 5xx(假成功路径死亡);**C4a 2 个 active episode → 2 行独立 outbox**(重复请求零新增);**recovery-nudge(R18 #1)**:Lead actor + send 确认成功 → 恰一条 handled_remanaged 回执;auto-repair-bot nudge → 零;send 失败 → 零;send 成功 + prepare fault → `nudged:true, dispositionPersisted:false` + loud(显式恢复合同);issue_id 空 → 同事务 unroutable + session_events 审计;disposition 人话全集(含 snooze 带 clamp 期限 / needs_founder)逐一 golden(R17 #4);
...
- 开关矩阵:`FLYWHEEL_DISPOSITION_RECEIPT=0` → prepare 照写、零投递;重开 → 窗口内补投、窗口外 expired;**`FLYWHEEL_DETECTION_RECONCILE_EVERY_N_TICKS=0` 不影响回执投递**(消费者是独立 stage,R18 #2);
- sender 合同(R18 #4 + R19 #2):`postThreadMessage` body 含 `allowed_mentions:{parse:[]}`;token 用 **thread 行的 lead_id**(非 actor)传 `resolveBotTokenForThread`——fixture ①actor 与 thread owner 不同且两者都有 token → 必用 owner token;②owner token 缺失但 label-resolved Lead token 在 → 走中段回退,不提前落 global;
