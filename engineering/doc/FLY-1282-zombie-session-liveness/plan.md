...
3. **delivery 腿(独立 GatePoller stage 单消费者;R17 #3 + R18 #2)**:**不挂在** `runDetectionReconcileTick` 里(它受 `FLYWHEEL_DETECTION_RECONCILE_EVERY_N_TICKS=0` 支配——回执消费者不能被一个未声明的开关静默停摆);GatePoller 新增独立 piggyback `onDispositionReceiptTick`(默认每 20 tick,零新 timer),`FLYWHEEL_DISPOSITION_RECEIPT` 是**唯一** delivery gate。step 自身 own try/catch + **模块级进程内单飞闩,`finally` 释放**;重叠 tick 跳过:
   ...
   - 每行:`created_at_ms` 早于 7 天 → `state='expired'` + loud log;否则解析 thread(`resolveChatThreadId(store, issue_id, actor_lead 的 chatChannel)`,actor lead 在 projects 里不可解析/thread 未绑 → **按一次失败计**:stamp attempts/last_attempt 轮转到队尾,直到 7 天 expired——不许零 stamp 霸占 NULLS-FIRST 前五名,R17 #4)→ 自带投递函数 `postThreadMessage(botToken, threadId, content, {fetchImpl, timeoutMs≈15s})`(**AbortController 真取消**,不是裸 Promise.race——迟到 POST 与重试并发会扩大重复窗口,R17 #3;body 带 `allowed_mentions:{parse:[]}` 传输层防线,不只靠 formatter strip;botToken = `resolveBotTokenForThread` **优先该 issue thread 的 owner-Lead bot**、缺失才 global fallback——不许 global bot 误发进它不是成员的 private thread,R18 #4):confirmed 2xx 后 `UPDATE ... SET state='posted', posted_at_ms=? WHERE receipt_id=? AND state='pending'`(**receipt_id 定位 + state 守卫**,ABA 结构性关闭,R17 #2);throw/timeout → 同守卫只 stamp attempts/last_attempt,继续后项。
...
- sender 合同(R18 #4):`postThreadMessage` body 含 `allowed_mentions:{parse:[]}`;token 解析优先 owner-Lead bot、缺失才 global fallback(两侧 fixture);
- 迁移:旧库(无表)重开 → CREATE TABLE 幂等零 throw;
- INV-10 回归:回执文案反断言不含 pane 文本;founder 面原始检测事件仍为 0。

**验收补充(真机)**:重演脚本追加一步——zombie 告警进 Lead 队列后,以 Lead 身份调 detection-ack → 下一个 reconcile pass(≈1 分钟)内 issue thread 肉眼可见回执;founder 面原始检测事件仍为 0。
