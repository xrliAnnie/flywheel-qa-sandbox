...
> Lead 要求写进 plan,因为它比这一单通用。

这一单从头到尾都在治同一个病,而它在**三个不同层面**长出了同一种形状:

| 形态 | 事实 |
|---|---|
| **落盘 ≠ 投递** | FLY-742 的守卫 15 天开火 16 次,**16 次全部**卡在 `messages` 到 `lead_inbox` 之间,`read_at` 非空 0 条。它的验收通过了,因为验的是「守卫会不会喊」。 |
| **进程活着 ≠ 在干那件事** | Lead 连续几轮看到 Runner「在跑」,就当它在做交代的事;去 `ps` 查才发现根本没有 `gh auth` 进程。 |
| **CI 绿 ≠ 护栏有效** | 本单的变异第一次追加在 `converge-flywheel-bin.sh` 的**末尾**,而那里在 `exit "$rc"` 之后 —— 解析器永远走不到,于是三个套件「全绿」,什么也没证明。 |

### 两个方向的误读都致命,而且同一晚各出现一次

- **把「我的仪器坏了」读成「世界没事」**:Lead 的巡检查询日期过滤用 `'T'` 格式,而 `created_at`
  是空格分隔,ASCII 里空格 < `'T'` ⇒ 当天所有行被静默过滤 ⇒ 连报数轮「零欠答」,实际积压 28 条。
- **把「我的仪器坏了」读成「世界坏了」**:本单的无效变异让护栏看起来是摆设,差一点被报成缺陷。

**同一个根:没有先验证仪器本身。**

### 判据

**在相信任何读数之前,先证明这个量具在当下这次测量里确实会动。**

不是「读数是好是坏」——绿和红在仪器未经验证时**同样不可信**。

### 本单里这条判据的具体形态(可复用)

| 手法 | 它证明了什么 |
|---|---|
| **D0 阳性对照** | 收件方存在、起始为空、**且对一个已知探针确实会记录** |
| **A1 阴性对照** | 事故形态下读数确实为零 —— 修复前后都必须绿 |
| **D4a/b/c 反向自检** | 拿断言自己去打空收件方、只有 receipt、只有字节,**三种都必须红** |
| **可达位置的变异** | 护栏真的会红;不可达的变异等于没做 |
| **`VALID=1` 单一裁决点** | 各用例不再用自己那套宽松 grep 重新推导有效性 —— 用同一种错误方式重复检查同一件事,正是第一版能保持绿的原因 |

### 一句话

> 一个永远不会红的护栏测试等于没有;而一个未经验证的量具,连它报的红都不算数。
