...
## 真实数据一次运行（只读）
- 观测 2026-09-26T09:09:38Z（代码评审 R1 修复后重跑）：FLY-2072 直接子单 221 张、3 页完整分页（含 archived）；12 张非类别管理单。
...
- **FLY-2373 当前有 active run `110e738b-87ff-4332-91a4-a29be96018be`，按验收条件「直到它有 run」被排除**，
  ...
  由单测 `patrol-root-causes.test.ts` 在去敏的同批真实数据上证明。FLY-2891、FLY-2906 同样因 active run 排除。
...
- 代码评审 R1（codex:rescue，2 HIGH + 2 MEDIUM，均已修复并补回归测试）：
  ...
  4. 移到其它 Linear 项目的子单以 `ROOT_CAUSE_EXCLUDED reason=project_scope_mismatch` 可见排除
     （project 为空的子单是常态，照常处理）；发送身份复核同时校验 team=FLY 与 project。
