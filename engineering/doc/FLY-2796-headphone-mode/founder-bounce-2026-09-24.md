...
3. **不悄悄丢句**：人数来源是新增的 `room-headcount.ts`（`ChannelHeadcount`）。
   - 它在 cli.ts 组装根包装传给房间的 deps，RoomIO 本身不改，监听频道里所有真人的进出。
   - 为什么不用 RoomIO 的 `humanCount`：它只在 founder 自己进出时刷新，别人加入后会过期。这一点由 Codex 复审 R1 指出，级别 HIGH。
   - 任何真人进出时，同步把人数置为「未知」，再异步读回新值。读失败、或读回的是被新事件取代的旧值，一律保持未知。
   - 单人房归给她要同时满足四条：她在房里；实时人数恰好为 1；这段话里所有有声帧都属于她；至少有一帧有声。满足时这句归给她，证据记 `attribution: "sole_human"`。
