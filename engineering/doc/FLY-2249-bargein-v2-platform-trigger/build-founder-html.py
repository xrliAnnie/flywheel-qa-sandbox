...
Fails loudly (exit 1) if any placeholder is left, any SVG is missing/empty, or any
diagram lost its <svg id="FLY-2249-dN"> root — the FLY-2238 lesson: never ship a
visual artifact you did not check."""
...
for n, name in diagrams.items():
    ...
    if f'id="FLY-2249-d{n}"' not in svg:
        sys.exit(f"diagram {name} lacks svgId FLY-2249-d{n}")
...
leftover = re.findall(r"<!--DIAGRAM_\d+-->|<!--REVIEW_LOG-->|<!--R[34]_[A-Z]+-->", out)
if leftover:
...
print(f"wrote {target} ({size} bytes); svg roots: {out.count('<svg')}; sections: {out.count('data-title=')}")
if size > 512 * 1024:
