...
- A2 隔离:CODEX_HOME 在 `${SLOT_DIR}/cdxh/<agent>`,state 在 `${SLOT_DIR}/q/<slot>/state/codex-lead/<key>`;TUI 窗口只在 slot tmux server;默认 server `flywheel` 窗口清单前后 `cmp` 相同;**可执行边界**:生产 `~/.flywheel/state/codex-lead/` 与三个生产 Codex home **零写入**(前后对照「**元数据清单 hash**」:相对路径、lstat 类型、size、mode、inode/nlink、mtime、symlink 目标 —— 允许的操作**精确为**目录枚举 + 每个条目 `lstat` + 仅对 symlink `readlink`,**禁止 open / read 任何常规文件**),不读取生产 home 的凭据 / 会话 / 内容(拒绝名单的 realpath/lstat 元数据查询允许),resolver **不回落**生产 `~/.flywheel/projects.json`(用活进程 `FLYWHEEL_PROJECTS_FILE` 实际值 + T10 去键阳性对照证明;atime 不作证据)。
