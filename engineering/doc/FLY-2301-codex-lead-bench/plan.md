...
| ⑦(f1683cd3)心跳读取器的「快照输出路径」只许演练 / 测试调用方传,生产调用路径(verify / ready)不传、行为逐字节不变并有守卫,不是开关;INT/TERM 用例用 Python 进程组驱动只改测试驱动,Bash 3.2 后台子 shell 忽略 SIGINT 记为边界 | T4、T5 |

### 0.2 Codex 三轮处置摘要
R1(13)与 R2(10)全部纳入(见 git 历史 rev2 / rev3 的 §0.2)。**R3(9,全部纳入,两处收窄)**:H1 生产改动回到**一行**(T7);H2 Codex `.env` 漏了固定基础赋值(含 `FLYWHEEL_PROJECTS_FILE`)→ 先建 `base_assignments` 向量再双投影,并断言进程只解析 slot projects 文件(T9/T10);H3 身份拒绝集扩为 canonical resolver 所有/断言/清理的全部变量(T9);H4 真房比对对全局 manifest / stdout 走命名叶子规范化(A7/T12),**收窄**:不做日志全文基线,只对既有 Claude 日志两行做逐字基数断言 + 一处变异(T1);H5 纯 Claude registry 走**逐字节相同的旧循环**,新收敛路径只在含 Codex 条目时启用(T6);M6 生产者接缝 = 纯搬运的 `scripts/lib/qa-lead-artifacts.sh`(T1);M7 形状元组校验器同时服务主 slot 与 extra(T8);M8 铸造事务子 shell 化、父目录与顺序写死(T5);M9 输出 / registry / 演练三方合同冻结(T9/T12)。**R4(6,全部纳入)**:H1 `codexLead` 补 `projectName/agentId`,T12 校验与交叉核对(T9/T12);H2 Codex 拆房把 slot tmux server 退役改为收敛式,Claude 段逐字节不动(T6);M3 铸造强制失败变异改为 `mv`→`false`,INT/TERM 以 130/143 退出(T5);M4 日志两行的格式化搬进产物库供 fixture 捕获(T1);M5 A2 改为可执行边界(零写入 + 不读内容 + resolver 不回落),atime 不作证据(A2/T12);M6 心跳读取器加 `state` 枚举,每次判定一份快照(T4)。**R5(5,全部纳入)**:H1 演练在任何信号前把 manifest 每个坐标与 CLI slot **精确绑定**(不只文法/包含)(T12);H2 生产 home 快照改为**只取元数据**的清单 hash,不打开任何常规文件,并有「不可读文件 fixture」守卫(A2/T12);M3 INT/TERM 测试用镜像变异把 `mv` 行换成栅栏实现确定性同步点(T5);M4 演练证据 old/new 各带心跳 sha256/state/pid,两份字节快照归档(T6/T12);L5 `qa_multilead_launch_manifest` 原地不动,只搬上层组装(T1)。**R6(3,全部纳入)**:H1 心跳读取器加快照输出路径(单次读入字节校验后原样落盘),drill 接收房外证据暂存目录,突变前用同一份旧快照做完整 A3 前置,old/new 各带谓词与 hash,整包原子 rename(T4/T6/T12);M2 INT/TERM 用例改由 Python 进程组驱动(Bash 3.2 后台 `&` 子 shell 忽略 SIGINT)(T5);M3 元数据快照允许操作精确定义为「枚举 + lstat + 仅 symlink readlink」,落为 `scripts/lib/qa-metadata-snapshot.py` + 专属测试 + 读文件变异必红(A2/T11/T12)。**R7(2,全部纳入;无 HIGH,按 Lead 收口规则由 Lead leadAcceptance 收口,不跑 R8)**:M1 INT/TERM 只发给被 trap 的内层子 shell PID(栅栏经 `/bin/sh -c 'printf "%s\n" "$PPID"'` 发布其 PID),`killpg` 仅用于超时清理(T5);L2 元数据快照定义版本化的规范序列化(排序、编码、纳秒 mtime、固定分隔符)并加乱序创建 fixture(A2/T12)。
...
- A2 隔离:CODEX_HOME 在 `${SLOT_DIR}/cdxh/<agent>`,state 在 `${SLOT_DIR}/q/<slot>/state/codex-lead/<key>`;TUI 窗口只在 slot tmux server;默认 server `flywheel` 窗口清单前后 `cmp` 相同;**可执行边界**:生产 `~/.flywheel/state/codex-lead/` 与三个生产 Codex home **零写入**(前后对照「**元数据清单 hash**」:相对路径、lstat 类型、size、mode、inode/nlink、mtime、symlink 目标 —— 允许的操作**精确为**目录枚举 + 每个条目 `lstat` + 仅对 symlink `readlink`,**禁止 open / read 任何常规文件**;记录格式版本化且规范:不跟随 symlink、含根条目、相对路径与 readlink 结果按字节无损编码、mode/size/inode/nlink 整数、mtime 纳秒、按编码后相对路径字节排序、固定分隔符、末尾换行后 SHA-256;fixture:目录条目以不同顺序创建的两棵等价树,两次快照字节与 hash 均相同),不读取生产 home 的凭据 / 会话 / 内容(拒绝名单的 realpath/lstat 元数据查询允许),resolver **不回落**生产 `~/.flywheel/projects.json`(用活进程 `FLYWHEEL_PROJECTS_FILE` 实际值 + T10 去键阳性对照证明;atime 不作证据)。
...
| `scripts/qa-fly-2301-codex-lead-drill.sh` | 新增 | `<slot> <crash|kickstart> <evidence-dir>`;预校验房外证据暂存目录,调用 drill 后原子 rename 成证据包 |
| `scripts/lib/qa-metadata-snapshot.py`、`scripts/__tests__/qa-metadata-snapshot.test.sh` | 新增 | 生产 home / state 的元数据清单 hash(枚举 + lstat + symlink readlink,禁 open)与其守卫(mode 000 条目在清单中;open 变异必红) |
...
2. **纯搬运**:把 `qa_slot_launch_env_json`、per-Lead manifest jq、`.env` 写者、launch-manifest 的**上层组装**(调用既有 `qa_multilead_launch_manifest` 再做追加 jq;`qa_multilead_launch_manifest` 本身**原地留在 `qa-multilead.sh`,签名与字节不变**,`test-deploy-multilead.test.sh` 对它的直接守卫不改)、stdout JSON heredoc 提炼进 `scripts/lib/qa-lead-artifacts.sh`(函数签名 = 现有实参),`test-deploy.sh` source 后调用;搬运后五面基线与两行日志必须仍绿(这就是零漂移证明)。
...
- `qa_launchd_read_heartbeat path [snapshot_out]`:lstat 拒 symlink、≤64KB、object、`v==1`、`processPid` 正整数、三个 id 非空 ≤256、`updatedAt` 非空 ≤64、`state` ∈ `running|online|generation_lost|shutdown`(`resident-codex-lead-lifecycle.ts` 生产枚举)→ TSV `pid gen carrier state sha256`(hash 取自同一次读入的字节);给了 `snapshot_out`(必须是已存在、非 symlink、房外的目录)时,把**同一份读入的字节**原样写到 `snapshot_out/heartbeat-<sha256>.json`(temp + rename,600),**不做第二次读**;**第二参只由 `qa_launchd_lead_restart_drill` 与测试传入**:`qa_launchd_codex_lead_verify` / `qa_launchd_codex_lead_ready`(即 `test-deploy.sh` 的生产调用路径)一律单参调用,单参时读取器不写任何文件、输出与两参时的 TSV 逐字节相同(守卫:`fly1663-qa-launchd.test.sh` 对同一 fixture 比对单参 / 两参 TSV 相同且单参后目录无新文件;并以 `grep -c` 基数断言 `qa-launchd-lead.sh` 中 verify / ready 两个调用点的 `qa_launchd_read_heartbeat` 都是单参);这不是开关,是演练专用的证据出口;verify / ready / drill **每次判定只读一次、只用这一份快照**;负例:缺 state、非法 state、状态转换 fixture、`snapshot_out` 为 symlink / 在房内 → 拒绝。
...
- 负例:dest 已存在、symlink 祖先逃逸、`current` 指向 releases 外、auth.json 为 symlink、socket 过长、**镜像变异把 `mv "$stage" "$dest"` 精确替换为 `false # forced post-auth/pre-rename failure`**(替换计数恰 1)→ 非零、无 dest、无 stage、父目录下无 sentinel 凭据字节;INT 与 TERM 用例的**确定性同步点与信号投递**:镜像变异把同一 `mv "$stage" "$dest"` 行(计数恰 1)替换为 in-shell 栅栏 `touch "$(dirname "$dest")/.fly2301-barrier"; while :; do sleep 0.2; done`;**驱动器是一个小 Python 进程**(不是 Bash `&` 后台作业 —— Bash 3.2 非作业控制下用 `&` 起的子 shell 继承 SIGINT=ignore,INT 用例会挂;此边界记入 playbook「测试驱动注意事项」;Python 只是测试驱动,签入实现不变):Python 在新会话 / 进程组里前台启动 `/bin/bash`(或 CI bash),该 bash 加载镜像库并正常调用铸造函数;栅栏在写标记的同时发布**被 trap 的内层子 shell 的真实 PID**(Bash 3.2 无 `BASHPID`,用 `/bin/sh -c 'printf "%s\n" "$PPID"'` 取得并写进标记文件);Python 轮询到栅栏标记与 stage 内 sentinel 凭据都出现后,校验该 PID 是 harness 的存活后代,**只向该 PID** 发 SIGINT / SIGTERM(外层未 trap 的 bash 若一起收到 SIGTERM 会以 -15 终止,拿不到 143 —— macOS bash 3.2.57 实测),有界等待(30s)→ 断言外层 harness 退出码恰 130/143、进程组无幸存成员、无 dest、无 stage、无 sentinel 字节;`killpg` 只用于超时 / 最终清理;签入实现不含任何钩子 / 开关;成功与全部失败路径在 `/bin/bash`(3.2)与 CI bash 各跑一遍。
...
- `qa_launchd_lead_restart_drill label carrier mode(crash|kickstart) codex_home state_dir tmux_socket project lead evidence_stage`(Codex 形状全部必填;调用前逐个校验:label 匹配 `com.flywheel.qa.lead.slot-<n>.<agent>`,`codex_home`/`state_dir` 为 slot 内绝对路径,`tmux_socket` == `${SLOT_DIR}/tmux-$(id -u)/default` 且为 socket,`evidence_stage` 为调用方预校验的房外空目录;任一缺失/畸形 → 在任何信号 / `launchctl` 之前返回 1):**旧侧完整 A3 前置**:用一份心跳快照(经 `snapshot_out=evidence_stage` 落盘)与同一轮的 pid_exact / process_matches / env_has / heartbeat pid 与 `state==online` / 恰一窗全部通过才允许突变(任一不过 → 返回 1,不动任何进程);记旧 pid/lstart/两 id/心跳 sha256;`crash` → `kill -9`;`kickstart` → `launchctl kickstart -k <domain>/<label>`;收敛后对**新快照**(同样 `snapshot_out=evidence_stage` 落盘)重复同一组谓词;断言 pid 或 lstart 变化,codex 两 id 均变;证据 JSON `{mode,label,domain,old:{pid,lstart,generationId,carrierInstanceId,state,heartbeatSha256,predicates:{…}},new:{同上},heartbeatPath,tmuxSocket,startedAt,convergedAt}`(两个 sha256 分别来自各自判定时解析并落盘的那份字节;写 JSON 前校验 `evidence_stage/heartbeat-<sha>.json` 的内容 hash == 文件名 hash),整包写完后由调用方原子 rename 到最终证据目录;无 env / 凭据;用例:两次读之间心跳被替换 → 归档字节 hash 必须等于记录字段对应的 hash,而非事后重读;fixture 变异:改动其中一份快照只改变对应的那个 hash。
...
### T11 — CI / 清单 / 文档
`ci.yml:352` 步骤追加 `qa-lead-artifact-fixtures.test.sh`、`fly1663-qa-launchd-mutants.test.sh`、`codex-lead-state-root.test.sh`、`qa-codex-lead-layers.test.sh`、`qa-codex-tmux-isolation.test.sh`、`qa-metadata-snapshot.test.sh`;`path-hygiene.sh` 与 s0-scope 清单加模板;playbook:slot 字段、`codexSourceHome` 前置(`CODEX_HOME=<home> codex login`)、roundtable 不支持、演练命令与外部证据目录、双 server 清单对照法。

### T12 — 真机验收
- `scripts/qa-fly-2301-codex-lead-drill.sh <slot> <crash|kickstart> <evidence-dir>`:`evidence-dir` 必填且在 SLOT_DIR 外;读 `${SLOT_DIR}/launch-manifest.json`(常规非 symlink 文件、≤64KB),要求 `leadCarrier == launchd-codex-tui` 且 `codexLead.{label,projectName,agentId,stateDir,codexHome,tmuxSocket,tuiWindow}` 齐全,并把每个坐标与 **CLI `<slot>` 推导的期望值精确相等绑定**:`projectName == test-slot-${slot}`;`agentId` ~ `^[a-z0-9][a-z0-9-]*$`;`mainLeadLabel == codexLead.label == $(qa_launchd_label "$slot" "$agentId")`;`codexHome == ${SLOT_DIR}/cdxh/${agentId}`;`stateDir == $(qa_launchd_codex_state_dir "${SLOT_DIR}/q/${slot}" "$projectName" "$agentId")`;`tmuxSocket == ${SLOT_DIR}/tmux-$(id -u)/default`;`tuiWindow == "present"`;期望窗口名 `${projectName}-${agentId}`;slot 路径用 canonical(realpath)包含性,`evidence-dir` 的 canonical 路径必须在 SLOT_DIR 外(任一缺失 / 畸形 / 错配 → 在任何信号或 `launchctl` 前拒绝;每个字段各一缺失变异与**语法合法但错误**的错配变异 —— 含错 slot 号的 label、另一 slot 内的 stateDir、另一合法 projectName —— 断言零信号零 launchctl 调用);在 `evidence-dir` 下 `mktemp -d` 一个暂存目录并预校验(非 symlink、房外)后调用 `qa_launchd_lead_restart_drill "$label" codex-tui "$mode" "$codexHome" "$stateDir" "$tmuxSocket" "$projectName" "$agentId" "$stage"`;成功后把暂存目录原子 rename 为 `evidence-dir/fly-2301-drill-<mode>-<ts>/`;证据 JSON + 当时 `launch-manifest.json` + **old / new 两份心跳字节快照**(文件名带各自 sha256)+ 两个 tmux server 窗口清单原子写入 `evidence-dir`。
- 顺序:① 全部 shell 测试;② 真 529 Claude slot(`test-deploy.sh 2 --mode slot`)起 / 验 / 拆:plist / `.env` / per-Lead manifest 与 main 同命令产物原始 `cmp -s`;全局 manifest / stdout 先断言键序类型再做 T1 同一规范化后 `cmp`;原始 sha256 留档;全部产物拆房前拷到外部目录;③ slot 4 声明 codex 形状(`codexSourceHome=~/.codex-259-qa`,先 `codex login` 校验),记录默认 tmux server 清单、生产 state 目录与三个生产 Codex home 的**元数据清单 hash** 基线(零写入证据;snapshotter = `scripts/lib/qa-metadata-snapshot.py`,只做枚举 + lstat + symlink readlink,不 open 常规文件;其专属测试 `scripts/__tests__/qa-metadata-snapshot.test.sh`:含 mode 000 常规文件的 fixture home 上运行必须成功**且该条目出现在清单中**;镜像变异让 snapshotter 对该文件 `open()`(计数恰 1)→ 在 macOS `/bin/bash` 3.2 与 Ubuntu CI 两侧都必须失败);`test-deploy.sh 4 --mode slot`;A1–A3;两式演练(A4);拆房(A5);对照 A2 基线;④ 证据已在外部目录;⑤ 恢复 slot 4 条目。
