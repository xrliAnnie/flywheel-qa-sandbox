...
| grant receipt（需求 episode 的追加式不可变子记录，仅 writer；主键 (demand_id, carrier_generation)） | activation_id（**引用既有** `workflow_execution_binding.activation_id`）、granted_turn_epoch、carrier_generation、credential_row_id（StateStore 凭证行 id + digest，不存明文）、grantedAt；每行只能写一次（CAS 从空到值），不能更新；换代时追加新行，旧行保留作审计 | §5.3 第 3 步封存。**一个 demand 只有一次 grant**：activation / execution binding / `workflow_activation_turn` 与今天完全一致，一行、一个 epoch，所有现有消费者（wake identity/ACK、retirement、completion credential 校验、turn expectation）不改。载体换代（G→G+1）不重新 grant：TURN 仍由同一 execution 持有，新代 receipt 只是把同一 activation/epoch 与刷新后的凭证行重新绑定到新代数。凭证权威分两层：CommDB `runner_workflow_activation` 只是**首次 grant 时刻**的身份/context/凭证快照，G+1 **不更新它**（生产 replay 也明确不让后来的 rotated credential 覆盖它）；live credential 的唯一真源是 StateStore 当前凭证行。receipt 只在 §5.3 的 refresh operation 拿到载体安装回执后才写。TURN 一旦被转授给别的 holder，本 demand **终结**为 `superseded_by_transfer`（见第 4 步 b），不重新授权；该节点之后只能通过新的合法返工得到新需求，那是现有 coordinator 路径下的新 requestId / 新 activation / 正常 `grantTurn`，与本设计无关 |
| invalidation operation `workflow_invalidation_operation`（StateStore，durable 意图 = pre-grant barrier） | operation_id（server 派生；transfer 用 incoming source_event_id，每个 incoming transfer 各一条，**不共享**）、kind = transfer / demand_cancel / run_terminal / generation_change、目标（exec / demand / run）、expected holder execution + expected TURN epoch（transfer 冻结）、fencing delivery_id（可空：没有活动 delivery 时 barrier 仍存在）、stage = requested / fencing / drain_or_stop_confirmed / committed / superseded / stale、request id、updated_at | §5.3 e 的每次延迟提交都必须有这条持久身份，Bridge 重启后从它续跑，不靠内存调用者；建立 barrier 的同一事务把 expected holder 的 exec 置 `intake_open=false`（reserve 与 claim 的 CAS 都核此位），reserved/enqueued→cancelled、in_flight→fencing；run_terminal 优先级最高，可 supersede 其它 pending operation；expected holder/epoch 与当前不符则 stale，transfer 方重新读当前 holder 再建自己的 barrier |
| delivery authorization（一行/首条输入投递，StateStore） | delivery_id PK、demand_id、carrier_generation、receipt 引用、envelope digest、state = reserved/enqueued/in_flight/fencing/turn_stale/cancelled/aborted/consumed、fence_reason、updated_at；同 demand 同时只允许一条活动（reserved/enqueued/in_flight/fencing）delivery；turn_stale/cancelled/aborted/consumed 是终态 | §5.3 第 4 步 a 预留、c claim、d turn 结束、e fence；失效操作（TURN transfer / demand cancel / 整单终态 / carrier 换代）遇 reserved/enqueued 直接 CAS→cancelled，遇 in_flight 先 CAS→fencing 并等 drain 或精确同代 stop 确认后才提交自己；永远不会被一次 reserve 阻止；闭集转换见 §5.3 表；CommDB 项与 transport claim 都以 delivery_id 关联并回核此行 |
...
| 3 | turn-belt-reconcile；workflow-engine-dispatcher 与 rework/launch admission；CommDB `grantTurn` 转授路径 | 无进程且正常待命不判 STALE；合法阶段交接照常推进 TURN；恢复者必须重新 turn 查询；转授 = 下一位 holder 自己需求的生产 `grantTurn` 路径，只在其前插入 fence：当前 holder 有 in_flight delivery 时先 fencing、旧 turn 排空/停机确认后才调 `grantTurn`，被转走的需求终结为 superseded_by_transfer（§5.3 e） | 旧 manifest 中 TURN 不可复活；同目录两需求不能双写；转授不得与旧 holder 的模型 turn 并发 |
...
3. 按需求 episode 的 `authority_mode` 取权限：writer 需求在此处走原工作流 holder 激活路径（reentry → worktree → holder activation → admission → 凭证轮换 → `grantTurn` → `recordWorkflowActivationTurn`，**顺序、表、source event 全部沿用生产现状，不新增任何 grant 子系统**）。一个 demand 只 grant 一次：首次进入本步时按生产路径拿到 activation / epoch，把 activation_id / granted_turn_epoch / carrier_generation / credential_row_id 以 set-once CAS 追加为本代数的 grant receipt（主键 (demand_id, carrier_generation)）；生产路径本身已经是崩溃可重放的（rework request durable、`grantTurn` 对同一 source event 回放返回冻结 epoch、activation turn 一行 no-update），本设计只在它之后补一次 receipt CAS，重放时 receipt 已存在即 no-op。失败（admission 拒绝、TURN 不可得）则 hold 并显示等待 TURN，本代数无 receipt、预算不变；权威重新可得后再次进入本步就是同一次 grant 的重放，不会双 grant。conversation_only 需求不做 TURN 申请，也不激活 holder，没有 receipt。

   载体在 grant 后、首条输入消费前死亡的，清理确认后以 G+1 重试时**不重新 grant**：TURN 仍由同一 execution 持有、epoch 不变，CommDB `runner_workflow_activation` 保持首次 grant 的快照不动。本步只做「凭证刷新 operation」——把生产既有的 Codex recovery 模式（撤销旧行、生成一次性明文、spawn 前注入 adapter context、同 attempt 重放返回 `capabilities_already_prepared`、明文丢失则走 fresh attempt）泛化为两 vendor 共用、且带可信安装回执的四步：

   - **verify**：核 CommDB 当前 `three_stage_turn` holder/epoch 仍等于已有 receipt（不等即 demand 已被转授，见第 4 步 b）。
   - **rotate**：现有 `rotateGeneralizedWorkflow*Credential` / `prepareCodexRecoveryCapabilities` 扩展为在同一 StateStore 事务里原子返回明文 + 凭证行 id + digest（N2 范围，基线只返回明文 capability）；旧行撤销、新 live 行插入；明文只留在本次拉起调用的内存里。
   - **install**：adapter 在 paused / 无业务输入状态下把凭证安装进新载体（§5.3 preflight hold seam），通过 trusted launcher/hook 通道回一张绑定 (generation, 凭证行 digest, 本次 nonce) 的安装回执。
   - **receipt**：拿到安装回执后才追加 (demand_id, G+1) 的 set-once grant receipt，引用同一 activation/epoch 与该凭证行 id/digest。

   崩溃缝：安装前崩溃 → 本代无 receipt，下一次 fresh recovery attempt 先撤销该未投递的行再 rotate；安装后、receipt CAS 前崩溃 → 先通过同代载体的回调证明它仍持有该行 digest，证明得了才补写 receipt，证明不了就清理该载体、递增 generation（不允许在同一 set-once 行上换 row）。旧代 receipt 保留作审计，不是投递候选；episode 预算不重置。
4. 只有第 3 步结束后，才把 pending envelope（基线→当前 HEAD 的提交/文件变更摘要与 dirty 差异，见 §5.2；原会话缓存的是退下时刻的文件内容，fresh 会重新读盘而 resume 不会）与原需求投递为首条业务输入。demand / carrier / receipt 在 StateStore，当前 TURN holder+epoch 与 `turn_wake_outbox`/mailbox 在 CommDB，两库没有跨库事务（§2.2），所以这一步**不是**一个「同时重核四项的入队事务」，而是下面的双库线性化协议（每一步以唯一 `delivery_id` 幂等，任一阶段崩溃都从持久状态重放）：
   ...
   a. **StateStore 预留（CAS INSERT）**：对当前 carrier generation 的 receipt 插入一条 delivery authorization（`delivery_id`、demand_id、carrier_generation、receipt 引用、envelope digest、state=reserved）。同一事务里读取并核 source facts：demand 未取消未过期、run 非终态、carrier generation 未变、carrier `intake_open=true`、本代数的 receipt 存在、该 demand 没有其它活动 delivery；任一不成立则不插入。已经发生的 cancel / 终态 / 换代因此天然阻止后续 reserve——它们改的是 demand/run/carrier 这些 source 行，不需要 delivery 行先存在。预留同事务写入 durable outbox（复用 park outbox 的投影链路，v2 事件）。
   b. **CommDB 入队（IMMEDIATE 事务）**：核 exact `three_stage_turn` holder+epoch 与 activation 元组等于 receipt 里的 granted 值，不等则不入队并把结果回投 StateStore（回投是一个 StateStore 事务：delivery CAS reserved→turn_stale 终态，同事务把 demand 终结为 `superseded_by_transfer`——TURN 已属于别的 holder，本需求不再重新授权，节点等待下一次合法返工产生的新需求）；相等则原子 enqueue 带同一 `delivery_id` 的 wake/mailbox 项（对 delivery_id 幂等，重复入队为 no-op），并回投 StateStore CAS reserved→enqueued。`grantTurn` 转授在**同一 CommDB 事务**里取消所有带旧 epoch、尚未消费的 delivery 项，因此转授与入队在 CommDB 内线性化。
   c. **transport claim（push 前）**：claim 时 StateStore CAS enqueued→`in_flight`，同事务再核 carrier generation 相符、carrier `intake_open=true`、run 非终态、demand 有效（未被 superseded/cancel）；投影未知或落后 fail closed，不推给模型。「B 检查 A 无 in_flight → A 随后 claim 并 push → B grant」这条竞态由此关闭：barrier 建立即关 intake，之后 A 的 claim CAS 必失败。in_flight 是一个 durable 的「模型正在这一轮里」标记，从 push 起一直到 turn 结束回执。这一步挡住「CommDB 入队后、StateStore 里 demand 被取消 / run 终结 / carrier 换代」的窗口。
   d. **turn 结束与回投**：模型 turn 结束回执（不是首个 token 消费）按 `delivery_id` 幂等地把 delivery CAS in_flight→consumed（重复 ACK no-op）；若该行已 aborted，ACK 记为 `late_ack` no-op。Bridge 在 a 后、b 后、push 后、turn 结束后任一点崩溃，重启都按 delivery_id 从两库持久状态续跑，不会产生第二条 delivery，也不会把陈旧 envelope 推给模型。

   e. **in_flight 期间的失效 = 两阶段 fence，转授/终态在旧 turn 排空或被精确停机确认之前不能提交**。TURN 合同要求任一时刻只有 holder 能碰 worktree，而 writer profile 是 workspace-write + network，撤销提交凭证挡不住本地文件 / Git / 外部副作用，所以「让旧 turn 跑完再说」不是 fence。规则：
      - 触发者（TURN transfer、demand cancel、整单终态、carrier 换代）**无论当前有没有活动 delivery** 都先在 StateStore 写一条 durable `workflow_invalidation_operation` 作 barrier（server 派生 operation_id、kind、目标、transfer 还冻结 expected holder + epoch，stage=requested），同一事务把目标 exec 的 `intake_open` 置 false、reserved/enqueued→cancelled、in_flight→fencing；旧 holder 之后的任何 reserve / claim 在 a / c 步被 `intake_open` 拒绝。没有 in_flight 时 barrier 直接进入 drain_or_stop_confirmed。Bridge 重启后从这条记录续跑，不依赖内存调用者。
      - **transfer 不是一个新协议，就是下一位 holder 自己那条合法需求的生产 grant 路径**（新 requestId 的 rework/phase-wake → coordinator reentry → worktree → holder activation → admission → 凭证轮换 → `grantTurn`）。本设计只在 coordinator 调 `grantTurn` **之前**插入一道 barrier：coordinator 读当前 holder execution + epoch，为**本次 incoming transfer**（operation_id = 自己的 source_event_id）建立上述 barrier 并冻结 expected holder/epoch——不管对方有没有活动 delivery；等 barrier 到 drain_or_stop_confirmed 后再继续原路径。`grantTurn` 仍是单个 IMMEDIATE 事务、一次到位改 holder/epoch、写 `runner_workflow_activation` / `turn_source_history` / `workflow_source_event`，凭证明文只存在于这一次调用的内存里，不需要 CommDB 侧任何 pending 记录；唯一的生产改动是给 `grantTurn` 加 **expected holder + epoch 参数**，在它原有的 IMMEDIATE 事务内不符则不写 epoch / source / history / activation 并返回 mismatch（基线对不同 source event 无 expected-current guard，会读当前值后直接 epoch+1，所以这一条不可省）。barrier 与 `grantTurn` 之间 Bridge 重启：rework request 与 operation 都是 durable 的，coordinator 重入后重新核 barrier 已完成、expected 仍是当前，再调 `grantTurn`；source event 回放冻结保证不会双 grant。**barrier 不共享**：并发的 B、C 各自建自己的 barrier；A 排空后 B 先 grant（expected=A/E 匹配）→ C 的 barrier expected 仍是 A/E，与当前 B/E+1 不符 → C 的 barrier 置 stale、C 重新读当前 holder=B，为 B 建新 barrier（B 若有 in_flight 同样要等 B 排空），再以 expected=B/E+1 调 `grantTurn`。
      - 控制器完成 fence 二选一：等旧 turn 正常 drain（turn 结束回执 → consumed）；或对该 carrier 发精确同代 stop（§3 的 PID+启动时间+所有权核验），确认专属进程/daemon/端点全部消失（同 §3 第 5 步的 60 秒规则，失败走 `retirement_unconfirmed` 收敛表）。两者之一成立才 CAS fencing→`aborted`（或 consumed），operation → drain_or_stop_confirmed。
      - 只有 barrier 到 drain_or_stop_confirmed（fencing delivery 到终态 consumed/aborted 并投影到 CommDB，或本就没有活动 delivery）后，operation 才能 committed：transfer 的 coordinator 才能以 expected holder/epoch 调 `grantTurn`、整单终态才能 close carrier、新 writer 才能被启用。转授/终态因此**永远不会**与旧 turn 的写并发；代价是转授最多等一个 turn drain 或一次有界 stop，这是接受的。转授提交后，被转走的 demand 在同一 StateStore 事务里终结为 `superseded_by_transfer`（其活动 delivery 已在 fence 中到终态；episode 内已花掉的尝试留痕，不退不重置）。
      - 纵深防御（不是主 fence）：Claude PreToolUse hook 在每次 mutating tool 前回核 (generation, delivery_id) 仍为 in_flight，fencing 即拒绝；Codex writer 走 approvalPolicy never，没有逐工具闸，只能靠 stop——所以 Codex 的 fencing 若旧 turn 未在 drain 期限内结束就直接 stop。输出/提交凭证按 §5.4 轮换只是第三道。
      - 结论统一：push 前的失效由 a/b/c 保证「陈旧输入不进模型」；push 后的失效由 e 保证「转授/终态/新 writer 在旧 turn 排空或停机确认前不提交」。两条合起来才是 writer fence，矩阵 P 按此验收。

   delivery authorization 的闭集转换（全部是 StateStore 内的单事务 CAS；没有「先 reserve 就赢」这回事，失效永远压过未消费的投递）：

   | 操作 | 允许的前态 → 后态 | 同事务必须做的事 / 冲突规则 |
   |---|---|---|
   | reserve | 无活动 delivery → reserved（INSERT） | 核 a 中的 source facts（含 carrier `intake_open`）；source 已失效或 intake 已关则拒绝 |
   | enqueued 回投 | reserved → enqueued | 仅 b 成功后；已 cancelled 则 no-op |
   | turn_stale 回投 | reserved → turn_stale（终态） | 仅 b 失败后；已 cancelled 则 no-op |
   | demand cancel / 过期 | 该 demand 的 reserved/enqueued → cancelled | 与更新 demand 行同一事务；不能被任何 reserve 阻止 |
   | 整单终态（ship/取消/关闭） | 该 run 所有 exec 的 reserved/enqueued → cancelled | 与 close carrier、撤销凭证同一事务（§4）；终态永远不因存在 reserve 而失败 |
   | carrier generation change | 该 exec 旧代数的 reserved/enqueued → cancelled | 与递增 generation 同一事务 |
   | claim（push 前） | enqueued → in_flight | 同事务核 generation / `intake_open` / run / demand 有效；不符则不 push，CAS→cancelled |
   | turn 结束 ACK | in_flight → consumed（终态） | 幂等；已 aborted 则记 `late_ack` no-op |
   | 失效触发（transfer / cancel / 终态 / 换代）遇 in_flight | in_flight → fencing | 同事务写 invalidation operation、关闭 intake；transfer 的 coordinator 在此等待，不调 `grantTurn` |
   | drain 或精确同代 stop 确认 | fencing → consumed（drain）/ aborted（stop） | 确认后投影到 CommDB；只有此后 transfer 的 coordinator 才继续原路径调 `grantTurn`（被转走的 demand 同事务终结为 superseded_by_transfer）、终态才 close carrier、新 writer 才启用；operation → committed |
   | 失效触发遇 reserved/enqueued | → cancelled | 不需要 fence（模型未拿到输入），但 barrier 仍建立并关 intake |
   | 失效触发时无活动 delivery | （无 delivery 行） | barrier 仍建立并关 intake，直接 drain_or_stop_confirmed；之后的 reserve/claim 被拒 |
   | 迟到的 projector / ACK / enqueue 回投 | 任一终态 → 不变 | 一律 no-op，只留审计 |

   没有「先 reserve 就赢」，也没有「先 push 就能跑完一轮再说」：push 前失效 → 输入不进模型；push 后失效 → 旧 turn 先排空或被停机确认，转授/终态/新 writer 才提交。旧 turn 在 fencing 期间产生的本地改动属于它仍合法持有 TURN 的时段，随 worktree 保留并进入下一位 holder 的基线比对（§5.2）；它的输出/提交凭证在 aborted 时随 episode 轮换失效，不能提交结果。

   writer 投递候选只有当前 carrier generation 对应的 receipt；需求创建后 TURN 已被转授的，该需求终结，旧 demand/旧 receipt 不得投递，也不为它重新授权。conversation_only 需求走同一协议但跳过 b 中的 TURN 核验（只核 activation 不存在与 profile）。摘要先持久在 envelope 里，不在核验前进入模型；任一阶段失败时 envelope 保留，不投递，episode 预算不重置。
...
会话连续不等于旧权限继续：普通恢复保持当前 activation；合法返工由原工作流产生新 activation/attempt/输出凭证时，刷新到原 exec 环境/协议，旧 credential 不再可用，旧输出回放不能成为新结果。对于无法安全刷新当前进程权限的 carrier，重建载体并验证原会话后再给新需求，不复用旧 shell env。凭证的两层权威见 §2.1 receipt 行与 §5.3 第 3 步：CommDB activation 只是 grant 时刻快照，live 凭证真源是 StateStore 当前行，刷新走带安装回执的 refresh operation。
...
| F | Bridge 分别在 intent、投影、stop、退出、spawn、身份确认、grant 后首条输入消费前、投递后重启/进程丢失；生产 grant 路径两处：凭证轮换后 `grantTurn` 前、`grantTurn` 提交后 receipt CAS 前；G+1 refresh operation 三处（Claude/Codex 各一遍）：rotate 后 install 前、install 后 receipt 前、receipt 后 | 每点都只有一个身体；回执可补写；旧代数 kill/finally 不生效；模型不双消费；grant 后载体丢失→清理确认→同 demand 以新 generation 重试可继续且**不重新 grant**（同 activation/epoch，新 receipt 行、凭证刷新），预算不重置；grant 两处崩溃后恢复：恰一个 source event/epoch（`grantTurn` 回放冻结）、receipt CAS 幂等；refresh 三处：rotate 后 install 前崩溃 → 本代无 receipt、fresh attempt 撤销未投递行再 rotate；install 后 receipt 前崩溃 → 同代载体回调证明持有该 digest 才补 receipt，否则清理载体递增 generation；**载体 observed credential digest = receipt 引用的 StateStore live 行**，且 CommDB `runner_workflow_activation` 保持首次 grant 快照不变；旧凭证已撤销，episode 预算不重置 |
...
| P | 普通 mailbox 坏 handle、gate response 坏 handle、TURN 已转授、需求创建后 TURN 变化再恢复、receipt 封存后 TURN 转授、conversation_only 载体上到达 writer 需求；投递协议每个缝隙：StateStore 预留→CommDB 入队之间、CommDB 入队→push 之间、**push 后首个工具前、mid-turn** 分别发生 TURN transfer / cancel / 整单终态 / carrier generation change / Bridge crash；cancel/终态**先于** reserve 与 reserve 成功**之后**两组；从 abce27a27 真实 schema 建库：holder A 持 epoch E 且 in_flight → B 的合法返工触发转授 → A 的 demand 终结、B 得 E+1；transfer 在 barrier 前/后、终态投影前/后、`grantTurn` 前后分别重启 Bridge；两条确定性竞态：B 检查 A 无 in_flight 后 A 立即 claim；B grant 后开始 claim、C 随后 grant | conversation_only 不产生 writer/不激活 holder 但仍有无写权限兜底；transfer 各点重启后旧 holder/epoch 在 fence 完成前不变、最终恰一个 source event/epoch 生效（回放冻结）、A 的 demand 恰好一次终结为 superseded_by_transfer 且旧 receipt/delivery 不能投递；A 在 B 的 barrier 建立后 claim 必因 `intake_open=false` 失败，B 的 grant 不与 A 的 turn 并发；C 沿用 expected=A/E 调 `grantTurn` 得 mismatch、不生成 source event/epoch，C 的 barrier 置 stale 后为 B 重建 barrier、等 B 排空再以 expected=B/E+1 grant；HEAD/dirty 摘要不在 TURN CAS 前进入模型；writer 投递只匹配当前 generation 的 receipt；CommDB 入队核 exact turn 元组；push 前失效 → 输入不进模型；push 后失效 → StateStore 先 fencing、转授方 coordinator 停在 `grantTurn` 之前，Claude/Codex 真载体在旧 turn 里实际尝试文件/Git/外部 mutation：Claude 被 hook 拒，Codex 在 drain 期限内被精确同代 stop，转授/终态/新 writer 在 consumed/aborted 投影后才提交，且新 holder 启动前旧进程已确认消失；activation / binding / `workflow_activation_turn` 行与今天完全一致、现有消费者零改动；delivery_id 不重复、episode 预算不重置；cancel 先于 reserve → 被 source facts 拒绝；reserve 后失效 → 活动 delivery 必到 cancelled/aborted，整单终态不因 reserve 失败；conversation_only → writer 不原地扩权，必须重建新代载体；崩溃后按 delivery_id 重放不双发 |
