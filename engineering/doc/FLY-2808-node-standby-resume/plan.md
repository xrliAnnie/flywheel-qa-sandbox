...
| TURN grant-episode 账本 `workflow_turn_grant_episode`（新表，追加式，仅 writer；主键 (demand_id, grant_id)） | grant_id（同 demand 内单调 ordinal）、activation_id（**引用既有** `workflow_execution_binding.activation_id`，不新建 activation）、source_event_id = `rework-turn:${requestId}:${activationId}:g${grant_id}`、granted_turn_epoch、carrier_generation、credential_nonce、grantedAt；每行只能写一次（CAS 从空到值），不能更新；demand 行上有 `current_grant_id` 指针（CAS 推进）；旧行保留但不再是投递候选 | §5.3 第 3 步在原工作流真实 grant 之后封存。**既有 workflow activation / execution binding 完全不动**：基线 `workflow_execution_binding` 以 activation_id 为主键、(execution_id, run_id, node_id, attempt) 唯一、no-update/no-delete，`admitGeneralizedWorkflowExecution` 对同 tuple 不同 activation_id 返回 activation_conflict，所以 TURN 重授不能伪装成第二个 activation。重授只在这张新账本追加一行，并用**含 grant_id 的新 source_event_id** 去调基线 `grantTurn`（同一 source event 回放只返回冻结的旧 epoch，新 source event 才拿到新 epoch），activation 仍是原来那个。每个 episode 有自己的 credential_nonce：输出/提交凭证按 episode 轮换（§5.4），旧 episode 的凭证失效；delivery authorization 精确引用 grant_id。grant 后、首条输入消费前载体死亡→清理确认→同 demand 以 G+1 重试，或 receipt 后 TURN 被转授→`turn_stale`，都追加新 episode，不重置 episode 预算。创建需求时不存在未来的 epoch，所以不在创建时冻结、不猜、不事后回填 |
| grant operation `workflow_turn_grant_operation`（StateStore，可变的 durable 意图，主键 (demand_id, grant_id)） | activation_id、source_event_id、carrier_generation、owner claim、request id、stage = allocated / credential_rotated / granted / finalized / failed、credential_attempt（当前轮换序号）、credential_row_id（StateStore 凭证行 id + digest，不存明文）、updated_at | §5.3 第 3 步的每个阶段各一次 CAS；Bridge 崩溃后恢复只续接**当前未 finalized 的 grant_id**，绝不再分配新 grant_id；finalized 后才写不可变 episode 行 |
| invalidation operation `workflow_invalidation_operation`（StateStore，durable 意图） | operation_id（server 派生）、kind = transfer / demand_cancel / run_terminal / generation_change、目标（exec / demand / run）、fencing delivery_id、stage = requested / fencing / drain_or_stop_confirmed / committed / superseded、request id、updated_at | §5.3 e 的每次延迟提交都必须有这条持久身份，不能只靠内存调用者；run_terminal 优先级最高，可 supersede 其它 pending operation |
| CommDB `turn_transfer_intent`（新表；或 `three_stage_turn` 的 pending 子表） | intent_id = 将来 grant 的 source_event_id（server 派生，唯一）、expected holder+epoch+activation、target holder / run / node / attempt / activation、fencing delivery_id、state = pending / committed / cancelled / superseded、prepared_at / committed_at；同一 worktree 同时只允许一条 pending | prepare（IMMEDIATE 事务）：只在 exact expected 元组匹配且无其它 pending 时插入，并在 `three_stage_turn` 置 `admission_closed_since_epoch`，**不改** holder/epoch、**不写** `workflow_source_event`；commit（IMMEDIATE 事务）：只在 fencing delivery 的终态投影已到且 expected 元组仍匹配时，原子做 epoch+1、`runner_workflow_activation`、`turn_source_history`、`workflow_source_event` 写入并置 committed；重复 commit 返回同一 epoch；expected 不匹配→cancelled |
| delivery authorization（一行/首条输入投递，StateStore） | delivery_id PK、demand_id、carrier_generation、grant_id、episode 引用、envelope digest、state = reserved/enqueued/in_flight/fencing/turn_stale/cancelled/aborted/consumed、fence_reason、updated_at；同 demand 同时只允许一条活动（reserved/enqueued/in_flight/fencing）delivery；turn_stale/cancelled/aborted/consumed 是终态 | §5.3 第 4 步 a 预留、c claim、d turn 结束、e fence；失效操作（TURN transfer / demand cancel / 整单终态 / carrier 换代）遇 reserved/enqueued 直接 CAS→cancelled，遇 in_flight 先 CAS→fencing 并等 drain 或精确同代 stop 确认后才提交自己；永远不会被一次 reserve 阻止；闭集转换见 §5.3 表；CommDB 项与 transport claim 都以 delivery_id 关联并回核此行 |
...
| 3 | turn-belt-reconcile；workflow-engine-dispatcher 与 rework/launch admission；CommDB `grantTurn` 转授路径 | 无进程且正常待命不判 STALE；合法阶段交接照常推进 TURN；恢复者必须重新 turn 查询；转授对 in_flight 的 holder 先 prepare `turn_transfer_intent`（不改 holder/epoch），旧 turn 排空/停机确认后才 commit（§5.3 e） | 旧 manifest 中 TURN 不可复活；同目录两需求不能双写；转授不得与旧 holder 的模型 turn 并发 |
...
3. 按需求 episode 的 `authority_mode` 取权限：writer 需求在此处走原工作流 holder 激活路径（reentry → worktree → holder activation → admission → `grantTurn` → 记录 turn，顺序不变），每次进入本步都是一个新的 grant episode，按基线 coordinator 的真实顺序（**先轮换凭证、再把明文 token 作为参数调 `grantTurn`**，CommDB 在同一 grant 事务里把 token 写进该 epoch 的 `runner_workflow_activation`；StateStore 只存凭证 hash，明文只在轮换当次返回）分成可重放的阶段，每阶段一次 StateStore CAS 落在 `workflow_turn_grant_operation`：

   - **allocated**：一个 StateStore 事务里 demand 行 `current_grant_id` CAS +1 得到 grant_id，同时固定 activation_id（**沿用既有** binding，reentry/worktree/holder activation/admission 对已有 binding 幂等通过）、source_event_id = `rework-turn:${requestId}:${activationId}:g${grant_id}`、carrier_generation、owner claim、request id。
   - **credential_rotated**：以 (grant_id, credential_attempt) 绑定轮换输出/提交凭证，StateStore 记 credential_row_id + digest，明文只留在本次调用内存。
   - **granted**：把明文 token 传给 `grantTurn(source_event_id, …)`；CommDB 事务提交后记 granted_turn_epoch。
   - **finalized**：追加不可变 `workflow_turn_grant_episode` 行（activation_id / source_event_id / epoch / generation / credential_row_id），operation 置 finalized。

   **崩溃恢复只续接当前未 finalized 的 grant_id，不再 +1**：恢复时先查 CommDB 是否已存在该 source_event_id——不存在则（若已轮换过）以新 credential_attempt 再轮换一次并撤销上一次未投递的 token，再调 `grantTurn`；已存在则读取其冻结 epoch 与 `runner_workflow_activation` 投影，直接补写 finalized，**不再轮换、不再 grant**。episode 行保存的是凭证行 id/digest，不试图恢复明文。失败（admission 拒绝、TURN 不可得）则 operation 置 failed 并 hold 显示等待 TURN，本 grant_id 无 episode 行、预算不变。conversation_only 需求不做 TURN 申请，也不激活 holder，没有 receipt。载体在 grant 后、首条输入消费前死亡的，清理确认后以 G+1 重试时重新走本步（新 grant_id）；receipt 封存后 TURN 被转授（第 4 步 b 报 `turn_stale`）且 demand 仍有效、权威重新可得时，同样重新走本步开新 grant episode——旧 receipt、旧 source event 回放、旧 activation 凭证永远不能复活。以上都不重置 episode 预算。若 Lead/产品将来决定「转授即永久取代当前需求」，须改为明确终结该 demand 并对后续需求沿用同一 episode 预算，本设计不采用该选项。
4. 只有第 3 步结束后，才把 pending envelope（基线→当前 HEAD 的提交/文件变更摘要与 dirty 差异，见 §5.2；原会话缓存的是退下时刻的文件内容，fresh 会重新读盘而 resume 不会）与原需求投递为首条业务输入。demand / carrier / receipt 在 StateStore，当前 TURN holder+epoch 与 `turn_wake_outbox`/mailbox 在 CommDB，两库没有跨库事务（§2.2），所以这一步**不是**一个「同时重核四项的入队事务」，而是下面的双库线性化协议（每一步以唯一 `delivery_id` 幂等，任一阶段崩溃都从持久状态重放）：
   ...
   a. **StateStore 预留（CAS INSERT）**：对当前 carrier generation、当前 grant_id 的 grant episode 插入一条 delivery authorization（`delivery_id`、demand_id、carrier_generation、grant_id、episode 引用、envelope digest、state=reserved）。同一事务里读取并核 source facts：demand 未取消未过期、run 非终态、carrier generation 未变、(generation, grant_id) 的 grant episode 行存在、该 demand 没有其它活动 delivery；任一不成立则不插入。已经发生的 cancel / 终态 / 换代因此天然阻止后续 reserve——它们改的是 demand/run/carrier 这些 source 行，不需要 delivery 行先存在。预留同事务写入 durable outbox（复用 park outbox 的投影链路，v2 事件）。
   b. **CommDB 入队（IMMEDIATE 事务）**：核 exact `three_stage_turn` holder+epoch 与 activation 元组等于 receipt 里的 granted 值，不等则不入队并把结果回投 StateStore（delivery CAS reserved→turn_stale，这是该 delivery 的终态；demand 仍有效则按第 3 步开新 grant episode 产生新 delivery_id，否则 hold）；相等则原子 enqueue 带同一 `delivery_id` 的 wake/mailbox 项（对 delivery_id 幂等，重复入队为 no-op），并回投 StateStore CAS reserved→enqueued。`grantTurn` 转授在**同一 CommDB 事务**里取消所有带旧 epoch、尚未消费的 delivery 项，因此转授与入队在 CommDB 内线性化。
   c. **transport claim（push 前）**：claim 时 StateStore CAS enqueued→`in_flight`，同事务再核 carrier generation 相符、run 非终态、demand 有效、grant_id 仍是 current；投影未知或落后 fail closed，不推给模型。in_flight 是一个 durable 的「模型正在这一轮里」标记，从 push 起一直到 turn 结束回执。这一步挡住「CommDB 入队后、StateStore 里 demand 被取消 / run 终结 / carrier 换代」的窗口。
   ...
   e. **in_flight 期间的失效 = 两阶段 fence，转授/终态在旧 turn 排空或被精确停机确认之前不能提交**。TURN 合同要求任一时刻只有 holder 能碰 worktree，而 writer profile 是 workspace-write + network，撤销提交凭证挡不住本地文件 / Git / 外部副作用，所以「让旧 turn 跑完再说」不是 fence。规则：
      - 触发者（TURN transfer 请求、demand cancel、整单终态、carrier 换代）先在 StateStore 写一条 durable `workflow_invalidation_operation`（server 派生 operation_id、kind、目标、fencing delivery_id，stage=requested），同事务 CAS in_flight→`fencing` 并把该 exec 标为 intake 已关闭；Bridge 重启后从这条记录续跑，不依赖内存调用者。
      - transfer 的 CommDB 侧是显式 **prepare / commit**，不是提前调 `grantTurn`（基线 `grantTurn` 是单个 IMMEDIATE 事务，会立刻改 holder/epoch 并写 `runner_workflow_activation` / `turn_source_history` / 不可变 `workflow_source_event`；提前调就是过早转授，提前占用同一 source_event 又会让最终调用走 frozen replay）。prepare：IMMEDIATE 事务里核 exact expected holder+epoch+activation 元组，插入 `turn_transfer_intent`（intent_id = 将来 grant 的 source_event_id，同一 worktree 只允许一条 pending），并置 `three_stage_turn.admission_closed_since_epoch`——从此旧 holder 的任何新 delivery 在 b 步都被拒；holder/epoch 不变，不写 source event。commit：只在 fencing delivery 的终态投影（consumed/aborted）已到达 CommDB 且 expected 元组仍匹配时，原子完成 epoch+1 与全部 grant 写入并置 committed；重复 commit 幂等返回同一 epoch；expected 不匹配（被另一 transfer/终态抢先）→ cancelled。闭集：同一 worktree 第二个 prepare 被拒；整单终态 supersede 任何 pending intent（→superseded）并自行 close；demand cancel / generation change 不需要 intent，只走 StateStore operation + fence；迟到投影对已 committed/cancelled/superseded 的 intent no-op。
      - 控制器随后二选一：等旧 turn 正常 drain（turn 结束回执 → consumed）；或对该 carrier 发精确同代 stop（§3 的 PID+启动时间+所有权核验），确认专属进程/daemon/端点全部消失（同 §3 第 5 步的 60 秒规则，失败走 `retirement_unconfirmed` 收敛表）。两者之一成立才 CAS fencing→`aborted`（或 consumed）。
      - 只有 delivery 到达终态（consumed/aborted）并投影到 CommDB 后，invalidation operation 才能进入 committed：transfer intent 才能 commit 为真正的转授（epoch+1）、整单终态才能 close carrier、新 writer 才能被启用。转授/终态因此**永远不会**与旧 turn 的写并发；代价是转授最多等一个 turn drain 或一次有界 stop，这是接受的。
      - 纵深防御（不是主 fence）：Claude PreToolUse hook 在每次 mutating tool 前回核 (generation, grant_id, delivery_id) 仍为 in_flight，fencing 即拒绝；Codex writer 走 approvalPolicy never，没有逐工具闸，只能靠 stop——所以 Codex 的 fencing 若旧 turn 未在 drain 期限内结束就直接 stop。输出/提交凭证按 episode 轮换只是第三道。
   ...
   |---|---|---|
   | reserve | 无活动 delivery → reserved（INSERT） | 核 a 中的 source facts；source 已失效则拒绝 |
   ...
   | claim（push 前） | enqueued → in_flight | 同事务核 generation / run / demand / current grant_id；不符则不 push，CAS→cancelled |
   | turn 结束 ACK | in_flight → consumed（终态） | 幂等；已 aborted 则记 `late_ack` no-op |
   | 失效触发（transfer / cancel / 终态 / 换代）遇 in_flight | in_flight → fencing | 同事务写 invalidation operation、关闭 intake；transfer 另在 CommDB prepare `turn_transfer_intent`，不改 holder/epoch、不写 source event |
   | drain 或精确同代 stop 确认 | fencing → consumed（drain）/ aborted（stop） | 确认后投影到 CommDB；只有此后 transfer intent 才 commit、终态才 close carrier、新 writer 才启用；operation → committed |
   | 失效触发遇 reserved/enqueued | → cancelled | 不需要 fence（模型未拿到输入） |
   ...
   writer 投递候选只有当前 carrier generation、当前 grant_id 对应的 receipt；需求创建后 TURN 已变化的，只有新 grant episode 取得真实新 epoch 的 receipt 才能投递，旧 demand/旧 receipt/旧 source event 回放不得投递。conversation_only 需求走同一协议但跳过 b 中的 TURN 核验（只核 activation 不存在与 profile）。摘要先持久在 envelope 里，不在核验前进入模型；任一阶段失败时 envelope 保留，不投递，episode 预算不重置。
...
| F | Bridge 分别在 intent、投影、stop、退出、spawn、身份确认、grant 后首条输入消费前、投递后重启/进程丢失；grant operation 的三处：grant_id 分配后、凭证轮换后、CommDB grant 后/StateStore finalized 前 | 每点都只有一个身体；回执可补写；旧代数 kill/finally 不生效；模型不双消费；grant 后载体丢失→清理确认→同 demand 以新 generation 重试可继续，追加新 episode，预算不重置；grant 三处崩溃后恢复都续接同一 grant_id：恰一个 source event/epoch，当前 epoch 的 `runner_workflow_activation` 带正确 live credential，旧 credential 已撤销，未投递 token 已撤销，episode 预算不重置 |
...
| P | 普通 mailbox 坏 handle、gate response 坏 handle、TURN 已转授、需求创建后 TURN 变化再恢复、receipt 封存后 TURN 转授、conversation_only 载体上到达 writer 需求；投递协议每个缝隙：StateStore 预留→CommDB 入队之间、CommDB 入队→push 之间、**push 后首个工具前、mid-turn** 分别发生 TURN transfer / cancel / 整单终态 / carrier generation change / Bridge crash；cancel/终态**先于** reserve 与 reserve 成功**之后**两组；从 abce27a27 真实 schema 建库，同一 (execution, run, node, attempt) 连续两个 grant episode（epoch E → 转授 → E+1 重授）；transfer 在 prepare 前/后、终态投影前/后、commit 中途分别重启 Bridge；两个并发 transfer 竞争同一 worktree | conversation_only 不产生 writer/不激活 holder 但仍有无写权限兜底；transfer 各点重启后旧 holder/epoch 在 fence 完成前不变、最终恰一个 source event/epoch 生效、重复 commit 返回同一 epoch；并发 transfer 只有一个 pending，另一个被拒或 superseded；HEAD/dirty 摘要不在 TURN CAS 前进入模型；writer 投递只匹配当前 generation+grant_id 的 episode；CommDB 入队核 exact turn 元组；push 前失效 → 输入不进模型；push 后失效 → StateStore 先 fencing、CommDB 只 prepare turn_transfer_intent，Claude/Codex 真载体在旧 turn 里实际尝试文件/Git/外部 mutation：Claude 被 hook 拒，Codex 在 drain 期限内被精确同代 stop，转授/终态/新 writer 在 consumed/aborted 投影后才提交，且新 holder 启动前旧进程已确认消失；两个 grant episode 都成功记录、activation/binding 行不变、旧 episode 的 source event 回放只还旧 epoch 且不能投递；delivery_id 不重复、episode 预算不重置；cancel 先于 reserve → 被 source facts 拒绝；reserve 后失效 → 活动 delivery 必到 cancelled/aborted，整单终态不因 reserve 失败；conversation_only → writer 不原地扩权，必须重建新代载体；崩溃后按 delivery_id 重放不双发 |
