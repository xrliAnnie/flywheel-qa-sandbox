...
| grant receipt（需求 episode 的追加式不可变子记录，仅 writer；主键 (demand_id, carrier_generation)） | activation_id（**引用既有** `workflow_execution_binding.activation_id`）、granted_turn_epoch、carrier_generation、credential_row_id（StateStore 凭证行 id + digest，不存明文）、grantedAt；每行只能写一次（CAS 从空到值），不能更新；换代时追加新行，旧行保留作审计 | §5.3 第 3 步封存。**一个 demand 只有一次 grant**：activation / execution binding / `workflow_activation_turn` 与今天完全一致，一行、一个 epoch，所有现有消费者（wake identity/ACK、retirement、completion credential 校验、turn expectation）不改。载体换代（G→G+1）不重新 grant：TURN 仍由同一 execution 持有，新代 receipt 只是把同一 activation/epoch 与刷新后的凭证行重新绑定到新代数（凭证按 §5.4 的现有轮换：撤销旧行、插入新 live 行，明文只在拉起新载体的那次调用里注入）。TURN 一旦被转授给别的 holder，本 demand **终结**为 `superseded_by_transfer`（见第 4 步 b），不重新授权；该节点之后只能通过新的合法返工得到新需求，那是现有 coordinator 路径下的新 requestId / 新 activation / 正常 `grantTurn`，与本设计无关 |
| invalidation operation `workflow_invalidation_operation`（StateStore，durable 意图） | operation_id（server 派生，对同一 fencing delivery 唯一）、kind = transfer / demand_cancel / run_terminal / generation_change、目标（exec / demand / run）、fencing delivery_id、stage = requested / fencing / drain_or_stop_confirmed / committed / superseded、request id、updated_at | §5.3 e 的每次延迟提交都必须有这条持久身份，Bridge 重启后从它续跑，不靠内存调用者；run_terminal 优先级最高，可 supersede 其它 pending operation |
...
3. 按需求 episode 的 `authority_mode` 取权限：writer 需求在此处走原工作流 holder 激活路径（reentry → worktree → holder activation → admission → 凭证轮换 → `grantTurn` → `recordWorkflowActivationTurn`，**顺序、表、source event 全部沿用生产现状，不新增任何 grant 子系统**）。一个 demand 只 grant 一次：首次进入本步时按生产路径拿到 activation / epoch，把 activation_id / granted_turn_epoch / carrier_generation / credential_row_id 以 set-once CAS 追加为本代数的 grant receipt（主键 (demand_id, carrier_generation)）；生产路径本身已经是崩溃可重放的（rework request durable、`grantTurn` 对同一 source event 回放返回冻结 epoch、activation turn 一行 no-update），本设计只在它之后补一次 receipt CAS，重放时 receipt 已存在即 no-op。失败（admission 拒绝、TURN 不可得）则 hold 并显示等待 TURN，本代数无 receipt、预算不变；权威重新可得后再次进入本步就是同一次 grant 的重放，不会双 grant。conversation_only 需求不做 TURN 申请，也不激活 holder，没有 receipt。
   ...
   载体在 grant 后、首条输入消费前死亡的，清理确认后以 G+1 重试时**不重新 grant**：TURN 仍由同一 execution 持有、epoch 不变，本步只做三件事——核 CommDB 当前 `three_stage_turn` holder/epoch 仍等于已有 receipt（不等即 demand 已被转授，见第 4 步 b）、按 §5.4 现有轮换刷新该 activation 的输出/提交凭证（撤销旧行、插入新 live 行，明文只注入新载体）、追加 (demand_id, G+1) 的 receipt 引用同一 activation/epoch 与新凭证行。旧代 receipt 保留作审计，不是投递候选；episode 预算不重置。
4. 只有第 3 步结束后，才把 pending envelope（基线→当前 HEAD 的提交/文件变更摘要与 dirty 差异，见 §5.2；原会话缓存的是退下时刻的文件内容，fresh 会重新读盘而 resume 不会）与原需求投递为首条业务输入。demand / carrier / receipt 在 StateStore，当前 TURN holder+epoch 与 `turn_wake_outbox`/mailbox 在 CommDB，两库没有跨库事务（§2.2），所以这一步**不是**一个「同时重核四项的入队事务」，而是下面的双库线性化协议（每一步以唯一 `delivery_id` 幂等，任一阶段崩溃都从持久状态重放）：
   ...
   c. **transport claim（push 前）**：claim 时 StateStore CAS enqueued→`in_flight`，同事务再核 carrier generation 相符、run 非终态、demand 有效（未被 superseded/cancel）；投影未知或落后 fail closed，不推给模型。in_flight 是一个 durable 的「模型正在这一轮里」标记，从 push 起一直到 turn 结束回执。这一步挡住「CommDB 入队后、StateStore 里 demand 被取消 / run 终结 / carrier 换代」的窗口。
   ...
   e. **in_flight 期间的失效 = 两阶段 fence，转授/终态在旧 turn 排空或被精确停机确认之前不能提交**。TURN 合同要求任一时刻只有 holder 能碰 worktree，而 writer profile 是 workspace-write + network，撤销提交凭证挡不住本地文件 / Git / 外部副作用，所以「让旧 turn 跑完再说」不是 fence。规则：
      - 触发者（TURN transfer、demand cancel、整单终态、carrier 换代）先在 StateStore 写一条 durable `workflow_invalidation_operation`（server 派生 operation_id、kind、目标、fencing delivery_id，stage=requested），同事务 CAS in_flight→`fencing` 并把该 exec 标为 intake 已关闭；旧 holder 之后的任何 reserve 在 a 步被 source facts 拒绝。Bridge 重启后从这条记录续跑，不依赖内存调用者。
      - **transfer 不是一个新协议，就是下一位 holder 自己那条合法需求的生产 grant 路径**（新 requestId 的 rework/phase-wake → coordinator reentry → worktree → holder activation → admission → 凭证轮换 → `grantTurn`）。本设计只在 coordinator 调 `grantTurn` **之前**插入一道 fence 检查：若当前 holder 的 exec 有 in_flight delivery，则先写上述 invalidation operation（kind=transfer），等 fence 完成（下一条）后再继续原路径；`grantTurn` 仍是单个 IMMEDIATE 事务、一次到位改 holder/epoch、写 `runner_workflow_activation` / `turn_source_history` / `workflow_source_event`，凭证明文只存在于这一次调用的内存里，不需要 CommDB 侧任何 pending 记录。fence 与 `grantTurn` 之间 Bridge 重启：rework request 与 operation 都是 durable 的，coordinator 重入后重新核 fence 已完成再调 `grantTurn`，source event 回放冻结保证不会双 grant。两个并发 transfer 都会挂在同一 fencing delivery 的 operation 上（operation 对 fencing delivery 唯一，第二个 transfer 复用而非新建），fence 完成后由 `grantTurn` 的 IMMEDIATE 事务串行化，后到者看到 holder 已变则按现有 coordinator 规则处理自己的请求。
      ...
      - 只有 delivery 到达终态（consumed/aborted）并投影到 CommDB 后，operation 才能 committed：transfer 的 coordinator 才能继续调 `grantTurn`、整单终态才能 close carrier、新 writer 才能被启用。转授/终态因此**永远不会**与旧 turn 的写并发；代价是转授最多等一个 turn drain 或一次有界 stop，这是接受的。转授提交后，被转走的 demand 在同一 StateStore 事务里终结为 `superseded_by_transfer`（其活动 delivery 已在 fence 中到终态；episode 内已花掉的尝试留痕，不退不重置）。
   ...
   | claim（push 前） | enqueued → in_flight | 同事务核 generation / run / demand 有效；不符则不 push，CAS→cancelled |
...
| F | Bridge 分别在 intent、投影、stop、退出、spawn、身份确认、grant 后首条输入消费前、投递后重启/进程丢失；生产 grant 路径三处：凭证轮换后 `grantTurn` 前、`grantTurn` 提交后 receipt CAS 前、G+1 换代凭证刷新后新载体启动前 | 每点都只有一个身体；回执可补写；旧代数 kill/finally 不生效；模型不双消费；grant 后载体丢失→清理确认→同 demand 以新 generation 重试可继续且**不重新 grant**（同 activation/epoch，新 receipt 行、凭证刷新），预算不重置；三处崩溃后恢复：恰一个 source event/epoch（`grantTurn` 回放冻结）、receipt CAS 幂等、当前 epoch 的 `runner_workflow_activation` 与 StateStore live 凭证行一致，旧凭证已撤销，episode 预算不重置 |
