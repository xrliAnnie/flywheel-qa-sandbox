...
| grant receipt（需求 episode 的追加式不可变子记录，仅 writer；主键 (demand_id, carrier_generation, grant_id)） | grant_id（同 demand 内单调 ordinal）、granted_activation_id、granted_turn_epoch、carrier_generation、grant request id、grantedAt；每条只能写一次（CAS 从空到值），不能更新；demand 行上有 `current_grant_id` 指针（CAS 推进）。同一 demand 在新载体代数或新 grant episode 下追加新记录，旧记录保留但不再是投递候选 | §5.3 第 3 步在原工作流真实 grant 之后封存。每个 grant episode 都是一次**新 activation**：coordinator 用 `activation:${requestId}:g${grant_id}` 作 activation_id、`rework-turn:${requestId}:${activationId}` 作 TURN source event——基线 `grantTurn` 对同一 source event 回放只返回冻结的旧 epoch，所以 source event 必须含 grant_id 才能真的拿到新 epoch；`workflow_activation_turn` 以 activation_id 为主键、no-update，新 activation 即追加新行，execution binding (execution_id, run_id, node_id, attempt) 不变。新 activation 的输出凭证按 §5.4 刷新，旧 activation 凭证失效。grant 后、首条输入消费前载体死亡→清理确认→同 demand 以 G+1 重试，或 receipt 后 TURN 被转授→`turn_stale`，都开新 grant episode，不重置 episode 预算。创建需求时不存在未来的 epoch，所以不在创建时冻结、不猜、不事后回填 |
| delivery authorization（一行/首条输入投递，StateStore） | delivery_id PK、demand_id、carrier_generation、grant_id、receipt 引用、envelope digest、state = reserved/enqueued/turn_stale/cancelled/consumed、updated_at；同 demand 同时只允许一条活动（reserved/enqueued）delivery；turn_stale/cancelled/consumed 都是该 delivery 的终态 | §5.3 第 4 步 a 预留、d 消费；失效操作（demand cancel / 整单终态 / carrier 换代）在各自 StateStore 事务里把该 demand/exec 所有活动 delivery CAS→cancelled，永远不会被一次 reserve 阻止；闭集转换见 §5.3 表；CommDB 项与 transport claim 都以 delivery_id 关联并回核此行 |
...
4. 只有第 3 步结束后，才把 pending envelope（基线→当前 HEAD 的提交/文件变更摘要与 dirty 差异，见 §5.2；原会话缓存的是退下时刻的文件内容，fresh 会重新读盘而 resume 不会）与原需求投递为首条业务输入。demand / carrier / receipt 在 StateStore，当前 TURN holder+epoch 与 `turn_wake_outbox`/mailbox 在 CommDB，两库没有跨库事务（§2.2），所以这一步**不是**一个「同时重核四项的入队事务」，而是下面的双库线性化协议（每一步以唯一 `delivery_id` 幂等，任一阶段崩溃都从持久状态重放）：
   ...
   a. **StateStore 预留（CAS INSERT）**：对当前 carrier generation、当前 grant_id 的 receipt 插入一条 delivery authorization（`delivery_id`、demand_id、carrier_generation、grant_id、receipt 引用、envelope digest、state=reserved）。同一事务里读取并核 source facts：demand 未取消未过期、run 非终态、carrier generation 未变、(generation, grant_id) 的 receipt 存在、该 demand 没有其它活动 delivery；任一不成立则不插入。已经发生的 cancel / 终态 / 换代因此天然阻止后续 reserve——它们改的是 demand/run/carrier 这些 source 行，不需要 delivery 行先存在。预留同事务写入 durable outbox（复用 park outbox 的投影链路，v2 事件）。
   ...
   c. **transport claim / 首次模型消费前**：claim 该项时再核 StateStore 投影里的 delivery 状态 = enqueued 且 carrier generation 相符、run 非终态、demand 有效；投影未知或落后 fail closed，不推给模型。这一步挡住「CommDB 入队后、StateStore 里 demand 被取消 / run 终结 / carrier 换代」的窗口。
   d. **ACK 与回投**：模型消费回执按 `delivery_id` 幂等地把 StateStore 的 delivery CAS enqueued→consumed（重复 ACK no-op）；若该行已 cancelled，ACK 记为 `late_ack` no-op。StateStore 若已 cancelled，CommDB/transport 侧的项在下一次投影或 claim 时丢弃。Bridge 在 a 后、b 后、push 后、consume 后任一点崩溃，重启都按 delivery_id 从两库持久状态续跑，不会产生第二条 delivery，也不会把陈旧 envelope 推给模型。
   ...
   | consume（ACK） | enqueued → consumed（终态） | 同事务再核 demand 有效、run 非终态、generation 相符；不符则 CAS→cancelled 并记 `consumed_after_invalidation` |
   | 迟到的 projector / ACK / enqueue 回投 | 任一终态 → 不变 | 一律 no-op，只留审计 |

   cancel 与 consume 的顺序由这一行决定：consume 的 CAS 先提交则 delivery 为 consumed，之后的 cancel 对该 delivery no-op，但仍在同事务撤销该 activation 的输出/提交凭证，模型这一轮的产物无法提交（§5.4）；cancel 先提交则 ACK 变 `late_ack`。claim fence（c）把「已推给模型但随后失效」的暴露收窄到一个模型 turn，且其输出被撤销凭证 fenced，这是本设计接受并可审计的残余，不是静默放行。
...
| P | 普通 mailbox 坏 handle、gate response 坏 handle、TURN 已转授、需求创建后 TURN 变化再恢复、receipt 封存后 TURN 转授、conversation_only 载体上到达 writer 需求；投递协议每个缝隙：StateStore 预留→CommDB 入队之间、CommDB 入队→push/consume 之间分别发生 TURN transfer / cancel / 整单终态 / carrier generation change / Bridge crash；cancel/终态**先于** reserve 与 reserve 成功**之后**再 cancel/终态/换代两组；G 代 receipt(epoch E)→转授到 E+1→demand 仍有效且权威重新可得 | conversation_only 不产生 writer/不激活 holder 但仍有无写权限兜底；HEAD/dirty 摘要不在 TURN CAS 前进入模型；writer 投递只匹配当前 generation+grant_id 的真实 receipt；CommDB 入队核 exact turn 元组，转授在同库事务取消旧 epoch 未消费项；transport claim 回核 StateStore 投影，未知/落后 fail closed；每种缝隙里陈旧输入都不进模型、delivery_id 不重复、episode 预算不重置；cancel 先于 reserve → reserve 被 source facts 拒绝；reserve 后 cancel/终态/换代 → 活动 delivery 必被 CAS→cancelled，整单终态不因 reserve 失败；转授后新 grant episode 产生新 activation/epoch/receipt/delivery_id 且恰好消费一次，旧 source replay、旧 receipt、旧凭证不能复活；每条 receipt set-once 不可改；conversation_only → writer 不原地扩权，必须重建新代载体；崩溃后按 delivery_id 重放不双发 |
