...
| 3 | turn-belt-reconcile；workflow-engine-dispatcher 与 rework/launch admission；CommDB `grantTurn` 转授路径 | 无进程且正常待命不判 STALE；合法阶段交接照常推进 TURN；恢复者必须重新 turn 查询；转授对 in_flight 的 holder 先落 `transfer_pending`，旧 turn 排空/停机确认后才提交（§5.3 e） | 旧 manifest 中 TURN 不可复活；同目录两需求不能双写；转授不得与旧 holder 的模型 turn 并发 |
...
3. 按需求 episode 的 `authority_mode` 取权限：writer 需求在此处走原工作流 holder 激活路径（reentry → worktree → holder activation → admission → `grantTurn` → 记录 turn，顺序不变），每次进入本步都是一个新的 grant episode：demand 行 `current_grant_id` CAS +1 得到 grant_id；activation 与 execution binding **沿用既有的那一个**（reentry/worktree/holder activation/admission 对已有 binding 幂等通过），只有 TURN source event 换成含 grant_id 的 `rework-turn:${requestId}:${activationId}:g${grant_id}`（这样基线 `grantTurn` 的 source-event 回放冻结不会把旧 epoch 还回来），真实 grant 成功后把 activation_id / source_event_id / granted_turn_epoch / carrier_generation / credential_nonce 以 set-once CAS 追加到 `workflow_turn_grant_episode`（主键 (demand_id, grant_id)），并按 §5.4 以该 nonce 轮换输出凭证；失败则 hold 并显示等待 TURN，本 grant_id 无记录。conversation_only 需求不做 TURN 申请，也不激活 holder，没有 receipt。载体在 grant 后、首条输入消费前死亡的，清理确认后以 G+1 重试时重新走本步（新 grant_id）；receipt 封存后 TURN 被转授（第 4 步 b 报 `turn_stale`）且 demand 仍有效、权威重新可得时，同样重新走本步开新 grant episode——旧 receipt、旧 source event 回放、旧 activation 凭证永远不能复活。以上都不重置 episode 预算。若 Lead/产品将来决定「转授即永久取代当前需求」，须改为明确终结该 demand 并对后续需求沿用同一 episode 预算，本设计不采用该选项。
4. 只有第 3 步结束后，才把 pending envelope（基线→当前 HEAD 的提交/文件变更摘要与 dirty 差异，见 §5.2；原会话缓存的是退下时刻的文件内容，fresh 会重新读盘而 resume 不会）与原需求投递为首条业务输入。demand / carrier / receipt 在 StateStore，当前 TURN holder+epoch 与 `turn_wake_outbox`/mailbox 在 CommDB，两库没有跨库事务（§2.2），所以这一步**不是**一个「同时重核四项的入队事务」，而是下面的双库线性化协议（每一步以唯一 `delivery_id` 幂等，任一阶段崩溃都从持久状态重放）：
   ...
   e. **in_flight 期间的失效 = 两阶段 fence，转授/终态在旧 turn 排空或被精确停机确认之前不能提交**。TURN 合同要求任一时刻只有 holder 能碰 worktree，而 writer profile 是 workspace-write + network，撤销提交凭证挡不住本地文件 / Git / 外部副作用，所以「让旧 turn 跑完再说」不是 fence。规则：
      - 触发者（TURN transfer 请求、demand cancel、整单终态、carrier 换代）先在 StateStore CAS in_flight→`fencing`（记录触发原因、请求 id），同事务把该 exec 标为 intake 已关闭；CommDB 侧 `grantTurn` 转授请求只落成 `transfer_pending`（holder 与 epoch不变），此时旧 holder 的任何新 delivery 在 b 步都被拒。
      ...
      - 只有 delivery 到达终态（consumed/aborted）并投影到 CommDB 后，`transfer_pending` 才能提交为真正的转授（epoch+1）、整单终态才能 close carrier、新 writer 才能被启用。转授/终态因此**永远不会**与旧 turn 的写并发；代价是转授最多等一个 turn drain 或一次有界 stop，这是接受的。
   ...
   | 失效触发（transfer / cancel / 终态 / 换代）遇 in_flight | in_flight → fencing | 同事务关闭 intake、记录触发者；CommDB 转授只落 `transfer_pending`，不改 holder/epoch |
   | drain 或精确同代 stop 确认 | fencing → consumed（drain）/ aborted（stop） | 确认后投影到 CommDB；只有此后 `transfer_pending` 才提交、终态才 close carrier、新 writer 才启用 |
