...
- **触发**：**只看当前号 5h 已用 ≥ 90%**（运行时可调）。weekly **永不当阈值触发器**——保证每个号的 weekly 钱花完，绝不在 95-99% 时提前弃号。边界解读（工程判断，已向 Lead 标注）：weekly **实际封顶（≥100%）** 是事实性死号而非阈值——仍触发切号，否则慢烧型 weekly 先尽时全 fleet 卡死且 5h 触发器永不发火。
...
6. Annie 第三轮终版拍板（2026-07-14 经 Lead 转达，明示最后一轮、折完直接定稿不出 v4）：
   ...
   - 流程令：本轮折入后**直接定稿 plan.md + 跑 Codex design review**，不再出 v4 给她。
