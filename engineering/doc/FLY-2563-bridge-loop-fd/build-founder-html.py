...
SECTIONS = [
 ...
 ('cause','02 / 两个问题，都需要修',
  table(['问题','这次查到的证据','计划修法'],[
   ...
   ('连接名额耗尽','代码中确认了两处数据库打开后未关闭，其中一处会按项目重复执行。','操作完成就关闭；异常和网络挂起也检验关闭情况。'),
   ('旧归档扫描风险','现有时间预算只能在语句之间检查，不能打断一条慢查询。','先取有限候选，再检查是否能归档，保存已检查位置。')])+
  p('索引是数据库的检索目录，能减少查找范围；仅补索引仍未达到这次要求的 50ms。fd 是进程持有文件和网络连接的系统名额；提高名额到至少 8192 的同时，也必须修掉不关闭连接的问题。')),
 ('model','03 / 记住读到哪，不改谁有权决定',
  ...
  p('让卡片进入终态的那件事件仍处理一次；处理过的证据不反复扫描。旧 holder 表继续保留，它保存的是审批卡的稳定身份，回放与复盘仍可读取。')),
 ('tradeoffs','04 / 选择与舍弃',
  table(['选择','收益','代价与边界'],[
   ('有界候选页 + 持久化进度','处理量随新事件前进；异常可接续。','要处理跨页、时间乱序和重放；这些都列入回归。'),
   ('及时关闭连接 + 提高软限','减少泄漏，也给正常峰值留空间。','提高软限只对新启动进程生效，部署由独立更新服务执行。'),
 ...
 ('qa','05 / 怎样证明问题修好了',
  table(['检查','通过线','证据来源'],[
   ...
   ('连接名额','上限至少 8192；超过 80% 发日志和统一告警。','健康页显示实际使用量、上限、样本时间。'),
