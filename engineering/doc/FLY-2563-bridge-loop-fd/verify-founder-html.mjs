...
f.rows[0].value = '想看学习的边界'; f.rows[0].events.input();
assert.equal(f.saved.get('flywheel:design-comments:/test/one:flow'), '想看学习的边界');
assert(f.chunks.children[0].children[0].textContent.includes('01 / 让查账每次只做一小步\n想看学习的边界'));
let reload = fixture({saved:f.saved}); assert.equal(reload.rows[0].value, '想看学习的边界');
...
for (const clipboard of ['reject','absent']) {
  ...
  assert.equal(denied.savedLabels.get('flow').textContent,'未保存；仍可复制');
}
...
console.log('PASS: artifact shape, 7 section comments, one nonce script, no external assets/inline handlers, pathname storage, live summary, 1800-character Unicode chunks, clipboard success/rejection/absence/failure, storage denied, clear/reload isolation.');
console.log('LIMIT: no browser rendering or real CSP execution verified by this VM fixture.');

// Optional hosted verification: publishing alone is not execution evidence.
if (process.argv[2]) {
  const response = await fetch(process.argv[2]);
  assert.equal(response.status, 200);
  const served = await response.text();
  assert(!served.includes('__CSP_NONCE__'));
  const hostedScripts = [...served.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/g)];
  assert.equal(hostedScripts.length,1);
  const nonce=hostedScripts[0][1].match(/nonce="([^"]+)"/)?.[1];
  assert(nonce);
  const policy=response.headers.get('content-security-policy') || served.match(/<meta[^>]*http-equiv="Content-Security-Policy"[^>]*content="([^"]+)"/i)?.[1];
  assert(policy?.includes("'nonce-"+nonce+"'"));
  assert.equal(hostedScripts[0][2],scripts[0][2]);
  assert(served.includes('让查账每次只做一小步'));
  assert.doesNotMatch(served, /\bon[a-z]+\s*=/i);
  assert.doesNotMatch(served, /<(script|link|img|iframe)[^>]+(?:src|href)=/i);
  console.log('PASS: hosted HTTP 200, replaced nonce, matching CSP/script, expected content, no external assets/inline handlers.');
}
