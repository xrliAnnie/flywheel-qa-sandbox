...
const { StateStore } = await import(`${WT}/packages/teamlead/dist/StateStore.js`);
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F8a — CommDB-only orphan (issue_id=NULL in CommDB, NO StateStore row) [CommDB-only]");
  ...
  let threw = false;
  try {
    await orch.reconcileQaLoss({ issueId: "FLY-QA1070", terminalExecId: "d2f31930-orphan" });
  } catch {
    threw = true;
  }
  check("does not throw", !threw);
  check("zero spawn (getSession→undefined → main-role default no-op)", h.calls.start.length === 0);
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  const orch = new PhaseOrchestrator(h.deps);
  await orch.reconcileQaLoss({ issueId: "FLY-QA1070", terminalExecId: "main-dead-1" });
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F8c-1 — issue_id='' dead qa row, scoped + boot → no crash, zero spawn [real-store]");
  const h = await makeRealHarness();
  h.store.upsertSession({
  ...
  let threw = false;
  try {
    await orch.reconcileQaLoss({ issueId: "", terminalExecId: "qa-empty-issue" });
...

{
  ...
  let threw = false;
  try {
    await orch.reconcileQaLoss({ issueId: "OTHER-77", terminalExecId: "qa-cross-proj" });
    await orch.reconcileOnStartup();
  } catch {
    threw = true;
  }
  check("does not throw", !threw);
  check("zero spawn (no stranded implement → no-op)", h.calls.start.length === 0);
  check("zero alerts", h.calls.alerts.length === 0);
  h.cleanup();
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F8c-3 — listPhaseSessionRows throws (qa query) → fail-closed, zero spawn, warn [fault-injected]");
  const h = await makeRealHarness({
  ...
  let threw = false;
  try {
    await orch.reconcileQaLoss({ issueId: "FLY-QA1070", terminalExecId: "qa-dead-1" });
  } catch {
    threw = true;
  }
  check("does not throw (caught inside criteria)", !threw);
  check("zero spawn (fail-closed: treated as progressed)", h.calls.start.length === 0);
  check(
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F8d — cross-project stranded implement IS traversed by boot reconcile, criteria no-ops [real-store]");
  const h = await makeRealHarness();
  h.store.upsertSession({
  ...
  h.store.setReviewBinding("impl-other", { questionId: "q-other", prHeadSha: HEAD });
  ...
  check(
    ...
    h.logs.some((l) => l.includes("impl-other") && l.includes("pipeline still owns itself")),
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("FLY-1018 — impl@awaiting_review + 2 dead failed qa (no intent) → exactly 1 respawn [real-store]");
  const h = await makeRealHarness();
  seedImpl(h.store);
  ...
  const orch = new PhaseOrchestrator(h.deps);
  await orch.reconcileOnStartup();
  check("exactly 1 spawn", h.calls.start.length === 1, `start=${h.calls.start.length}`);
  ...
  check("respawn thread note posted (dead-qa 前科 + alive row landed)", h.calls.threadNotes.length === 1);
  const turn = h.getTurn("FLY-QA1070");
  check("TURN granted to fresh QA via pre-launch seam", turn?.holder_exec_id === "qa-new-1");
  check("zero alerts", h.calls.alerts.length === 0);
  h.cleanup();
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F9 — production merge_block marker string truncates respawn on BOTH paths [real-store]");
  const MARKER = "merge_without_approval:review_question_unbound/qa_snapshot_missing_exempt";
  const h = await makeRealHarness();
...
// ─────────────────────────────────────────────────────────────────────────────
{
  section("F10-1 — FLY-1023 现状形态(生产快照重构):无 implement@awaiting_review → 零触发 [real-store]");
  ...
  const h = await makeRealHarness();
  const MARKER = "merge_without_approval:review_question_unbound/qa_snapshot_missing_exempt";
  h.store.upsertSession({ execution_id: "321fb0cd", issue_id: "FLY-1023", project_name: PROJ, status: "completed", session_role: "design", chat_thread_role: "design" });
  h.store.upsertSession({ execution_id: "9b4838c3", issue_id: "FLY-1023", project_name: PROJ, status: "completed", session_role: "implement", chat_thread_role: "implement" });
  h.store.setReviewBinding("9b4838c3", { questionId: "q-1023", prHeadSha: HEAD });
  h.store.upsertSession({ execution_id: "f561baa4", issue_id: "FLY-1023", project_name: PROJ, status: "completed", session_role: "qa", chat_thread_role: "qa" });
  h.store.setMergeBlock({ executionId: "f561baa4", reason: "merge_without_approval:gate_not_answered", head: HEAD });
  h.store.upsertSession({ execution_id: "3f8be4bb", issue_id: "FLY-1023", project_name: PROJ, status: "terminated", session_role: "implement", chat_thread_role: "implement" });
  h.store.setMergeBlock({ executionId: "3f8be4bb", reason: MARKER, head: HEAD });
  const orch = new PhaseOrchestrator(h.deps);
  await orch.reconcileQaLoss({ issueId: "FLY-1023", terminalExecId: "f561baa4" });
  await orch.reconcileOnStartup();
...
{
  section("F10-2 — FLY-1047 现状形态(生产快照重构):无 implement@awaiting_review → 零触发 [real-store]");
  const h = await makeRealHarness();
  h.store.upsertSession({ execution_id: "ad172522", issue_id: "FLY-1047", project_name: PROJ, status: "completed", session_role: "design", chat_thread_role: "design" });
  h.store.upsertSession({ execution_id: "a0d9163f", issue_id: "FLY-1047", project_name: PROJ, status: "completed", session_role: "implement", chat_thread_role: "implement" });
  h.store.setReviewBinding("a0d9163f", { questionId: "q-1047", prHeadSha: HEAD });
  h.store.setMergeBlock({ executionId: "a0d9163f", reason: "merge_without_approval:response_not_structured", head: HEAD });
  h.store.upsertSession({ execution_id: "c05e6ab8", issue_id: "FLY-1047", project_name: PROJ, status: "terminated", session_role: "qa", chat_thread_role: "qa" });
  h.store.upsertSession({ execution_id: "b7d7adf1", issue_id: "FLY-1047", project_name: PROJ, status: "terminated", session_role: "implement", chat_thread_role: "implement" });
  const orch = new PhaseOrchestrator(h.deps);
  await orch.reconcileQaLoss({ issueId: "FLY-1047", terminalExecId: "c05e6ab8" });
  await orch.reconcileOnStartup();
