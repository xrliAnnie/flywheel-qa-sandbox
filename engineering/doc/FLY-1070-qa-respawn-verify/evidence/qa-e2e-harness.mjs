...
async function makeEnv(opts = {}) {
  ...
  const calls = { start: [], alerts: [], threadNotes: [], parks: 0, wakes: 0, closes: 0 };
  const logs = [];
  const warns = [];
  const ALIVE = new Set(["running", "awaiting_review", "approved_to_ship", "design_done"]);
  let spawnSeq = 0;
  ...
  const deps = {
    startDispatcher: {
      start: async (req) => {
        ...
        withComm((db) => db.grantTurn(req.issueId, execId, req.sessionRole, Date.now()));
        store.upsertSession({
          execution_id: execId,
    ...
    listStrandedImplementPhases: () => store.getStrandedImplementPhaseSessions(),
    listPhaseSessionRows: (issueId, phase) =>
      store.getPhaseSessionsForIssue(issueId).filter((s) => s.chat_thread_role === phase),
    ...
    turnBelt: {
      listTurns: () => withComm((db) => db.listTurns().map((turn) => ({ projectName: PROJ, turn }))),
      ...
      getPhaseSessionsForIssue: (issueId) => store.getPhaseSessionsForIssue(issueId),
    },
    qaVerdicts: {
  ...
  const seedImpl = (over = {}) => {
    ...
    store.setReviewBinding(row.execution_id, { questionId: "q-real-1", prHeadSha: HEAD });
    return row;
  };
  ...
  return {
    ...
    seedImpl, seedQa, postAction, postEvent, getTurn, withComm, cleanup, baseUrl,
    transitionOpts,
  };
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E1 — /api/actions terminate live QA → event-driven respawn (epoch+1, belt quiet)");
  ...
  check("row flipped terminated", env.store.getSession("qa-0")?.status === "terminated");
  const spawned = await waitFor(() => env.calls.start.length === 1);
  check("event-driven respawn: exactly 1 start (no boot reconcile ran)", spawned, `start=${env.calls.start.length}`);
  if (spawned) {
    ...
    check("ignoreRunnerLabelSelection=true", req.ignoreRunnerLabelSelection === true);
  }
  await waitFor(() => env.getTurn(ISSUE)?.holder_exec_id === "qa-new-1");
  const post = env.getTurn(ISSUE);
  check("CommDB TURN holder = fresh QA", post?.holder_exec_id === "qa-new-1", JSON.stringify(post));
  check(`epoch strictly +1 (${pre?.epoch} → ${post?.epoch})`, post?.epoch === (pre?.epoch ?? 0) + 1);
  await silence();
  check("zero STALE-TURN alerts (silence 800ms)", !env.calls.alerts.some(isStaleAlert), JSON.stringify(env.calls.alerts));
  check("zero alerts overall", env.calls.alerts.length === 0, JSON.stringify(env.calls.alerts));
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  check("terminate HTTP 200", res.status === 200);
  const spawned = await waitFor(() => env.calls.start.length === 1);
  check("respawn via dashboard mount", spawned, `start=${env.calls.start.length}`);
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E3a — terminate success + live-ghost probe → ghostGuard blocks spawn, TURN untouched");
  const env = await makeEnv({
    ghostLivenessFor: (row) => (row.execution_id === "qa-0" ? "alive" : "absent"),
  });
  env.seedImpl();
  ...
  const pre = env.getTurn(ISSUE);
  const res = await env.postAction("/api/actions", { execution_id: "qa-0" });
  check("terminate HTTP 200 (tmux lookup gone → success shape)", res.status === 200);
  check("FSM row flipped terminated", env.store.getSession("qa-0")?.status === "terminated");
  const alerted = await waitFor(() => env.calls.alerts.length >= 1);
  check("qa-loss fired → ghostGuard fail-closed alert", alerted, JSON.stringify(env.calls.alerts));
  check(
    "alert names the live ghost (refusing duplicate spawn)",
    env.calls.alerts.some((r) => /LIVE|possibly-live/.test(r) && r.includes("qa-0")),
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E3b — REAL cleanupPending (corrupt CommDB read) → qa-loss STILL fires → respawn");
  ...
  try {
    ...
    writeFileSync(join(fakeHome, ".flywheel", "comm", PROJ, "comm.db"), "NOT A SQLITE DB");
    ...
    check("FSM row still flipped terminated", env.store.getSession("qa-0")?.status === "terminated");
    const spawned = await waitFor(() => env.calls.start.length === 1);
    check("qa-loss fired on the cleanupPending shape → respawn", spawned, `start=${env.calls.start.length}`);
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E4a — DirectEventSink.emitFailed(qa) → respawn (qa-loss before belt)");
  const env = await makeEnv();
  env.seedImpl();
  env.seedQa("qa-0");
  env.withComm((db) => db.grantTurn(ISSUE, "qa-0", "qa", Date.now()));
  await env.sink.emitFailed(
    { executionId: "qa-0", issueId: ISSUE, projectName: PROJ, sessionRole: "qa" },
    "killed in OOM",
  );
  const spawned = await waitFor(() => env.calls.start.length === 1);
  ...
  await waitFor(() => env.getTurn(ISSUE)?.holder_exec_id === "qa-new-1");
  check("TURN holder = fresh QA (belt reconcile no-oped via guard 1)", env.getTurn(ISSUE)?.holder_exec_id === "qa-new-1");
  await silence();
  check("zero alerts", env.calls.alerts.length === 0, JSON.stringify(env.calls.alerts));
  await env.cleanup();
}

{
  ...
  check("sessionRole=qa @ head", env.calls.start[0]?.sessionRole === "qa" && env.calls.start[0]?.startPoint === HEAD);
  await silence();
  check("zero alerts", env.calls.alerts.length === 0);
  await env.cleanup();
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E5 — crash-reaper reaps stale QA → onQaPhaseTerminated closure → respawn");
  const env = await makeEnv();
  env.seedImpl();
  ...
  const reapDeps = {
    ...
    onQaPhaseTerminated: (executionId, issueId) => {
      void env.holder.current
        ...
        .catch((err) => console.warn(`qa-loss reconcile failed: ${err.message}`));
    },
  };
  ...
  check("row flipped terminated", env.store.getSession("qa-0")?.status === "terminated");
  const spawned = await waitFor(() => env.calls.start.length === 1);
  check("respawn triggered by reaper hook", spawned, `start=${env.calls.start.length}`);
  await env.cleanup();
}
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E6 — cap storm: kill → respawn ×2 (epoch increments), 3rd dead row → failClosed, re-trigger re-alerts");
  const env = await makeEnv();
  env.seedImpl();
  env.seedQa("qa-0");
  ...
  check("round1: epoch 2, holder qa-new-1", (() => { const t = env.getTurn(ISSUE); return t?.epoch === 2 && t?.holder_exec_id === "qa-new-1"; })(), JSON.stringify(env.getTurn(ISSUE)));
  // round 2
  await env.postAction("/api/actions", { execution_id: "qa-new-1" });
  check("round2: respawn", await waitFor(() => env.calls.start.length === 2));
  check("round2: epoch 3, holder qa-new-2", (() => { const t = env.getTurn(ISSUE); return t?.epoch === 3 && t?.holder_exec_id === "qa-new-2"; })(), JSON.stringify(env.getTurn(ISSUE)));
  // round 3 → 3rd dead row → cap
  await env.postAction("/api/actions", { execution_id: "qa-new-2" });
  const capAlerted = await waitFor(() => env.calls.alerts.some((r) => r.includes("respawn cap")));
  check("round3: failClosed cap alert", capAlerted, JSON.stringify(env.calls.alerts));
  await silence();
  check("round3: ZERO further spawn (start stays 2)", env.calls.start.length === 2, `start=${env.calls.start.length}`);
  const alertsBefore = env.calls.alerts.filter((r) => r.includes("respawn cap")).length;
  // re-trigger through a second production path (emitFailed on the dead row)
  await env.sink.emitFailed(
    { executionId: "qa-new-2", issueId: ISSUE, projectName: PROJ, sessionRole: "qa" },
    "re-trigger",
  );
  const reAlerted = await waitFor(
    () => env.calls.alerts.filter((r) => r.includes("respawn cap")).length === alertsBefore + 1,
  );
  check("re-trigger → cap re-alerts (discrete-event semantics)", reAlerted, JSON.stringify(env.calls.alerts));
  await silence();
...
// ─────────────────────────────────────────────────────────────────────────────
{
  scenario("E7 — escape hatch =0: scoped inert + boot reverts row-exists + stranded-pass hardening NOT gated");
  ...
  try {
    ...
    check("terminate HTTP 200", res.status === 200);
    await silence();
    check("(a) scoped reconcileQaLoss inert: zero spawn (silence 800ms)", env.calls.start.length === 0);
    // (b) boot criteria reverts to row-exists → skip (pre-fix behavior)
    await env.orch.reconcileOnStartup();
    await silence();
    check("(b) boot row-exists criteria: zero spawn", env.calls.start.length === 0);
    ...
    check(
      ...
      env.calls.alerts.some((r) => r.includes("terminated") && r.includes("ship gate")),
      JSON.stringify(env.calls.alerts),
    );
    await env.cleanup();
  } finally {
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  await silence();
  check("re-trigger no-op: start stays 1 (alive qa-new-1 owns the pipeline)", env.calls.start.length === 1);
  await env.cleanup();
}

{
  scenario("E8b — two concurrent reconcileQaLoss on the same issue → exactly 1 spawn (in-flight guard)");
  ...
  check("exactly 1 spawn under concurrency", env.calls.start.length === 1, `start=${env.calls.start.length}`);
  await env.cleanup();
}

console.log(`\n${"─".repeat(60)}`);
console.log(`silence window for negative assertions: ${SILENCE_MS}ms; positive poll bound: ${WAIT_MS}ms`);
console.log(failures === 0 ? "ALL E2E SCENARIOS PASS" : `${failures} CHECK(S) FAILED`);
