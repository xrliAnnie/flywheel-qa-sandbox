...
diagram lost its <svg id="FLY-2484-dN"> root, or a diagram lost its inner width
...
for n, name in diagrams.items():
    ...
    if f'id="FLY-2484-d{n}"' not in svg:
        sys.exit(f"diagram {name} lacks svgId FLY-2484-d{n}")
...
leftover = re.findall(r"<!--DIAGRAM_\d+-->|\{\{?[A-Z_0-9]+\}?\}", out)
if leftover:
...
if textareas < 10 or sections < 10:
    sys.exit(f"comment layer incomplete: sections={sections} textareas={textareas}")
if "【页面意见汇总】" not in out or 'ISSUE = "FLY-2484"' not in out:
    sys.exit("summary marker or issue id missing")
# F13: the page's own wording must carry no personal names
for name in ("塔大", "Tadashi", "Honey Lemon", "Annie", "Flavio"):
    if name in out:
        sys.exit(f"personal name in page wording: {name}")
