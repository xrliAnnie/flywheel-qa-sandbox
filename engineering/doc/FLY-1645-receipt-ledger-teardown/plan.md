...
| S2 | `trustedFounderApprovalAndReceipt`(:2962)/ `trustedFounderGateResponseAndReceipt`(:3035)/ `respondAndReceipt`(:2866)/ `respond` 动词 | 前两者保 gate-response + founder-attribution 事务、摘 settle、更名去 `AndReceipt`。**`respond` 现状对非 approve_to_ship 问题缺守卫**(Codex R1 #4:`insertResponse` db.ts:1576-1612 不查 `resolved_at`/`superseded_at`/过期/终态——已退休问题可被迟到回答):`respond` 改用从 `insertResponseIfGateOpen` 泛化的**守卫式立即事务响应原语**。契约端到端(Codex R2 #4):新入参 `--expect-owner <question.from_agent,即提问方>` / `--expect-checkpoint <kind>`(owner 语义对齐既有原子 gate writer:owner = 提问的 runner/agent[from_agent],不是收件 Lead[to_agent])(founder 路由场景另有 `--expect-project/--expect-issue`),值来源 = gate-poller handoff payload 携带的候选问题冻结快照(S8 文本同步给出);事务内重检谓词 = 问题存在 + owner/checkpoint 与期望一致 + 非排除类(report/review 门/approve_to_ship 走 founder 专路)+ 未答(无 response)+ 未 resolved/superseded/过期/终态。**语义声明**:普通 Lead `respond` 由此获得同一守卫(普遍加强);本单**不声称**复刻 routeFounderReply 整台路由状态机——冻结 scope 绑定仅在 handoff 提供期望值时生效,这是有意的收敛并如实记录。原 routeFounderReply 测试的 stale/scope/竞态断言(候选在选中与写入之间被答/被退休)移植到新原语测试 |
...
7. **N1 触发器安装(与 legacy 迁移解耦,Codex R2 #2)**:触发器**不进 `MAILBOX_CORE_SCHEMA`**(`migrateLegacyDatabaseFile` 先装核心 schema 再灌行,进核心会拦死迁移);作为独立安装步(INSERT / UPDATE 两枚分立触发器),仅在两个前提后执行:①closeout sweep 后 `relay_state IN ('open','protected') AND type != 'question'` 集合为空(先清后装,否则拦住 sweep 自己);②迁移侧 relay 归一化规则落地——`mailbox-migration.ts:509-511` 现把 legacy 缺失 relay 映射为 `open`(全类型),改为:question 行保留 legacy 生命周期,**非 question 行 commit 前归一化为 `terminal_disposed`**(对「迁移 byte 保真」承诺的显式修订,T1 同步测 question 保留 + 非 question 归一化,含 legacy 缺失/open 两形态)。
...
2. **S3 前置(external 行收敛真值表,Codex R2 #1——markDead 满足不了后置条件[DEAD 仍匹配集合且对保留谓词可见],故整表不用 markDead)**:
   - `xdept:*`(存活 lane)→ 一行不动;
   - `chat:*` 任意非 ACKED(含 QUEUED/LEASED/DEAD;settlement 有无均同)→ **ACKED**(sweep provenance 落 acked_at 语义注记;FLY-1646 已让 delivered 标记可收 DEAD 行,同款谓词)——其中 12 条未投递 QUEUED 行先逐条人工 review(founder 内容);
