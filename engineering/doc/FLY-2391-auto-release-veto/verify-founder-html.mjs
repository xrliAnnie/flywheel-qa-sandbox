...
f.rows[0].value = '希望窗口故障时有明确说明'; f.rows[0].events.input();
assert.equal(f.saved.get('flywheel:design-comments:/test/one:summary'), '希望窗口故障时有明确说明');
assert(f.chunks.children[0].children[0].textContent.includes('01 / 健康时默认发布，你想拦时点一次\n希望窗口故障时有明确说明'));
let reload = fixture({saved:f.saved}); assert.equal(reload.rows[0].value, '希望窗口故障时有明确说明');
...
console.log('PASS: artifact shape, section comments, one nonce script, no external assets/inline handlers, pathname storage, live summary, 1800-character Unicode chunks, clipboard success/rejection/absence/failure, storage denied, clear/reload isolation.');
