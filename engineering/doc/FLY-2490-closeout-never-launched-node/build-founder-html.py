...
+ the review-log snippet.

Fails loudly (exit 1) if any placeholder is left, any SVG is missing/too small, a
diagram lost its <svg id="FLY-2490-dN"> root, or the CSP/comment-layer contract is
broken — never ship a visual artifact you did not check."""
...
for n, name in diagrams.items():
    ...
    if f'id="{ISSUE}-d{n}"' not in svg:
        sys.exit(f"diagram {name} lacks svgId {ISSUE}-d{n}")
...
if textareas < 9 or sections < 10:
    sys.exit(f"comment layer incomplete: sections={sections} textareas={textareas}")
if "【页面意见汇总】" not in out or f'ISSUE = "{ISSUE}"' not in out:
