...
- **整机 owner index**:collector 从同一 projects registry 枚举每个合法 `projectName`,
  ...
  owner index 任一非缺失 DB 查询失败时沿用 `.timeout 3000` + 一次重试分类,
  受影响/无法归属的 target 标 `UNAVAILABLE(transient|structural: owner_index_incomplete)`,
  **不得降成 unknown finding**;registry 项目从未生成 comm.db 且无对应 pane 时良性跳过,
  有对应 pane 时则是 structural incomplete。任一实际列出的 Runner pane 都不能因
  owner index 不全而静默省略。
- 默认 socket 还可能出现未收录在当前生产 registry 的 QA/临时 `runner-<name>` session:
  这类 pane 仍完整 capture 并写 `owner=foreign-registry`,但“registry 外”本身不是
  finding;clear 行自动 `action=none result=foreign_registry_clear`,实况异常仍聚合跨界
  上报且禁止本 Lead 按键/唤醒。registered project 的 target 首次从 owner index 消失时
  给 teardown race 一 tick grace(`action=none result=session_terminated`);连续第二 tick
  仍活着且无 owner 才升 `unknown` orphan finding。
...
> 2. **pane 实况** — run: `awk '/^## STEP 2$/{show=1; next} /^## STEP 3$/{show=0} show' "$REPORT_PATH"`。快照必须对步 1 的**每一个** Runner pane 运行一次 5s 有界 `TMUX= tmux capture-pane -p -S - -t <pane_id>`;零抽样、零 Lead 范围裁剪、零 `tail -40`。脚本不落原文,只落 `PANE_EVIDENCE pane=<id> target=<session:window_id> owner=owned|cross-boundary|foreign-registry|unknown exec=<execution-id|none> capture_sha256=<sha> lines=<n> bytes=<n> state_sha256=<sha> last_change_epoch=<epoch> findings=<csv|none> action=<none|REQUIRED> result=<clear|UNSET>`。`target` 用 projects registry 枚举的所有项目只读 CommDB 的 `comm.sessions.tmux_window` 构建 machine-wide index,并精确映射 `execution_id/lead_id/project_name`:唯一命中当前 Lead = owned;唯一命中其他 Lead/项目 = cross-boundary(正常态不算 finding);session project 不在 registry = foreign-registry(存在本身不告警,但实况仍全查);registered target 首次零命中按 teardown race 自动记 `session_terminated`,连续第二 tick 仍零命中才是 unknown orphan finding;多重命中 = `UNAVAILABLE(structural: session_target_ambiguous)`。任一 owner-index DB 非缺失读失败都让无法归属 target 标 `UNAVAILABLE(transient|structural: owner_index_incomplete)`,禁止铸 unknown。清单 pane 数、无条件存在的 `pane_count` 与 evidence 行数必须相等,任何 capture 失败或漏行都把步 2 定为 `UNAVAILABLE(structural: pane_capture_incomplete)`。
>
>   逐 pane 判据与处置命令:
...
- Founder 增量里的「每个 pane」在本计划操作化为**每个 canonical Runner pane**:
  ...
  在当前命令里含糊混入。
  **归属状态:该 canonical 口径尚待 founder 追认**;本实现与验收会明确标注,不得把
  design review 的技术通过表述成 founder 已确认范围。
...
2. **RED**:`lead-patrol-snapshot.test.sh`(§4 的 CI 契约,真代码建库 + 双项目阳性对照)+ 加进 `ci.yml` 字面枚举。pane fixture 必须证明:列出的每个 Runner pane 都以 `-S -` 捕获、同 window name 的 `cmux-*` 镜像被排除、secret 原文不落报告、全 capture/状态 hash 与计数落盘、上一报告状态 hash 连续 1h 触发 `STALLED_60M`、limit 与 interactive menu 各有阳性、漏 capture 使步 2 UNAVAILABLE、逐 pane `action=REQUIRED` 使完成门失败。双项目 CommDB fixture 还要证明正常外项目 pane 映射为 `cross-boundary` 但不产生 finding、owner-index 读失败归 UNAVAILABLE、registry 外 pane 是 non-finding `foreign-registry`、registered target 首次消失 grace/第二 tick 才 orphan finding、重复 target fail-visible,跨界异常聚合一次而非逐 pane刷屏;完成门对 candidate、缺失/非数字 `pane_count` 都干净判否。
...
- **Founder 增量正式复审 Round 3**(gate `1b0104fe-d388-46d3-8532-970c9a791f43`,request `3101fd35-1e7e-45f0-8d0c-9670b791e344`):**APPROVED**。两个前轮 HIGH 均确认闭合。4 条 non-blocking advisory 全采纳:owner-index 读失败归 UNAVAILABLE 而非 unknown;registry 外 QA/临时 pane 归 `foreign-registry` 且存在本身不告警;registered target teardown race 给一 tick grace、连续两 tick 才判孤儿;canonical Runner-pane 口径明确标「待 founder 追认」。设计门通过,进入增量 TDD。
