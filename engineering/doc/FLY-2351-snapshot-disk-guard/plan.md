...
2GB 是所有受管副本获取/衍生生成的硬准入预算，不是操作系统给任意 shell 写入设的配额。runner 规则禁止绕过、禁止在目录外再复制生产库；会增长的验证操作要事先预算，空间不足则改用小 fixture 或顺序释放/验证。维护扫描发现外部违规增长必须报 over_budget，不能把主动超量后删除当作准入成功。该边界已问 Lead；答复需反映到文档和 HTML。
