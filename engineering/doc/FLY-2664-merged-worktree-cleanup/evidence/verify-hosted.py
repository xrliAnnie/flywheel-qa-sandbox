...
url=publication['url']
req=urllib.request.Request(url,headers={'User-Agent':'FLY-2664-design-verification'})
...
assert 'FLY-2664' in body and '【页面意见汇总】FLY-2664' in body
meta_csp = re.findall(r'<meta[^>]+http-equiv=[\"\']Content-Security-Policy[\"\'][^>]+content=[\"]([^\"]+)', body, re.I)
policy = header_csp + ' ' + ' '.join(meta_csp)
assert 'script-src' in policy and 'nonce-'+nonce in policy
