...
## FLY-2504 contract amendment: rework content never delivered

The run-holding producer now has two explicit causes. `holdUndeliverableTx` retains its existing terminal-recipient/grace/liveness checks. `openReworkContentUndeliverableTx` is the second cause: an exact replacement attempt completing without launch content evidence. Both use `finalizeUndeliverableHoldTx`; the run held SQL remains a single write site. The separate reroute-limit producer continues to emit a delivery-scoped operator door without holding the run.

For the new cause, the same transaction opens a deterministic undeliverable episode on the exact live rework delivery revision, holds the run, writes `delivery_reroute_operator_required` with `cause: rework_content_not_delivered`, `livenessVerdict: not_applicable`, and `liveness: {verdict: not_applicable, reason: content_never_sent}`, writes a stable `completion_transition_refused` receipt and enqueues exactly one `rework_completion_refused` alert. It does not also enqueue a delivery-reroute outcome alert. Repeat refusal is zero-write, including after the run is held. Identity conflict only records refusal and alert, preserving the healthy current delivery/run/episode.

The existing public cancel/reroute hold API and all 19 hold shapes remain unchanged. Cancelling the rework delivery settles the attempt and allows completion. Tests: `fly2504-rework-replacement-receipt.test.ts` verifies cancellation, repeat refusal and rollback on alert insertion failure; existing FLY-2278 cancellation and undeliverable tests retain their predicates.
