...
<div class="wrap">
  ...
  <div class="card purple" data-section="谁的答复算数">
    ...
    <table>
      ...
      <tr><td>回合问题</td><td>第几轮 + 变更说明 + 网页链接 + <strong>产物内容指纹</strong></td><td>runner 开回合时写入且不可再改;没提交产出连回合都开不了</td></tr>
      <tr><td>你的裁定</td><td>通过 / 打回 + 意见全文</td><td>只有核验过"作者是你"才写得进;写入者身份机器可查</td></tr>
      <tr><td>通过绑定版本</td><td>你的「通过」只对那一版产物有效</td><td>通过之后再改产物 → 指纹不符 → 必须重新送你 review,偷改无效</td></tr>
      <tr><td>通过只算本次任务的</td><td>按本次运行(run)记账,不按整个 issue</td><td>同一 issue 以前某次跑出来的旧「通过」,新一轮任务不能拿来复用</td></tr>
      <tr><td>服务端双门</td><td>完成入账门 + 合入(land)门</td><td>都在系统中枢(Bridge)里跑同一个判定、读同一份账本 —— runner 本地绕不过,也不会出现"一道门撤了另一道还卡着"的老毛病</td></tr>
    </table>
  ...
  <div class="card green" data-section="验收">
    <h2>8 · 怎么证明它真的挡得住</h2>
    <ul>
      <li>会写<strong>三个主动去破它</strong>的测试:伪造"我做完了"的上报、绕过收工直接推进、直接触发合入 —— 三条路都必须被服务端拦下,而不是一句"我们保证"。</li>
      <li>Lead 用命令代答 founder_review(不管是替你"通过"还是替你"打回")—— 必须被拒,测试覆盖。</li>
      <li>你通过了某一版之后 runner 又偷偷改了产物 —— 指纹不符,收工/合入必须失败,必须重新送你。</li>
      <li>同一 issue 上一次任务的旧「通过」,新任务拿来复用 —— 必须被拦。</li>
