...
从旧的 FLY-202 branch 复制 `sandbox-notes.md`，再修补目录与 README 差异。

- 优点：速度快，格式已经被多次 E2E 使用。
- 风险：容易漏掉当前分支变化，也会把旧 run 的措辞、日期或 tree output 带入新文档。
...
4. `packages/qa-framework/README.md` Summary，保持 10 个 bullet，覆盖框架定位、五步协议、slot lifecycle、镜像模式、证据边界与 contracts。
...
- GREEN：创建文档后重复运行同一脚本，验证三段 Purpose、17 个目录表项、10 个 README bullets，以及 fenced block 与实际命令输出逐行一致。
