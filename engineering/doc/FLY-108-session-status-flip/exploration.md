...
> 当前 worktree HEAD(`1855f7a1`,含 main 已 merge 的 PR #155)**已包含 FLY-108 的修复**,
...
3. Bridge `event-route.ts` 显式把 `stage_changed=completed` 当 informational only,不触发 FSM;
...
2. **terminal event 必须可靠**:`stage_changed` 丢了无所谓(informational),`session_completed` 丢了 = bug 原样复现。
...
4. **runPostShipFinalization 第一步就 kill tmux**:发射时机必须在 Runner 全部收尾动作(docs commit/push、worktree remove)之后,否则 Bridge 会杀掉还没做完收尾的 Runner。
...
3. **QA session role**:`complete` 只覆盖 `session_role=main`(Decision 4),QA session 的完成信号后续单独处理。
