...
for (const [key, file] of Object.entries(map)) {
	...
	if (!svg.includes(`id="FLY-2508-${file.replace(/\.svg$/, "")}"`))
		throw new Error(`${file} lacks unique svg id`);
...
if (html.includes("<!--DIAGRAM:") || html.includes("__REVIEW_META__"))
	throw new Error("placeholder left");
if ((html.match(/__CSP_NONCE__/g) ?? []).length !== 1)
	throw new Error("nonce placeholder count != 1");
if (/Content-Security-Policy/i.test(html)) throw new Error("own CSP present");
if (/<script(?![^>]*nonce="__CSP_NONCE__")/i.test(html))
	throw new Error("non-nonced script present");
if (/\son[a-z]+=/i.test(html.replace(/<svg[\s\S]*?<\/svg>/g, "")))
	throw new Error("inline handler attribute present");
