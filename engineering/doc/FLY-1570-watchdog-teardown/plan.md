...
## 3. 开工前裁定(相对 issue 原文的边界修正,已在设计 HTML 中向 Lead/founder 报备)

1. **`stuck-pane-confirm.ts` 不删** —— FLY-1234 心跳确认层(非 FLY-195 集群),HeartbeatService/watchdog-judge-assembly 硬依赖,删则编译失败 + 5/5 误报事故回归
2. **六个文件是「拆」不是「删」**:stuck-escalation / stuck-candidate / stuck-remanage-routes / detection-escalation / detection-gap-scan / detection-reconcile-tick 各托管保留侧共享符号或活接口(research.md §2.2)
