...
| `lead_said` | 两个来源合并去重：① 收件箱里 **Lead 主频道**（`chatChannel`，不含 thread）中 `origin=lead_authored` 的消息；② 标题的「要你答」（`level==="answer"`，也就是 founder_ask 或旧 founder_gate，本身就是 Lead 主动问她） | `said:<channelId>:<messageId>`，或 `ask:<askId>`；同一条消息经问题 binding 合并为一件 |
...
- `lead_said` 在她之后已经在同一频道发过言，这一件视为已处理（沿用 `hasFounderAttentionReplyAfter` 的语义）。
...
- 复用 FLY-2796 的持久 carrier（`voice_handoffs`，预分配 deliveryId，走 mailbox），新增 `intentKind: "agenda_brief"`，与 query、judgment、action 在同一个枚举源里定义（2796 §7.1 ④）。它**只请 Lead 写话**，不授权任何动作。
...
flywheel-comm voice agenda close --request <handoffId> --item <itemKey> --disposition resolved|deferred --reason "<一句话>"
```

- `say` 写入 2796 的 `voice_handoff_results`，`resultKind="agenda_say"`。`--order` 只在 `purpose=open` 时有效，而且必须是开场快照里 itemKey 的一个排列，否则拒收。
- `close` 写入 `resultKind="agenda_close"`。模式层收到后关闭这一件。
- 两条命令都要求：请求属于本 Lead、session 或 generation 仍然有效、itemKey 属于该快照（2796 §7.1 ⑦ 的鉴权）。
...
3. **口语**：
   ...
   - 不编造，不说还没做的动作「已经做了」。
4. **收尾**：她拍板后，Lead **先执行或记下**，再 `close`；她说「回头再看」就 `close --disposition deferred`。
...
| 兜底 | 用同一声线念模板句「<单号> 在等你<批/授权/回答>，细节我发在 thread 里了」，并记录 `agenda_fallback_spoken` 事件和原因。兜底是**可见的降级**，不冒充 Lead 原话 |

### 3.5 延迟

- 开场请求在会话 `live` 时立即发出，这段时间播 2796 的有界等待音。
- Lead 在 `agenda.leadReplyTimeoutMs`（工程默认 20000）内没有回复时，用同一声线说一句「我在整理，有 N 件要你拍，马上说」，然后继续等。
- 再过一个超时仍然没有回复，就按 §3.4 念兜底句，逐件处理。⛔ 不许长时间静默，也不许回退成念原文。
...
stateDiagram-v2
  ...
  ItemActive --> NextItem: close resolved/deferred，或来源已不在三类
...
| Q4 | 一件关闭的条件：① Lead `close`；或 ② 每次刷新快照时发现它已经不在三类里（比如她批了之后标题变了）。如果是当前这一件被 ② 关闭，就发 `item` 请求让 Lead 用一句话确认并接下一件 |
| Q5 | `deferred` 的条目本场不再提起；下一场如果它仍在三类里，重新入队 |
...
| Q8 | 队列状态持久化到 Bridge 表 `voice_agenda_state(sessionId, generation, snapshotId, order[], cursor, itemStates, urgentStack, lastActivityAt)`。重启或换 generation 后从持久状态接着走。它只管**议程条目**，会话状态的权威仍然是 `VoiceSessionState`（K5） |
...
- 她说话，前台（A 的快答或 B）先正常识别。
- **议程进行中**，这句话要作为 handoff（`intentKind` 按 2796 的 query、judgment、action 分类）连同 `currentItemKey` 一起交给 Lead，由 Lead 判断。⛔ 前台不能自己回答「好的已批」。
- A 的前台 instructions 要加一条：「议程件由 Lead 回答」。A 已有「需要查询、执行或判断时先说我问下 Lead」，这里是把它扩展到议程件。
...
| 收件箱、claim、ack | 保留；`lead_said` 被 Lead `close` 时，ack 对应的收件箱条目 |
...
| 语音说「批」不能直接生效 | 用嘴批 ship 不在 V2 范围内（2796 plan §1）。在那之前，Lead 如实说「我记下了，你在 thread 点一下」，这一件保持打开，直到标题状态真正变化（Q4 ②）。**这和她「说好就算结束」的期望有差距**，要在页面上讲清楚 |
...
| S1 | Bridge `GET /api/voice/agenda` 加 `originClass` 迁移 | 三类判定与 `readIssueTitleState` 对拍；排除清单逐项负测；scope（耳机/会议）；鉴权 403、409 |
| S2 | carrier 的 `agenda_brief` 和 comm 的 `voice agenda say/close` | 越权、错 item、错 order、过期 generation 一律拒收；幂等 |
| S3 | `AgendaConductor`（Q1–Q9、U1–U2、报平安） | FakeV1Session 加注入时钟：开场先报数再逐件；新件排到队尾；urgent 在安全边界插播并 resume；deferred 不重提；10 分钟报平安且不累积；重启后恢复 cursor |
| S4 | A：OpenAiTts 播报器、声线解析、会议和耳机分开接线 | 配置成 edge-tts 启动失败；voice 不一致报错；会议会话不读耳机收件箱 |
| S5 | B：接 `AgendaConductor`，prompt 去掉状态罗列 | 与 A 用同一组 fixture 逐字对比议程文本 |
| S6 | 真房 QA | 见下 |
...
2. 她说「好，批」或「授权」以后，这一件关闭（有 close 或状态变化的证据），然后进入下一件。
...
3. 标题上的「要你答」（Lead 在 thread 里问你的问题）算进「Lead 主动说的」这一类，对吗？
