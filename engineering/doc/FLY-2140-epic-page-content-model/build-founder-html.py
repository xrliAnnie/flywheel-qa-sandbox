...
diagram lost its <svg id="FLY-2140-dN"> root — never ship a visual artifact you did not check."""
...
diagrams = {1: "d1-core-flow.svg", 2: "d2-cell-model.svg", 3: "d3-four-questions.svg"}
for n, name in diagrams.items():
    p = here / "diagrams" / name
    svg = p.read_text(encoding="utf-8")
    if len(svg) < 2000:
        sys.exit(f"diagram {name} too small ({len(svg)} bytes)")
    if f'id="FLY-2140-d{n}"' not in svg:
        sys.exit(f"diagram {name} lacks svgId FLY-2140-d{n}")
...
print(f"wrote {target.name} ({size} bytes); svg roots: {out.count('<svg')}; sections: {out.count('data-title=')}")
if size > 512 * 1024:
