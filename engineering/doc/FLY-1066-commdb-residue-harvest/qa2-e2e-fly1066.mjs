...
console.log("\n=== A. POSITIVE CONTROL — the pre-fix bug is real ===");
{
	...
	raw
		.prepare(
			"INSERT INTO sessions (execution_id,tmux_window,project_name) VALUES ('exec-old','win-1','flywheel')",
		)
		.run();
	let threw = null;
	try {
		raw
			.prepare(
				"UPDATE sessions SET status='failed' WHERE execution_id='exec-old'",
			)
			.run();
	} catch (e) {
		threw = e;
	...
	ok(
		"pre-fix CommDB physically REJECTS status='failed' (CHECK violation)",
		threw !== null && /CHECK constraint/i.test(String(threw)),
		`got: ${threw}`,
	);
	const row = raw
		.prepare("SELECT status FROM sessions WHERE execution_id='exec-old'")
		.get();
	ok(
		"→ pre-fix row is therefore STUCK at 'running' (the FLY-1066 symptom)",
		row.status === "running",
		`status=${row.status}`,
	);
	raw.close();
...
console.log("\n=== B. Migration — old DB opened by new code ===");
{
	...
	raw
		.prepare(
			"INSERT INTO sessions (execution_id,tmux_window,project_name,issue_id,lead_id,vendor,status) VALUES ('keep-1','win-k','flywheel','FLY-1','lead-a','codex','completed')",
		)
		.run();
	raw
		.prepare(
			"INSERT INTO sessions (execution_id,tmux_window,project_name,status) VALUES ('keep-2','win-2','flywheel','running')",
		)
		.run();
	raw.close();
	...
	ok(
		"pre-existing row survives migration byte-for-byte",
		k1 &&
			k1.tmux_window === "win-k" &&
			k1.issue_id === "FLY-1" &&
			k1.lead_id === "lead-a" &&
			k1.vendor === "codex" &&
			k1.status === "completed",
	...
	ok(
		"second pre-existing row survives",
		db.getSession("keep-2")?.status === "running",
	);
	db.markSessionTerminalStatus("keep-2", "failed");
	ok(
		"'failed' is now writable post-migration",
		db.getSession("keep-2")?.status === "failed",
	);
	db.close();

	const db2 = new CommDB(p); // idempotency
	ok(
		"re-open is idempotent (data intact)",
		db2.getSession("keep-1")?.issue_id === "FLY-1",
	);
	db2.close();
}
...
console.log("\n=== C. PRODUCT SURFACE — what the Lead actually sees ===");
{
	...
	const beforeActive = db
		.getActiveSessions("flywheel")
		.map((s) => s.execution_id);
	ok(
		"baseline: a live runner IS in the active/running list",
		beforeActive.includes("exec-crash"),
	);
	...
	const afterActive = db
		...
		.map((s) => s.execution_id);
	ok(
		"FIX: crashed runner LEAVES the running list (no more phantom 'running')",
		!afterActive.includes("exec-crash"),
		`active=${JSON.stringify(afterActive)}`,
	);

	const term = db.getRecentTerminalSessions("flywheel", "lead-a", 50);
	const t = term.find((s) => s.execution_id === "exec-crash");
	ok(
		"FIX: it APPEARS in the Lead's terminal list as 'failed'",
		t?.status === "failed",
		`got=${t?.status}`,
	);
	ok(
		"FIX: countTerminalSessions counts it",
		db.countTerminalSessions("flywheel", "lead-a") === 1,
	);
	ok(
		"routing target (tmux_window) preserved for retry teardown",
		t?.tmux_window === "win-crash",
	);
	db.close();
}
...
);
{
	...
	ok(
		"ended_at stable across re-mark",
		first === second.ended_at,
		`${first} vs ${second.ended_at}`,
	);
...
);
{
	...
	ok(
		"adapter tail-write does NOT overwrite 'failed'",
		db.getSession("race")?.status === "failed",
	...
	ok(
		"adapter tail-write DOES retire a still-running row (control)",
		db.getSession("normal")?.status === "completed",
	);
	db.close();
}
...
);
{
	...
	ok(
		"late register does NOT flip a marked row back to 'running'",
		s?.status === "failed",
		`got=${s?.status}`,
	);
	db.close();
