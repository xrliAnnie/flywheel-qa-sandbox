...
| 4 | Audio device test (speakers + Bluetooth headphones) | in progress | Shokz OpenRun detected; `say` worked once, then audio became unavailable (likely BT disconnect) |
...
| 17 | Implement `tts.lua` | done | Uses `say` command via hs.task (hs.speech API broken on this macOS) |
| 18 | Config file with pane target + alias | done | `~/.claude/voice-loop/config.lua` with defaults in `config.lua` loader |
| 19 | E2E test: echo prompt in pane → hear TTS | in progress | Detection pipeline verified via logs; TTS audio pending BT reconnect |
